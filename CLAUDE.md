# SlotDeck Engineering Standard

This file is the authoritative engineering standard for SlotDeck. It defines the current product
...
- The interface identifier is `dev.slotdeck.PluginInterface/6.0`, represents only the current ABI and accepts no older ABI.
...
- Page headers, data grids, compact tool buttons, section titles, empty states, status indicators, settings forms, tab bars, Markdown documents, avatar discs, painted rounded surfaces, busy indicators, filter fields, row actions, the monospaced family catalog and local timestamp presentation come from the shared `slotdeck::ui` components and are never rebuilt inside a plugin.
...
- Plugins communicate through asynchronous JSON requests and events exposed by `PluginHost`.
- The core gives each plugin a dedicated host context bound to its validated identity.
- Plugins never provide or override their sender identity when requesting or publishing.
- A request identifies sender, target, topic and JSON object payload.
...
- Terminal integration is optional and requests snapshots through the asynchronous message bus only when the terminal plugin is available.
...
- An owner that abandons a child process disconnects from it first, because a read already queued is still delivered and would reach an owner that no longer holds it.
...
- The `src/plugins` directory owns the interface, discovery, lifecycle, localization and message bus.
...
- The `SlotDeckUi` library is the single build unit for the shared visual primitives and is linked by the core, the shared terminal engine and every plugin.
...
- The root C++ namespace is `slotdeck` and no feature name owns the application namespace.
...
- The `SlotDeckTerminalEngine` library is the single terminal implementation and plugins link it instead of duplicating terminal sources.
...
- The `plugins/ai` directory owns the provider catalog, the model connections, model discovery, the chat transport, the agent tools, the Model Context Protocol client, workspaces, kanban state, executions, execution logs, scheduling and UI.
...
- A service publishing a closed voice set is chosen from that set, while a service with an account catalog is typed and listed by the voice tool.
...
- The agent dialog offers a template that already carries the tags in place and a list of every tag with what it stands for, because a writer who cannot see them writes one that does not exist.
...
- Skills are discovered under the project roots `.claude/skills`, `.agents/skills`, `.skills` and `skills` and then under the machine roots `~/.claude/skills` and `~/.agents/skills`, in that order, and the first root declaring a name owns it.
- A skill whose front matter declares no description is skipped by name in the log and never drops the catalog around it.
- The catalog is read through the asynchronous filesystem service and kept for the run, because the prompt and the tools ask for it on every turn and a disk is not answered that often.
...
- The published context files `AGENTS.md`, `AGENT.md` and `CLAUDE.md` are always-on instructions and join the prompt in full, because they describe how the workspace itself expects work to be done.
...
- The chat owns its reading size with its own default and answers the shared zoom step like every other content surface.
...
- Other plugins may open a tab only through the asynchronous `browser.open` JSON request contract.
...
- The shared `slotdeck::test::waitUntil` delegates to `QTest::qWaitFor`, so Qt Test owns the event-loop aware waiting while GoogleTest owns every assertion and outcome.
...
- The doctor command is `python3 make.py doctor`.
...
- [x] The chat owns its reading size and answers the shared zoom step.
...
- [x] The complete suite builds and passes on real Windows hardware in the debug configuration, which continuous integration never exercises because it builds that platform in release.
...
- The `BACKLOG.md` file records the work that is deliberately not implemented yet and the reason for each decision.
- A rule reaches this file only when the product already guarantees it, so a plan never becomes a standard.
