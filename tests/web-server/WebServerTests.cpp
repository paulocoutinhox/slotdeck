...
TEST(StaticFileResolverTest, ResolvesFilesDirectoriesEncodingAndMimeTypes) {
    ...
    EXPECT_FALSE(plugins::webserver::StaticFileResolver::mimeType(QStringLiteral("unknown.slotdeck-extension")).isEmpty());
}
TEST(StaticFileResolverTest, RejectsInvalidRootsTraversalLinksAndUnreadableTargets) {
    plugins::webserver::StaticFileResolver missing(QStringLiteral("/missing/slotdeck/root"));
...
TEST(WebServerPluginTest, DeclaresMetadataRestoresStateAndSynchronizesTerminals) {
    ...
    EXPECT_EQ(host.requests.size(), 1);
...
TEST(WebServerPluginTest, OperatesWithoutTerminalAndRemovesIndependentConfigurationsTransactionally) {
    ...
    ASSERT_TRUE(plugin.initialize(host).hasValue());
    QApplication::processEvents();
    EXPECT_TRUE(host.requests.isEmpty());
    ...
    EXPECT_EQ(test::awaitFuture(plugin.removeWebServer(QStringLiteral("server-1"))).error().code, QStringLiteral("write_failed"));
    EXPECT_TRUE(plugin.webServerConfigured(QStringLiteral("server-1")));
    host.executeError.reset();
    EXPECT_TRUE(test::awaitFuture(plugin.removeWebServer(QStringLiteral("server-1"))).hasValue());
    EXPECT_FALSE(plugin.webServerConfigured(QStringLiteral("server-1")));
    EXPECT_EQ(test::awaitFuture(plugin.removeWebServer(QStringLiteral("server-1"))).error().code, QStringLiteral("web_server_configuration_missing"));
}
TEST(WebServerPluginTest, RejectsInvalidPersistentStateAndStorageFailures) {
    ...
    readFailureHost.queryError = utils::Error{QStringLiteral("read_failed"), QStringLiteral("Read failed"), {}};
...
TEST(WebServerPluginTest, ValidatesConfigurationRunsServerAndPreservesStateOnFailures) {
    ...
    EXPECT_EQ(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), {}, directory.path(), QStringLiteral("127.0.0.1"), 8080)).error().code, QStringLiteral("web_server_start_failed"));
    EXPECT_EQ(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), directory.path(), QStringLiteral("127.0.0.1"), 0)).error().code, QStringLiteral("web_server_start_failed"));
    EXPECT_EQ(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), QStringLiteral("/missing"), QStringLiteral("127.0.0.1"), 8080)).error().code, QStringLiteral("web_server_start_failed"));
    EXPECT_EQ(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), directory.path(), QStringLiteral("invalid"), 8080)).error().code, QStringLiteral("web_server_start_failed"));
    EXPECT_EQ(test::awaitFuture(plugin.startWebServer(QStringLiteral("server-1"))).error().code, QStringLiteral("web_server_start_failed"));

    host.executeError = utils::Error{QStringLiteral("write_failed"), QStringLiteral("Write failed"), {}};
    EXPECT_EQ(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), directory.path(), QStringLiteral("127.0.0.1"), 8080)).error().code, QStringLiteral("write_failed"));
    EXPECT_FALSE(plugin.webServerConfigured(QStringLiteral("server-1")));
    host.executeError.reset();
    ...
    portProbe.close();
    ASSERT_TRUE(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), directory.path(), QStringLiteral("127.0.0.1"), availablePort)).hasValue());
    ASSERT_TRUE(plugin.webServerRunning(QStringLiteral("server-1")));
    EXPECT_TRUE(test::awaitFuture(plugin.startWebServer(QStringLiteral("server-1"))).hasValue());
    EXPECT_EQ(test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), directory.path(), QStringLiteral("127.0.0.1"), availablePort)).error().code, QStringLiteral("web_server_start_failed"));
    ...
    socket.flush();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return plugin.requestLogEntriesSince(QStringLiteral("server-1"), 0, 10).entries.size() == 1; }));
    // clang-format on
    EXPECT_TRUE(test::awaitFuture(plugin.clearRequestLog(QStringLiteral("server-1"))));
    EXPECT_TRUE(plugin.requestLogEntriesSince(QStringLiteral("server-1"), 0, 10).entries.isEmpty());
    EXPECT_FALSE(test::awaitFuture(plugin.clearRequestLog(QStringLiteral("missing"))));
...
TEST(WebServerPluginTest, SerializesStartsAndCancelsThePendingRuntimeOperation) {
    ...
    test::TestPluginHost host;
    WebServerTestsHelper::configureWebDatabase(host);
    auto persistence = std::make_shared<QPromise<utils::Result<void>>>();
    persistence->start();
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return persistenceDeferred; }));
    // clang-format on
    const auto duplicate = test::awaitFuture(plugin.configureAndStartWebServer(QStringLiteral("server-1"), QStringLiteral("Preview"), directory.path(), QStringLiteral("127.0.0.1"), availablePort));
    ASSERT_FALSE(duplicate.hasValue());
    EXPECT_EQ(duplicate.error().code, QStringLiteral("web_server_start_failed"));
    plugin.stopWebServer(QStringLiteral("server-1"));
    persistence->addResult(utils::Result<void>::success());
    persistence->finish();
    const auto cancelled = test::awaitFuture(firstStart);
...
TEST(WebServerPluginTest, ValidatesEventsSnapshotsRemovalAndSettingsPersistence) {
    ...
    // clang-format off
    host.settingsFutureHandler = [](const QJsonObject&) { return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"write_failed", "Write failed", {}})); };
    // clang-format on
    ...
    // clang-format off
    plugin.handleRequest(QStringLiteral("terminal"), QStringLiteral("unknown"), {}, [&response](utils::Result<QJsonObject> result) { response = std::move(result); });
    // clang-format on
...
TEST(WebServerPluginTest, CancelsPendingPersistenceCallbacksDuringShutdown) {
    ...
    plugin.setSplitRatio(600);
    plugin.shutdown();
    operation->addResult(utils::Result<void>::failure({"write_failed", "Write failed", {}}));
    operation->finish();
    QApplication::processEvents();
...
TEST(WebServerPluginTest, OpensTheFormForAFolderAndNeverConfiguresTheSameRootTwice) {
    ...
    plugin.handleRequest(QStringLiteral("ai"), QStringLiteral("web-server.instance.create"), {{QStringLiteral("path"), directory.path()}}, collect);
    ...
    plugin.handleRequest(QStringLiteral("ai"), QStringLiteral("web-server.instance.create"), {{QStringLiteral("path"), fresh.path()}}, collect);
    ...
    plugin.handleRequest(QStringLiteral("ai"), QStringLiteral("web-server.instance.create"), {{QStringLiteral("path"), QDir(directory.path()).filePath(QStringLiteral("absent"))}}, collect);
...
TEST(WebServerDialogTest, ClosesItselfWhenTheServerItConfiguredIsRunning) {
    ...
    ui::WebServerDialog dialog(QStringLiteral("server-1"), {}, directory.path(), plugin);
    ...
    const QImage danger = ui::icon(ui::IconName::Stop, host.theme().color(ui::ThemeColor::Danger)).pixmap(32, 32).toImage();
    ...
    start->click();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return accepted.count() == 1; }));
    // clang-format on
    EXPECT_TRUE(plugin.webServerConfigured(QStringLiteral("server-1")));
...
TEST(WebServerViewTest, KeepsARowActionAliveWhileItIsDeliveringItsOwnClick) {
    ...
    table->selectRow(0);
    QApplication::processEvents();
    EXPECT_EQ(edit->icon().pixmap(32, 32).toImage(), ui::icon(ui::IconName::Edit, host.theme().color(ui::ThemeColor::OnAccent)).pixmap(32, 32).toImage());
    ...
    EXPECT_EQ(status->icon().pixmap(32, 32).toImage(), ui::icon(ui::IconName::WebServer, host.theme().color(ui::ThemeColor::OnAccent)).pixmap(32, 32).toImage());
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([edit, &host]() { return edit->icon().pixmap(32, 32).toImage() == ui::icon(ui::IconName::Edit, host.theme().color(ui::ThemeColor::OnAccent)).pixmap(32, 32).toImage(); }));
    // clang-format on
    table->clearSelection();
    QApplication::processEvents();
    EXPECT_EQ(edit->icon().pixmap(32, 32).toImage(), ui::icon(ui::IconName::Edit, host.theme().color(ui::ThemeColor::TextMuted)).pixmap(32, 32).toImage());
...
TEST(WebServerDialogTest, ExpandsInsteadOfCompressingControlsWhenAnErrorAppears) {
    ...
    ui::WebServerDialog dialog(QStringLiteral("server-1"), {}, {}, plugin);
    dialog.show();
    QApplication::processEvents();
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return error->isVisible(); }, 3000));
    // clang-format on
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return dialog.height() > initialHeight; }, 3000));
    // clang-format on
...
void WebServerTestsHelper::configureSnapshotReply(test::TestPluginHost& host, QJsonObject snapshot) {
    host.availablePlugins.insert(QStringLiteral("terminal"));
    // clang-format off
    host.requestHandler = [snapshot = std::move(snapshot)](const QString&, const QString&, const QJsonObject&, QObject*, plugins::PluginReply reply) { reply(utils::Result<QJsonObject>::success(snapshot)); };
    // clang-format on
}
...
void WebServerTestsHelper::configureWebDatabase(test::TestPluginHost& host, persistence::DatabaseRows configurations, int splitRatio) {
    ...
    host.queryHandler = [configurations = std::move(configurations)](const QString& statement, const QVariantList&) {
        if (statement.contains(QStringLiteral("web_server_configurations"))) {
            return utils::Result<persistence::DatabaseRows>::success(configurations);
        }
        return utils::Result<persistence::DatabaseRows>::failure({"unexpected_query", "The test received an unexpected database query", statement});
    };
    // clang-format on
}
} // namespace slotdeck
...
TEST(WebServerTranslationsTest, SpellsEveryKeyInEveryLanguageTheSelectorOffers) {
    slotdeck::plugins::webserver::WebServerPlugin plugin;
    slotdeck::test::expectCompleteCatalog(QStringLiteral("web-server"), plugin.translations());
}
