...
TEST(ResultTest, PreservesSuccessfulValuesAndFailures) {
    auto success = utils::Result<QString>::success(QStringLiteral("value"));
    ...
    auto failure = utils::Result<QString>::failure({QStringLiteral("code"), QStringLiteral("message"), QStringLiteral("detail")});
    ...
    EXPECT_TRUE(utils::Result<void>::success().hasValue());
    const auto emptyFailure = utils::Result<void>::failure(failure.error());
...
TEST(ApplicationLanguageTest, ResolvesSupportedRegionalBaseAndEnglishLanguages) {
    EXPECT_EQ(domain::supportedApplicationLanguages(), QStringList({QStringLiteral("en"), QStringLiteral("pt")}));
    EXPECT_EQ(domain::resolveApplicationLanguage(QStringLiteral("pt_BR")), QStringLiteral("pt"));
    EXPECT_EQ(domain::resolveApplicationLanguage(QStringLiteral("pt-PT")), QStringLiteral("pt"));
    EXPECT_EQ(domain::resolveApplicationLanguage(QStringLiteral("en-US")), QStringLiteral("en"));
    EXPECT_EQ(domain::resolveApplicationLanguage(QStringLiteral("de-DE")), QStringLiteral("en"));
    EXPECT_TRUE(domain::isSupportedApplicationLanguage(QStringLiteral("pt")));
    EXPECT_FALSE(domain::isSupportedApplicationLanguage(QStringLiteral("pt-br")));
}
...
TEST(StateStoreTest, CreatesAndPersistsCoreStateAndPluginOwnedSchema) {
...

    {
        ...
        EXPECT_TRUE(store.settings(QStringLiteral("slotdeck")).isEmpty());
        ASSERT_TRUE(store.markShutdown(false).hasValue());
        ASSERT_TRUE(store.saveSettings(QStringLiteral("slotdeck"), {{QStringLiteral("language"), QStringLiteral("en")}}).hasValue());
        EXPECT_EQ(store.settings(QStringLiteral("slotdeck")).value(QStringLiteral("language")).toString(), QStringLiteral("en"));
    ...
    EXPECT_EQ(restored.settings(QStringLiteral("slotdeck")).value(QStringLiteral("language")).toString(), QStringLiteral("en"));
...
TEST(StateStoreTest, ReportsPathMigrationAndTransactionErrors) {
    ...
    QTemporaryDir directory;
    ASSERT_TRUE(directory.isValid());
    persistence::StateStore missingParent(directory.filePath(QStringLiteral("missing/slotdeck.sqlite3")));
...
TEST(DatabaseExecutorTest, SerializesRuntimeQueriesAndReportsStorageErrorsAsynchronously) {
    ...
    ASSERT_TRUE(test::awaitFuture(firstInsert).hasValue());
    ASSERT_TRUE(test::awaitFuture(secondInsert).hasValue());

    const auto rows = test::awaitFuture(executor.queryPluginDatabase(QStringLiteral("sample"), QStringLiteral("SELECT id, value FROM sample_values ORDER BY id"), {}));
    ASSERT_TRUE(rows.hasValue());
    ...
    const auto duplicate = test::awaitFuture(executor.executePluginDatabase(QStringLiteral("sample"), QStringLiteral("INSERT INTO sample_values(id, value) VALUES(?, ?)"), {1, QStringLiteral("duplicate")}));
    EXPECT_EQ(duplicate.error().code, QStringLiteral("database_execute_failed"));
    const auto invalidOwner = test::awaitFuture(executor.queryPluginDatabase(QStringLiteral("Invalid"), QStringLiteral("SELECT id FROM sample_values"), {}));
...
TEST(ConfigurationTransferTest, ExportsStagesAndAppliesACompleteDatabaseSnapshot) {
...

    {
        ...
        const auto conflict = test::awaitFuture(persistence::ConfigurationTransfer::exportDatabase(sourcePath, sourcePath));
    ...
    const auto incompatible = test::awaitFuture(persistence::ConfigurationTransfer::stageImport(exportPath, directory.filePath(QStringLiteral("incompatible.sqlite3")), {{QStringLiteral("sample"), 2}}));
...
TEST(ConfigurationTransferTest, RestoresThePreviousDatabaseWhenAnAppliedImportIsRejected) {
    ...
    ASSERT_TRUE(test::awaitFuture(persistence::ConfigurationTransfer::stageImport(exportPath, pendingPath, {})).hasValue());
    ...
    ASSERT_TRUE(restored.initialize().hasValue());
    EXPECT_TRUE(restored.settings(QStringLiteral("slotdeck")).isEmpty());
}
...
TEST(ConfigurationTransferTest, RecoversAnInterruptedAppliedImportWithoutReapplyingIt) {
    ...
    ASSERT_TRUE(test::awaitFuture(persistence::ConfigurationTransfer::stageImport(sourcePath, pendingPath, {})).hasValue());
    const auto applied = persistence::ConfigurationTransfer::beginPendingImport(currentPath, pendingPath, backupPath, {});
...
TEST(ConfigurationManagerTest, RejectsConcurrentConfigurationTransfers) {
    ...
    std::optional<utils::Result<void>> concurrentResult;
    // clang-format off
    QObject::connect(&manager, &app::ConfigurationManager::transferStateChanged, &manager, [&manager, &directory, &concurrentResult](bool active) {
        if (active && !concurrentResult.has_value()) {
            concurrentResult = test::awaitFuture(manager.exportConfiguration(directory.filePath(QStringLiteral("export.sqlite3"))));
        }
    });
    // clang-format on

    auto first = manager.importConfiguration(directory.filePath(QStringLiteral("missing.sqlite3")));
    EXPECT_FALSE(test::awaitFuture(first).hasValue());
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return stateChanged.count() == 2; }));
    // clang-format on
...
TEST(ApplicationSettingsTest, LoadsPersistsAndValidatesTheApplicationSettingsDocument) {
...

    {
        persistence::StateStore store(path);
        {
            ...
            EXPECT_EQ(settings.language(), domain::resolveApplicationLanguage(QLocale::system().name(QLocale::TagSeparator::Dash)));
...
TEST(ApplicationSettingsTest, KeepsTheDeclaredDefaultForEveryValueItCannotUse) {
...

    {
        persistence::StateStore store(path);
        ASSERT_TRUE(store.initialize().hasValue());
        ASSERT_TRUE(store.saveSettings(QStringLiteral("slotdeck"), {{QStringLiteral("themeId"), QStringLiteral("unavailable")}}).hasValue());
    }
    ...
    ASSERT_TRUE(store.saveSettings(QStringLiteral("slotdeck"), {{QStringLiteral("nobodyDeclaresThis"), true}}).hasValue());
    ...
    for (const auto& document : mistyped) {
        ...
        EXPECT_TRUE(domain::isSupportedApplicationLanguage(mistypedSettings.language()));
    }
}
...
TEST(ApplicationSettingsTest, RollsBackLanguageWhenAsynchronousPersistenceFails) {
    ...
    persistence::DatabaseExecutor unavailableExecutor(directory.filePath(QStringLiteral("missing/slotdeck.sqlite3")));
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return saveFailed.count() == 1; }, 3000));
    // clang-format on
...
TEST(ToastOverlayTest, StacksBoundedToastsAndDismissesThemWithoutChangingHostLayout) {
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return overlay->findChildren<QWidget*>(QStringLiteral("toast")).size() <= 4; }, 3000));
    // clang-format on

    overlay->applyTheme(*catalog.find(QStringLiteral("red")));
    overlay->dismissAll();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return overlay->findChildren<QWidget*>(QStringLiteral("toast")).isEmpty(); }, 3000));
    // clang-format on
    EXPECT_EQ(content->geometry(), contentGeometry);
}
...
TEST(SharedComponentsTest, BuildsThePageHeaderGridButtonsAndLabelsFromTheActiveTheme) {
    ...
    auto* grid = ui::dataGrid({QStringLiteral("Name"), QStringLiteral("Value")}, &host);
    ...
    auto* button = ui::toolButton(ui::IconName::Refresh, theme, QStringLiteral("Refresh"), &host);
    ...
    auto* section = ui::sectionTitleLabel(QStringLiteral("General"), &host);
    EXPECT_EQ(section->objectName(), QStringLiteral("settingsSectionTitle"));
    auto* empty = ui::emptyStateLabel(QStringLiteral("Nothing here"), &host);
    ...
    EXPECT_EQ(ui::localTimestamp(captured), QLocale::system().toString(captured.toLocalTime(), QLocale::ShortFormat));
}
...
TEST(StoredValuesTest, NormalizesTimestampsAndRejectsEveryInvalidStoredValue) {
    ...
    const QString stored = persistence::storedTimestamp(local);
    EXPECT_TRUE(stored.endsWith(QLatin1Char('Z')));
    EXPECT_EQ(persistence::parseStoredTimestamp(stored).toMSecsSinceEpoch(), local.toMSecsSinceEpoch());
    EXPECT_TRUE(persistence::validStoredTimestamp(persistence::parseStoredTimestamp(stored)));

    EXPECT_FALSE(persistence::parseStoredTimestamp(QString{}).isValid());
    EXPECT_FALSE(persistence::parseStoredTimestamp(QStringLiteral("not a timestamp")).isValid());
    EXPECT_FALSE(persistence::validStoredTimestamp(QDateTime{}));
    EXPECT_FALSE(persistence::validStoredTimestamp(local));
    EXPECT_FALSE(persistence::validStoredTimestamp(persistence::parseStoredTimestamp(QStringLiteral("2026-08-15T09:00:00.000-03:00"))));

    qint64 value = -1;
    EXPECT_TRUE(persistence::readStoredInteger(QVariant(7), value));
    EXPECT_EQ(value, 7);
    EXPECT_TRUE(persistence::readStoredInteger(QVariant(qint64{9000000000}), value));
    EXPECT_EQ(value, 9000000000);
    EXPECT_FALSE(persistence::readStoredInteger(QVariant(1.5), value));
    EXPECT_FALSE(persistence::readStoredInteger(QVariant(QStringLiteral("7")), value));
    EXPECT_FALSE(persistence::readStoredInteger(QVariant{}, value));
}
...
TEST(PluginPayloadTest, AcceptsExactKeysAndExactIntegersOnly) {
    ...
    EXPECT_TRUE(plugins::hasExactKeys(payload, {QStringLiteral("limit"), QStringLiteral("beforeSequence")}));
    EXPECT_FALSE(plugins::hasExactKeys(payload, {QStringLiteral("limit")}));
    EXPECT_FALSE(plugins::hasExactKeys(payload, {QStringLiteral("limit"), QStringLiteral("beforeSequence"), QStringLiteral("extra")}));
    EXPECT_TRUE(plugins::hasExactKeys(QJsonObject{}, {}));

    qint64 value = -1;
    EXPECT_TRUE(plugins::readJsonInteger(QJsonValue(42), value));
    EXPECT_EQ(value, 42);
    EXPECT_FALSE(plugins::readJsonInteger(QJsonValue(1.5), value));
    EXPECT_FALSE(plugins::readJsonInteger(QJsonValue(QStringLiteral("42")), value));
    EXPECT_FALSE(plugins::readJsonInteger(QJsonValue(), value));
    EXPECT_FALSE(plugins::readJsonInteger(QJsonValue(std::numeric_limits<double>::infinity()), value));
    EXPECT_FALSE(plugins::readJsonInteger(QJsonValue(9.3e18), value));
}
...
TEST(ThemeTest, ResolvesEveryStyleTokenAndNeverLeavesAPrefixBehind) {
    ...
    const QString resolved = ui::applyThemeTokens(QStringLiteral("@text @textMuted @border @borderStrong @accent @accentHover @accentStrong @onAccent @onDanger @danger @dangerHover @dangerBackground @dangerText @window @panel @raised @hover @success @warning @terminal @badgeRadius @badgeHorizontalPadding @badgeVerticalPadding @controlRadius @controlHorizontalPadding @controlVerticalPadding @scrollBarExtent @interfaceFontSize @pageTitleFontSize @sectionTitleFontSize"), theme);
    ...
    EXPECT_EQ(ui::applyThemeTokens(QStringLiteral("@textMuted"), theme), theme.color(ui::ThemeColor::TextMuted).name());
    EXPECT_EQ(ui::applyThemeTokens(QStringLiteral("@borderStrong"), theme), theme.color(ui::ThemeColor::BorderStrong).name());
    EXPECT_EQ(ui::applyThemeTokens(QStringLiteral("@accentHover"), theme), theme.color(ui::ThemeColor::AccentHover).name());
    EXPECT_EQ(ui::applyThemeTokens(QStringLiteral("@dangerBackground"), theme), theme.color(ui::ThemeColor::DangerBackground).name());
}
...
TEST(IconsTest, RendersEveryDeclaredIcon) {
    ...
    for (const auto name : names) {
        const QIcon icon = ui::icon(name, theme);
...
TEST(IconsTest, RendersEveryVectorIconAndLayoutVariant) {
    ...
    for (const auto name : names) {
        ...
        EXPECT_FALSE(ui::icon(name, QColor(Qt::red)).pixmap(32, 32).isNull());
    }

    EXPECT_FALSE(ui::layoutIcon(QStringLiteral("3-left"), 2, 2, 3, ui::themeManager().theme()).pixmap(32, 32).isNull());
    EXPECT_FALSE(ui::layoutIcon(QStringLiteral("3-bottom"), 2, 2, 3, ui::themeManager().theme()).pixmap(32, 32).isNull());
    EXPECT_FALSE(ui::layoutIcon(QStringLiteral("4-grid"), 2, 2, 4, ui::themeManager().theme()).pixmap(32, 32).isNull());
}
...
TEST(ComponentsTest, BreaksALabelBetweenItsWordsUntilOneWordHasNoBoundaryLeft) {
    ...
    const int fits = ui::longestWordWidth(label, metrics) + 4;
    EXPECT_EQ(ui::labelWrapping(label, metrics, fits), Qt::TextWordWrap);

    // A single word wider than the space it has left has no word boundary to break at.
    EXPECT_EQ(ui::labelWrapping(QStringLiteral("Configurações"), metrics, 8), Qt::TextWrapAnywhere);
    EXPECT_EQ(ui::longestWordWidth(QStringLiteral("Editor de Código"), metrics), ui::longestWordWidth(QStringLiteral("Código"), metrics));
    EXPECT_EQ(ui::longestWordWidth(QString{}, metrics), 0);
}

TEST(ModeBarTest, ExpandsForTranslatedWordsWrapsLabelsAndEmitsModeRequests) {
    ui::ModeBar bar;
    const int initialWidth = bar.sizeHint().width();
    bar.addMode(QStringLiteral("terminal/main"), ui::icon(ui::IconName::Terminal, ui::themeManager().theme()), QStringLiteral("Terminal"), plugins::NavigationPlacement::Primary);
    bar.addMode(QStringLiteral("slotdeck/settings"), ui::icon(ui::IconName::Settings, ui::themeManager().theme()), QStringLiteral("Configurações avançadas"), plugins::NavigationPlacement::Secondary);
    ...
    bar.addMode(QStringLiteral("system-information/overview"), ui::icon(ui::IconName::System, ui::themeManager().theme()), QStringLiteral("Unwrappablylongnavigationword"), plugins::NavigationPlacement::Secondary);
    EXPECT_LE(bar.sizeHint().width(), ui::themeManager().theme().metric(ui::ThemeMetric::ModeBarMaximumWidth));
    EXPECT_GE(bar.sizeHint().width(), ui::themeManager().theme().metric(ui::ThemeMetric::ModeBarMinimumWidth));
...
TEST(SettingsViewTest, LetsTheDividerAndTheGridReachTheEdgeAndInsetsEverythingElse) {
    ...
    const ui::SettingsSectionFactory createSection = [](const QString&, const QString&, QWidget* parent) {
        ...
        ui::addSettingsRow(form, QStringLiteral("Caption"), new QLineEdit(page));
        layout->addLayout(form);
        layout->addWidget(ui::dataGrid({QStringLiteral("Column")}, page));
        return page;
    };
    // clang-format on

    const plugins::SettingsSection first{QStringLiteral("first"), QStringLiteral("slotdeck.settings.title"), {QStringLiteral("slotdeck.settings.search")}};
    const plugins::SettingsSection second{QStringLiteral("second"), QStringLiteral("slotdeck.settings.title"), {QStringLiteral("slotdeck.settings.search")}};
    const plugins::SettingsGroup group{QStringLiteral("application"), QStringLiteral("slotdeck.settings.title"), {first, second}};

    ui::SettingsView view(manager, {{group, createSection}});
    view.resize(900, 700);
    view.show();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&view]() { return view.findChild<QTableWidget*>() != nullptr && view.findChild<QTableWidget*>()->width() > 0; }));
    // clang-format on
...
TEST(SettingsViewTest, WritesEverySectionTitleInUpperCase) {
    auto* title = ui::sectionTitleLabel(QStringLiteral("Model Connections"), nullptr);
...
TEST(DataGridTest, KeepsTheActionsOfASelectedRowReadableThroughEveryRebuild) {
    ...
    QTableWidget* grid = ui::dataGrid({QStringLiteral("Name"), QStringLiteral("Actions")}, nullptr);
    // clang-format off
    const auto fillRow = [grid, &theme]() {
        ...
        layout->setContentsMargins(0, 0, 0, 0);
        layout->addWidget(ui::rowActionButton(ui::IconName::Start, ui::ThemeColor::Accent, theme, QStringLiteral("Start"), actions));
        layout->addStretch(1);
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([grid]() { return grid->cellWidget(0, 1) != nullptr; }));
    // clang-format on
    EXPECT_EQ(glyph(), ui::icon(ui::IconName::Start, theme.color(ui::ThemeColor::Accent)).pixmap(32, 32).toImage());

    // A selected row is painted in the accent, so its actions switch to the ink that reads on it.
    grid->selectRow(0);
    QCoreApplication::processEvents();
    EXPECT_EQ(glyph(), ui::icon(ui::IconName::Start, theme.color(ui::ThemeColor::OnAccent)).pixmap(32, 32).toImage());

    // A rebuilt row carries new actions and keeps the selection it had, so they are readable without a second click.
    fillRow();
    grid->selectRow(0);
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&glyph, &theme]() { return glyph() == ui::icon(ui::IconName::Start, theme.color(ui::ThemeColor::OnAccent)).pixmap(32, 32).toImage(); }));
    // clang-format on
...
TEST(ComponentsTest, OpensASurfaceOverTheApplicationAsAModalWindowOfThePlatform) {
    QDialog surface;
    ui::showDialogWindow(&surface, QStringLiteral("Preview"));
...
TEST(SettingsViewTest, GivesACaptionTheWidthItsOwnWordsNeed) {
    ...
    auto* form = ui::settingsForm();
    ui::addSettingsRow(form, shortCaption, new QLineEdit(page));
    ui::addSettingsRow(form, longCaption, new QLineEdit(page));
    ui::addSettingsRow(form, unreadableCaption, new QLineEdit(page));
    pageLayout->addLayout(form);
    page->resize(1100, 400);
    page->show();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([page]() { return page->findChild<QLineEdit*>() != nullptr && page->findChild<QLineEdit*>()->width() > 0; }));
    // clang-format on
    ...
    const int cap = ui::themeManager().theme().metric(ui::ThemeMetric::SettingsLabelMaximumWidth);
...
TEST(SettingsViewTest, CombinesCoreGroupsWithMultipleSectionLayouts) {
    ...
    const plugins::SettingsSection general{QStringLiteral("general"), QStringLiteral("slotdeck.settings.title"), {QStringLiteral("slotdeck.settings.search")}};
    const plugins::SettingsSection interfaceSection{QStringLiteral("interface"), QStringLiteral("slotdeck.settings.title"), {QStringLiteral("slotdeck.settings.search")}};
    const plugins::SettingsSection notificationsSection{QStringLiteral("notifications"), QStringLiteral("slotdeck.settings.title"), {QStringLiteral("slotdeck.settings.search")}};
    const plugins::SettingsGroup application{QStringLiteral("application"), QStringLiteral("slotdeck.settings.title"), {general}};
    const plugins::SettingsGroup experience{QStringLiteral("experience"), QStringLiteral("slotdeck.settings.title"), {interfaceSection, notificationsSection}};
...
TEST(SharedComponentsTest, SortsASelectableListAlphabeticallyAndKeepsWhatWasSelected) {
    ui::ComboBox box(ui::themeManager().theme(), nullptr);
    ...
    ui::ComboBox editable(ui::themeManager().theme(), nullptr);
    ...
    ui::ComboBox empty(ui::themeManager().theme(), nullptr);
    ui::sortComboBoxItems(&empty);
...
TEST(IconsTest, PaintsEveryIconInTheColourItIsAskedFor) {
    ...
    for (const auto name : ui::allIconNames()) {
        const QImage image = ui::icon(name, asked).pixmap(48, 48).toImage();
        ASSERT_FALSE(image.isNull()) << static_cast<int>(name);
        int opaque = 0;
        for (int y = 0; y < image.height(); ++y) {
            for (int x = 0; x < image.width(); ++x) {
...
TEST(CalendarPopupTest, OpensFromTheIndicatorAndPaintsEveryDayOfTheMonthItIsReading) {
    ...
    QWidget host;
    host.resize(600, 400);
    host.setStyleSheet(ui::applicationStyleSheet(theme));
    host.show();
...
TEST(ConfigurationTransferTest, RejectsInvalidSourcesDestinationsAndPendingImports) {
    ...
    const auto invalidImport = test::awaitFuture(persistence::ConfigurationTransfer::stageImport(invalidPath, directory.filePath(QStringLiteral("pending.sqlite3")), {}));
    EXPECT_EQ(invalidImport.error().code, QStringLiteral("configuration_database_invalid"));
    const auto invalidExport = test::awaitFuture(persistence::ConfigurationTransfer::exportDatabase(invalidPath, directory.filePath(QStringLiteral("missing/export.sqlite3"))));
...
TEST(ConfigurationTransferTest, RejectsReadableDatabasesWithCorruptedCoreState) {
    ...
    const QString schemaPath = CoreTestsHelper::createValidDatabase(directory, QStringLiteral("schema.sqlite3"));
    CoreTestsHelper::executeSqliteStatements(schemaPath, {QStringLiteral("ALTER TABLE core_state RENAME TO core_state_old"), QStringLiteral("CREATE TABLE core_state(singleton INTEGER PRIMARY KEY, clean_shutdown INTEGER NOT NULL) STRICT"), QStringLiteral("INSERT INTO core_state SELECT * FROM core_state_old"), QStringLiteral("DROP TABLE core_state_old")});
    const auto invalidSchema = test::awaitFuture(persistence::ConfigurationTransfer::stageImport(schemaPath, directory.filePath(QStringLiteral("schema-pending.sqlite3")), {}));
    EXPECT_EQ(invalidSchema.error().code, QStringLiteral("configuration_database_invalid"));

    const QString statePath = CoreTestsHelper::createValidDatabase(directory, QStringLiteral("state.sqlite3"));
    CoreTestsHelper::executeSqliteStatements(statePath, {QStringLiteral("DELETE FROM core_state")});
    const auto invalidState = test::awaitFuture(persistence::ConfigurationTransfer::stageImport(statePath, directory.filePath(QStringLiteral("preferences-pending.sqlite3")), {}));
...
TEST(ApplicationTest, ValidatesStartupLockInterfaceLifecycleAndRecovery) {
    ...
    const QString persistedTheme = QStringLiteral("blue");
    {
        ...
        auto* settingsModeBefore = mainWindow->findChild<QWidget*>(QStringLiteral("slotdeck/settings"));
        ...
        EXPECT_EQ(language->count(), domain::supportedApplicationLanguages().size());
        ...
        auto* settingsMode = mainWindow->findChild<QWidget*>(QStringLiteral("slotdeck/settings"));
        ...
        EXPECT_EQ(ui::themeManager().theme().id(), persistedTheme);
        EXPECT_TRUE(mainWindow->styleSheet().contains(ui::themeManager().theme().color(ui::ThemeColor::Accent).name()));
    ...
    const QString statePath = directory.filePath(QStringLiteral("slotdeck.sqlite3"));
    ...
    const QJsonObject document = state.settings(QStringLiteral("slotdeck"));
...
TEST(PluginManagerIntegrationTest, DiscoversInitializesAndBuildsThePluginDrivenInterface) {
    ...
    {
        app::ConfigurationManager configurationManager(databaseExecutor, directory.filePath(QStringLiteral("slotdeck-import.sqlite3")), manager.databaseSchemaVersions());
        const auto coreSettings = ui::applicationSettingsContributions(manager, settings, configurationManager);
        ...
        QApplication::processEvents();
        EXPECT_EQ(window.windowTitle(), QStringLiteral("SlotDeck"));
