...
class FakeChatClient : public AiChatClient {
  public:
    ...
    void fail(const utils::Error& error) {
        m_running = false;
        emit failed(error);
    }
...
inline QString AiTestsHelper::sleepingCommand(int seconds) {
    return test::sleepingCommand(seconds);
}
...
inline ModelConnection AiTestsHelper::testConnection() {
    ...
    return {provider->id, model, {}, QStringLiteral("sk-test"), {}, defaultParameters(*provider, model), {}};
}
...
inline void AiTestsHelper::installEmptyProviderRows(test::TestPluginHost& host) {
    // clang-format off
    host.queryHandler = [](const QString&, const QVariantList&) { return utils::Result<persistence::DatabaseRows>::success({}); };
    // clang-format on
}
...
inline void AiTestsHelper::installAiRows(test::TestPluginHost& host, const QVector<AiWorkspace>& workspaces, const QVector<AiTask>& tasks, const QStringList& queued, const QVector<ModelConnection>& connections, const QVector<AiAgent>& agents) {
    // clang-format off
    host.settingsDocument = AiTestsHelper::settingsDocument(connections, connections.isEmpty() ? QString{} : connectionKey(connections.first()), agents);
    host.queryHandler = [workspaces, tasks, queued](const QString& statement, const QVariantList&) {
        ...
        return utils::Result<persistence::DatabaseRows>::success(AiTestsHelper::selectedColumns(statement, rows));
    };
    // clang-format on
}
...
inline QJsonObject streamEvent(const QByteArray& json) {
