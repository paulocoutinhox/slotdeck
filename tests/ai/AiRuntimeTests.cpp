...
TEST(AiToolRegistryTest, WritesAGeneratedImageWhereTheAgentAsksAndReportsProviderFailures) {
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return !results.isEmpty(); }));
    // clang-format on
...
TEST(AiToolRegistryTest, WritesGeneratedSpeechWhereTheAgentAsksAndRefusesWithoutConfiguration) {
    ...
    registry.setSpeechConfiguration({SpeechProvider::ElevenLabs, QStringLiteral("voice-42"), QStringLiteral("xi-secret")}, QStringLiteral("http://127.0.0.1:%1").arg(server.serverPort()));
...
TEST(AiMcpClientTest, InitializesDiscoversToolsAndCallsThemOverTheStdioTransport) {
    ...
    QSignalSpy initialized(&client, &AiMcpClient::initialized);
    ...
    client.start();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return initialized.count() == 1 && toolsChanged.count() == 1; }));
    // clang-format on
    ...
    client.ping(collect);
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return replies.size() == 2; }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return replies.size() == 3; }));
    // clang-format on
...
TEST(AiCommandRunnerTest, KeepsOnlyTheReadableTextOfACommandThatDrawsInTheTerminal) {
    ...
    EXPECT_EQ(plainCommandOutput(pending, coloured), QStringLiteral("help    Show this help\n"));
    EXPECT_TRUE(pending.isEmpty());

    // A window title, a cursor move and a bell are not text the execution recorded.
    EXPECT_EQ(plainCommandOutput(pending, QString::fromUtf8("\x1b]0;building\x07ready\n")), QStringLiteral("ready\n"));
    EXPECT_EQ(plainCommandOutput(pending, QString::fromUtf8("\x1b[2K\x1b[1Gdone\n")), QStringLiteral("done\n"));

    // A sequence split across two reads is completed by the next chunk instead of leaking its bytes.
    EXPECT_EQ(plainCommandOutput(pending, QString::fromUtf8("first\x1b[3")), QStringLiteral("first"));
    EXPECT_FALSE(pending.isEmpty());
    EXPECT_EQ(plainCommandOutput(pending, QString::fromUtf8("6msecond")), QStringLiteral("second"));
    EXPECT_TRUE(pending.isEmpty());

    // A progress rewrite becomes its own line and a carriage return before a newline is not doubled.
    EXPECT_EQ(plainCommandOutput(pending, QStringLiteral("10%\r20%\r\n")), QStringLiteral("10%\n20%\n"));
...
TEST(AiProviderCatalogTest, BoundsTheOutputBudgetByWhatTheSelectedModelAccepts) {
    ...
    const ModelDescriptor* model = findModel(*anthropic, QStringLiteral("claude-opus-5"));
    ASSERT_NE(model, nullptr);
    ASSERT_GT(model->maximumOutputTokens, 0);

    const auto parameters = applicableParameters(*anthropic, model->id);
    // clang-format off
    ...
    QJsonObject invalid = defaultParameters(*anthropic, model->id);
    invalid[QStringLiteral("maxOutputTokens")] = model->maximumOutputTokens + 1;
    EXPECT_FALSE(validateParameters(*anthropic, model->id, invalid).hasValue());
    EXPECT_TRUE(validateParameters(*anthropic, model->id, defaultParameters(*anthropic, model->id)).hasValue());

    // A model the catalog does not declare keeps the provider wide bound instead of inventing one.
    const auto userDefined = applicableParameters(*anthropic, QStringLiteral("claude-custom"));
    // clang-format off
...
TEST(AiModelDiscoveryTest, ReadsTheCatalogEachProtocolPublishesAndReportsAnEmptyOne) {
    ...
    QObject::connect(&discovery, &AiModelDiscovery::failed, &discovery, [&failures](const utils::Error& error) { failures.append(error); });
    // clang-format on

    discovery.discover(QStringLiteral("openai"), QStringLiteral("sk-test"), catalog.address());
    EXPECT_TRUE(discovery.running());
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return !discovered.isEmpty(); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return discovered.size() == 2; }));
    // clang-format on
    ...
    discovery.discover(QStringLiteral("openai"), QStringLiteral("{env.SLOTDECK_ABSENT_KEY}"), catalog.address());
...
TEST(AiToolRegistryTest, HandsAnImageToAModelThatSeesAndRefusesForOneThatDoesNot) {
    ...
    ASSERT_TRUE(modelTraits(*findProvider(seeingConnection.providerId), seeingConnection.modelId).contains(ModelTrait::Vision));
    ...
    ASSERT_FALSE(modelTraits(*findProvider(blind.providerId), blind.modelId).contains(ModelTrait::Vision));
...
TEST(AiToolRegistryTest, AcceptsAPathInsideTheWorkingDirectoryHoweverItWasWritten) {
    ...
    registry.invoke({QStringLiteral("w3"), QStringLiteral("write_file"), QJsonObject{{QStringLiteral("path"), QDir::rootPath() + QStringLiteral("slotdeck-escaped.txt")}, {QStringLiteral("content"), QStringLiteral("x")}}}, root, collect);
    // clang-format off
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return results.size() == 5; }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return results.size() == 6; }));
    // clang-format on
...
TEST(AiToolRegistryTest, NamesTheArgumentACallGotWrongInsteadOfLeavingItToGuess) {
    ...
    EXPECT_FALSE(findToolArgumentError(schema, QJsonObject{{QStringLiteral("count"), 3}, {QStringLiteral("extra"), QStringLiteral("kept")}}).has_value());
    EXPECT_TRUE(findToolArgumentError(schema, QJsonObject{{QStringLiteral("count"), QStringLiteral("three")}}).has_value());
    EXPECT_EQ(findToolArgumentError(schema, QJsonObject{}).value().argument, QStringLiteral("count"));
}
...
TEST(AiToolRegistryTest, StopsACommandStillRunningForACallItWasAskedToCancel) {
    ...
    registry.invoke({QStringLiteral("c1"), QStringLiteral("run_command"), QJsonObject{{QStringLiteral("command"), AiTestsHelper::sleepingCommand(3)}, {QStringLiteral("timeout_seconds"), 0}}}, workdir.path(), collect);
    // clang-format off
    ASSERT_FALSE(test::waitUntil([&]() { return !results.isEmpty(); }, 600));
    // clang-format on
...
TEST(AiToolRegistryTest, OwnsTheCompleteFilesystemSetInsideTheWorkingDirectory) {
    ...
    const auto invoke = [&](const QString& name, const QJsonObject& arguments) {
        ...
        return test::waitUntil([&]() { return static_cast<int>(results.size()) > before; });
    };
    // clang-format on
...
TEST(AiToolRegistryTest, ListsTheVoicesEachSpeechServicePublishes) {
    ...
    registry.setSpeechConfiguration({SpeechProvider::OpenAi, QStringLiteral("alloy"), QStringLiteral("sk")}, speechProviderAddress(SpeechProvider::OpenAi));
    ...
    ASSERT_TRUE(catalog.listen());
    registry.setSpeechConfiguration({SpeechProvider::ElevenLabs, QStringLiteral("voice"), QStringLiteral("xi-secret")}, catalog.address());
    ...
    registry.setSpeechConfiguration({SpeechProvider::ElevenLabs, QStringLiteral("voice"), QStringLiteral("xi-secret")}, rejecting.address());
...
TEST(AiPluginTest, SearchAndSpeechRunWithTheCredentialTheirServiceDeclares) {
    ...
    EXPECT_EQ(searchAddress(plugin.effectiveSearchSettings()), QStringLiteral("https://api.search.brave.com"));
    EXPECT_EQ(plugin.effectiveSpeechSettings().apiKey, QStringLiteral("{env.ELEVENLABS_API_KEY}"));

    ASSERT_TRUE(test::awaitFuture(plugin.saveSearchSettings({SearchProvider::Tavily, {}, {}})).hasValue());
    ...
    ASSERT_TRUE(test::awaitFuture(plugin.saveSearchSettings({SearchProvider::Brave, {}, QStringLiteral("literal-key")})).hasValue());
...
TEST(AiToolRegistryTest, PublishesServerToolsUnderAQualifiedNameAndRoutesTheCallToTheirServer) {
    ...
    EXPECT_TRUE(validateToolSchema(published).hasValue());
...
TEST(AiTaskRepositoryTest, StoresEveryDeclaredMcpServerAndRejectsAnIncompleteOne) {
    ...
    McpServerDescriptor issues;
    issues.id = QStringLiteral("issues");
    issues.transport = McpTransport::Http;
    ...
    ASSERT_TRUE(test::awaitFuture(repository.saveSettings(settings)).hasValue());
    ...
    EXPECT_EQ(stored.mcpServers.at(0).transport, McpTransport::Stdio);
    ...
    EXPECT_EQ(stored.mcpServers.at(1).transport, McpTransport::Http);
    ...
    McpServerDescriptor withoutCommand;
    withoutCommand.id = QStringLiteral("broken");
    AiSettings invalid;
    invalid.mcpServers = {withoutCommand};
    ASSERT_TRUE(test::awaitFuture(repository.saveSettings(invalid)).hasValue());
...
TEST(AiPluginTest, PlacesTheInstructionsWhereTheModelAcceptsThem) {
    ...
    withoutSystemRole.parameters = defaultParameters(*findProvider(QStringLiteral("gemini")), withoutSystemRole.modelId);
    AiAgent agent = AiTestsHelper::testAgent();
    agent.connectionKey = connectionKey(withoutSystemRole);
...
TEST(AiTaskRepositoryTest, RoundTripsAConversationAndRejectsAStoredMessageNobodyCouldRead) {
    ...
    ASSERT_TRUE(test::awaitFuture(repository.appendConversation({user, assistant, tool})).hasValue());
    ...
    EXPECT_EQ(test::awaitFuture(repository.conversation(QStringLiteral("task-1"), 0, 0)).error().code, QStringLiteral("ai_conversation_invalid"));
    EXPECT_EQ(test::awaitFuture(repository.conversation(QStringLiteral("task-1"), 0, 101)).error().code, QStringLiteral("ai_conversation_invalid"));
...
TEST(AiPluginTest, TellsTheAgentWhatThisRunCanActuallyDo) {
    ...
    EXPECT_TRUE(text.contains(connectionKey(AiTestsHelper::testConnection()))) << text.toStdString();
...
TEST(AiPluginTest, TakesTheAgentsAlongWhenTheConnectionTheyRunOnIsEdited) {
    ...
    AiAgent agent = AiTestsHelper::testAgent();
    agent.connectionKey = connectionKey(stored);
    AiTestsHelper::installAiRows(host, {workspace}, {task}, {}, {stored}, {agent});

    AiPlugin plugin;
    ASSERT_TRUE(plugin.initialize(host).hasValue());
    ASSERT_EQ(plugin.connections().size(), 1);
    EXPECT_EQ(plugin.defaultConnectionKey(), connectionKey(stored));
    ...
    edited.parameters = defaultParameters(*findProvider(edited.providerId), edited.modelId);
    ASSERT_TRUE(test::awaitFuture(plugin.replaceConnection(connectionKey(stored), edited)).hasValue());
    ASSERT_EQ(plugin.connections().size(), 1);
    EXPECT_EQ(connectionKey(plugin.connections().first()), connectionKey(edited));
    EXPECT_EQ(plugin.defaultConnectionKey(), connectionKey(edited));
    ASSERT_EQ(plugin.agents().size(), 1);
    EXPECT_EQ(plugin.agents().first().connectionKey, connectionKey(edited));

    // The connection the agent runs on is still refused when it is removed, and the sentence says what to do about it.
    const auto removed = test::awaitFuture(plugin.saveConnections({}, {}));
    ...
    EXPECT_EQ(test::awaitFuture(plugin.replaceConnection(QStringLiteral("openai/retired"), edited)).error().code, QStringLiteral("ai_connection_unknown"));
    plugin.shutdown();
}
...
TEST(AiAgentPromptTest, ReplacesEveryDeclaredTagAndRefusesTheOnesNobodyDeclares) {
    ...
    EXPECT_FALSE(promptTags().isEmpty());
    EXPECT_TRUE(unknownPromptTags(QStringLiteral("You are {{AGENT_NAME}} at {{DATE_TIME}}")).isEmpty());
    EXPECT_EQ(unknownPromptTags(QStringLiteral("{{AGENT_NAME}} and {{NOT_A_TAG}} and {{ALSO_MISSING}}")), QStringList({QStringLiteral("NOT_A_TAG"), QStringLiteral("ALSO_MISSING")}));
    ...
    EXPECT_EQ(renderPrompt(QStringLiteral("You are {{AGENT_NAME}} on {{TASK_TITLE}}"), values), QStringLiteral("You are Reviewer on Ship it"));

    // A declared tag the run has nothing for becomes empty rather than staying on the wire as a marker.
    EXPECT_EQ(renderPrompt(QStringLiteral("[{{TASK_ISSUE_URL}}]"), values), QStringLiteral("[]"));
}

TEST(AiAgentTest, ValidatesEveryFieldAndTheSetItBelongsTo) {
    const AiAgent valid = AiTestsHelper::testAgent();
    ASSERT_TRUE(validateAgent(valid).hasValue());

    AiAgent wrongIdentifier = valid;
    wrongIdentifier.id = QStringLiteral("Reviewer Two");
    EXPECT_EQ(validateAgent(wrongIdentifier).error().code, QStringLiteral("ai_agent_invalid"));

    AiAgent withoutName = valid;
    withoutName.name = QStringLiteral("   ");
    EXPECT_EQ(validateAgent(withoutName).error().code, QStringLiteral("ai_agent_invalid"));

    AiAgent withoutPrompt = valid;
    withoutPrompt.systemPrompt.clear();
    EXPECT_EQ(validateAgent(withoutPrompt).error().code, QStringLiteral("ai_agent_invalid"));

    AiAgent beyondIterations = valid;
    beyondIterations.maximumIterations = aiLimits().maximumAgentIterations + 1;
    EXPECT_EQ(validateAgent(beyondIterations).error().code, QStringLiteral("ai_agent_invalid"));
    ...
    EXPECT_EQ(validateAgent(unknownTag).error().code, QStringLiteral("ai_agent_tag_unknown"));

    const QVector<ModelConnection> connections{AiTestsHelper::testConnection()};
    ASSERT_TRUE(validateAgentSet({valid}, connections).hasValue());
    EXPECT_EQ(validateAgentSet({valid, valid}, connections).error().code, QStringLiteral("ai_agent_duplicate"));

    AiAgent orphan = valid;
    orphan.connectionKey = QStringLiteral("openai/retired-model");
    EXPECT_EQ(validateAgentSet({orphan}, connections).error().code, QStringLiteral("ai_connection_unknown"));
}
...
TEST(AiRequestGateTest, PacesEveryRequestOfOneProviderThroughOneQueue) {
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return admitted.size() == 2; }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return admitted.size() == 1; }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return admitted.size() == 3; }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return gate.waiting(QStringLiteral("cerebras")) == 0; }));
    // clang-format on
...
TEST(AiTaskRepositoryTest, RoundTripsTheProviderRateLimitsAndRejectsTheOnesNobodyCouldHonour) {
    ...
    beyond[QStringLiteral("minimumIntervalMs")] = aiLimits().maximumRequestDelayMs + 1;
...
TEST(AiMcpClientTest, RejectsAnInvalidServerAndCompletesPendingRequests) {
    McpServerDescriptor absent;
    absent.id = QStringLiteral("absent");
    absent.command = QStringLiteral("slotdeck-nonexistent-mcp-server");
    AiMcpClient missing(absent);
    QVector<utils::Error> failures;
    // clang-format off
    QObject::connect(&missing, &AiMcpClient::failed, &missing, [&failures](const utils::Error& error) { failures.append(error); });
    // clang-format on
    missing.start();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return !failures.isEmpty(); }));
    ...
    McpServerDescriptor blank;
    blank.id = QStringLiteral("empty");
    blank.command = QStringLiteral("   ");
    AiMcpClient empty(blank);
    QVector<utils::Error> emptyFailures;
    // clang-format off
    QObject::connect(&empty, &AiMcpClient::failed, &empty, [&emptyFailures](const utils::Error& error) { emptyFailures.append(error); });
    // clang-format on
    empty.start();
    ...
    QVector<utils::Result<QJsonObject>> replies;
    // clang-format off
    empty.callTool(QStringLiteral("anything"), {}, [&replies](utils::Result<QJsonObject> result) { replies.append(std::move(result)); });
    // clang-format on
...
TEST(AiMcpClientTest, SpeaksTheStreamableHttpTransportWithSessionAndEventStream) {
    ...
    QVector<utils::Result<QJsonObject>> replies;
    // clang-format off
    client.callTool(QStringLiteral("remote_search"), QJsonObject{{QStringLiteral("q"), QStringLiteral("slotdeck")}}, [&replies](utils::Result<QJsonObject> result) { replies.append(std::move(result)); });
    ASSERT_TRUE(test::waitUntil([&]() { return !replies.isEmpty(); }));
    // clang-format on
    ASSERT_TRUE(replies.first().hasValue());
    ...
    client.stop();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return server.deleted(); }));
    // clang-format on
}
...
TEST(AiMcpClientTest, RejectsAnInvalidHttpAddress) {
    ...
    invalid.transport = McpTransport::Http;
    invalid.url = QStringLiteral("ftp://example.com/mcp");
    AiMcpClient client(invalid);
    QVector<utils::Error> failures;
    // clang-format off
    QObject::connect(&client, &AiMcpClient::failed, &client, [&failures](const utils::Error& error) { failures.append(error); });
    // clang-format on
...
TEST(AiMcpClientTest, AnswersRootsSamplingProgressAndCancelsAbandonedRequests) {
    QTemporaryDir root;
    ASSERT_TRUE(root.isValid());
    ...
    descriptor.arguments = {QStringLiteral("--slotdeck-test-mcp")};
    descriptor.roots = {QDir(root.path()).canonicalPath()};
    descriptor.samplingEnabled = true;
    descriptor.samplingMaximumTokens = 64;

    AiMcpClient client(descriptor);
    int samplingCalls = 0;
    int samplingBudget = 0;
    // clang-format off
    client.setSamplingHandler([&samplingCalls, &samplingBudget](const QJsonObject&, int maximumTokens, McpReply reply) {
        ++samplingCalls;
        samplingBudget = maximumTokens;
        reply(utils::Result<QJsonObject>::success({{QStringLiteral("role"), QStringLiteral("assistant")}, {QStringLiteral("content"), QJsonObject{{QStringLiteral("type"), QStringLiteral("text")}, {QStringLiteral("text"), QStringLiteral("sampled")}}}}));
    });
    // clang-format on

    QSignalSpy initialized(&client, &AiMcpClient::initialized);
    QSignalSpy progress(&client, &AiMcpClient::progressReported);
    client.start();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return initialized.count() == 1; }));
    // clang-format on

    QVector<utils::Result<QJsonObject>> replies;
    // clang-format off
    client.request(QStringLiteral("probe/client"), {}, [&replies](utils::Result<QJsonObject> result) { replies.append(std::move(result)); });
    ASSERT_TRUE(test::waitUntil([&]() { return samplingCalls == 1 && progress.count() == 1; }));
    // clang-format on
...
TEST(AiMcpClientTest, DeclaresSamplingOnlyWhenTheServerIsAllowedToSpendTheModel) {
    McpServerDescriptor descriptor;
    descriptor.id = QStringLiteral("fixture");
    descriptor.command = QCoreApplication::applicationFilePath();
    descriptor.arguments = {QStringLiteral("--slotdeck-test-mcp")};
    ...
    AiMcpClient disallowed(descriptor);
    int calls = 0;
    // clang-format off
    disallowed.setSamplingHandler([&calls](const QJsonObject&, int, McpReply reply) { ++calls; reply(utils::Result<QJsonObject>::success({})); });
    // clang-format on
    QSignalSpy initialized(&disallowed, &AiMcpClient::initialized);
    disallowed.start();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return initialized.count() == 1; }));
    // clang-format on
    ...
    QVector<utils::Result<QJsonObject>> replies;
    // clang-format off
    disallowed.request(QStringLiteral("probe/client"), {}, [&replies](utils::Result<QJsonObject> result) { replies.append(std::move(result)); });
    ASSERT_TRUE(test::waitUntil([&]() { return !replies.isEmpty(); }));
    // clang-format on
    EXPECT_EQ(calls, 0);
    disallowed.stop();
...
TEST(AiToolRegistryTest, DiscoversSkillsInThePublishedLayoutAndDisclosesThemProgressively) {
    ...
    QVector<SkillDescriptor> catalog;
    // clang-format off
    registry.discoverSkills(sandbox, [&catalog](const QVector<SkillDescriptor>& found) { catalog = found; });
    ASSERT_TRUE(test::waitUntil([&]() { return !catalog.isEmpty(); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return results.size() == 7; }));
    // clang-format on
    EXPECT_TRUE(results.at(6).failed);
}
...
TEST(AiToolContractTest, FitsTheConversationToTheModelWindowWithoutBreakingToolTurns) {
    ...
    const FittedConversation fitted = fitConversation(conversation, 3000);
    EXPECT_GT(fitted.dropped.size(), 0);
    EXPECT_LE(estimateTokens(fitted.messages), 3000);
...
TEST(AiToolRegistryTest, TellsTheAgentWhichTaskItIsWorkingOn) {
    ...
    task.issueUrl = QStringLiteral("https://github.com/paulo/slotdeck/issues/42");
...
TEST(AiTaskRepositoryTest, ReadsBackEveryTaskItAcceptedForBothExecutionKinds) {
    ...
    ASSERT_TRUE(test::awaitFuture(repository.saveTask(command)).hasValue());
    ASSERT_TRUE(test::awaitFuture(repository.saveTask(agent)).hasValue());
}
