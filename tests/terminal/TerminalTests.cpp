...
class FakePtyBackend final : public terminalcore::IPtyBackend {
  public:
    utils::Result<void> start(const terminalcore::ShellProfile& profile, const QString& workingDirectory, const QString& historyFile, int columns, int rows) override {
        ++startCalls;
    ...
    utils::Result<void> resize(int columns, int rows, int cellWidth, int cellHeight) override {
        ++resizeCalls;
    ...
    std::optional<utils::Error> startError;
    std::optional<utils::Error> writeError;
    std::optional<utils::Error> resizeError;
...
TEST(TerminalShortcutsTest, SeparatesPluginApplicationAndTerminalOwnedInput) {
    const QKeyCombination newTerminalCombination = terminalcore::shortcuts::newTerminal()[0];
    ...
    QKeyEvent terminalControlEvent(QEvent::KeyPress, Qt::Key_A, terminalcore::shortcuts::terminalControlModifier);
    EXPECT_FALSE(terminalcore::shortcuts::isReservedForApplication(terminalControlEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(terminalControlEvent));
...
#endif
    EXPECT_TRUE(terminalcore::shortcuts::isPaste(pasteEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(pasteEvent));
    ...
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(interruptEvent));
    EXPECT_FALSE(terminalcore::shortcuts::isTerminalOwned(copyEvent));
    QKeyEvent closeEvent(QEvent::KeyPress, Qt::Key_W, Qt::ControlModifier);
    EXPECT_TRUE(terminalcore::shortcuts::isReservedForApplication(closeEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isCloseTerminal(closeEvent));

    // The shell interrupts with the physical control key, so only the application combination copies.
    EXPECT_TRUE(terminalcore::shortcuts::isCopy(copyEvent));
    EXPECT_FALSE(terminalcore::shortcuts::isCopy(interruptEvent));
    ...
    EXPECT_TRUE(terminalcore::shortcuts::isSelectAll(selectAllEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isClearBuffer(clearEvent));
    EXPECT_FALSE(terminalcore::shortcuts::isSelectAll(beginningOfLineEvent));
    EXPECT_FALSE(terminalcore::shortcuts::isClearBuffer(killLineEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(beginningOfLineEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(killLineEvent));
    QKeyEvent deleteWordEvent(QEvent::KeyPress, Qt::Key_W, Qt::MetaModifier);
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(deleteWordEvent));
#else
    ...
    EXPECT_TRUE(terminalcore::shortcuts::isCopy(shiftedCopyEvent));
    ...
    EXPECT_TRUE(terminalcore::shortcuts::isSelectAll(shiftedSelectAllEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isClearBuffer(shiftedClearEvent));
    EXPECT_FALSE(terminalcore::shortcuts::isSelectAll(beginningOfLineEvent));
    EXPECT_FALSE(terminalcore::shortcuts::isClearBuffer(killLineEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(beginningOfLineEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isTerminalOwned(killLineEvent));
    QKeyEvent shiftedCloseEvent(QEvent::KeyPress, Qt::Key_W, Qt::ControlModifier | Qt::ShiftModifier);
    EXPECT_TRUE(terminalcore::shortcuts::isCloseTerminal(shiftedCloseEvent));
    EXPECT_TRUE(terminalcore::shortcuts::isReservedForApplication(shiftedCloseEvent));
    ...
    EXPECT_TRUE(terminalcore::shortcuts::isReservedForApplication(plainIncrease));
    EXPECT_TRUE(terminalcore::shortcuts::isReservedForApplication(plainDecrease));
}
...
TEST(LayoutManagerTest, AssignsSwapsShelvesRemovesAndValidatesSlots) {
    ...
    EXPECT_TRUE(workspace::LayoutManager::contains(layout, QStringLiteral("a")));
    EXPECT_TRUE(workspace::LayoutManager::contains(layout, QStringLiteral("c")));
    EXPECT_FALSE(workspace::LayoutManager::contains(layout, QStringLiteral("missing")));
    ...
    workspace::LayoutManager::moveToShelf(layout, QStringLiteral("a"), 1);
    EXPECT_FALSE(layout.slotAssignments.at(1).has_value());
    EXPECT_EQ(layout.shelf.at(1), QStringLiteral("a"));
    workspace::LayoutManager::moveToShelf(layout, QStringLiteral("c"));
...
TEST(TerminalSettingsTest, StepsItsOwnFontSizeWithinTheSupportedRange) {
    ...
    settings.stepFontSize(plugins::ContentFontStep::Increase);
    EXPECT_EQ(settings.fontSize(), plugins::terminalplugin::defaultTerminalFontSize + 1);
    settings.stepFontSize(plugins::ContentFontStep::Decrease);
    EXPECT_EQ(settings.fontSize(), plugins::terminalplugin::defaultTerminalFontSize);
    ...
    settings.stepFontSize(plugins::ContentFontStep::Reset);
    EXPECT_EQ(settings.fontSize(), plugins::terminalplugin::defaultTerminalFontSize);
...
TEST(TerminalSettingsTest, TakesTheDeclaredDefaultForEveryStoredValueItCannotUse) {
    ...
    // clang-format off
    writeHost.settingsFutureHandler = [](const QJsonObject&) { return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"write_failed", "Write failed", {}})); };
    // clang-format on
...
#ifdef Q_OS_WIN
    EXPECT_EQ(terminalcore::formatLocalPathsForShell(profile, {QStringLiteral("C:/one path"), QStringLiteral("C:/it's")}), QStringLiteral("'C:/one path' 'C:/it''s' "));
#else
    EXPECT_EQ(terminalcore::formatLocalPathsForShell(profile, {QStringLiteral("/one path"), QStringLiteral("/it's")}), QStringLiteral("'/one path' '/it'\\''s' "));
#endif
    EXPECT_EQ(terminalcore::formatLocalPathsForShell(profile, {}), QStringLiteral(" "));
...
TEST(GhosttyTerminalAdapterTest, ReportsPreInitializationErrorsAndRendersTerminalData) {
    ...
    QKeyEvent interrupt(QEvent::KeyPress, Qt::Key_C, terminalcore::shortcuts::terminalControlModifier, QStringLiteral("c"));
    ...
    QKeyEvent typed(QEvent::KeyPress, Qt::Key_A, terminalcore::shortcuts::terminalControlModifier, QStringLiteral("a"));
...
TEST(GhosttyTerminalAdapterTest, AnswersTheQueriesAProgramSendsAndReportsTheBell) {
    ...
    ASSERT_EQ(responses.count(), 1);
    EXPECT_TRUE(responses.takeFirst().first().toByteArray().contains(QByteArrayLiteral("SlotDeck")));
...
TEST(WorkspaceManagerTest, ManagesTabsSessionsLayoutsAndInvalidOperations) {
    ...
    EXPECT_EQ(manager.data(manager.index(0), workspace::WorkspaceManager::NameRole).toString(), QStringLiteral("terminal.workspace.numbered"));
    ...
    EXPECT_EQ(manager.data(manager.index(0), workspace::WorkspaceManager::NameRole).toString(), QStringLiteral("Main"));
    manager.createTab();
    ...
    QSignalSpy notifications(&manager, &workspace::WorkspaceManager::notificationRequested);
    ...
    QSignalSpy closing(&manager, &workspace::WorkspaceManager::terminalClosing);
    ...
    manager.closeTab(0);
    EXPECT_EQ(manager.rowCount(), 1);
    manager.shutdown(workspace::WorkspaceManager::ShutdownMode::Discard);
    manager.shutdown();
}
...
TEST(TerminalUiTest, ConnectsWorkspacePaneShelfAndToolbarInteractions) {
    ...
    ui::TerminalPane pane(*session, host);
    EXPECT_EQ(pane.sessionId(), sessionId);
    EXPECT_EQ(pane.draggedSessionId(), sessionId);
    pane.setDropDestination({ui::SessionDropTarget::Shelf, -1});
    ...
    pane.raise();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&pane]() { return pane.isVisible(); }));
    // clang-format on
    ...
    QSignalSpy selected(&pane, &ui::TerminalPane::selected);
    pane.focusTerminal();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&selected]() { return selected.count() >= 1; }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { terminalWidget->update(); return !terminalWidget->grab().isNull(); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return terminalWidget->hasSelection(); }));
    ASSERT_TRUE(test::waitUntil([&]() { return terminalWidget->selectedText().contains(QStringLiteral("hello")); }));
    // clang-format on
    ...
    for (int y = 2; y < terminalWidget->height() && links.isEmpty(); y += 4) {
        for (int x = 2; x < terminalWidget->width() && links.isEmpty(); x += 4) {
            QTest::mousePress(terminalWidget, Qt::LeftButton, terminalcore::shortcuts::applicationModifier, QPoint(x, y));
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { terminalWidget->selectAll(); return terminalWidget->selectedText().contains(QStringLiteral("alpha beta alpha")); }));
    // clang-format on
    terminalWidget->clearSelection();
    terminalWidget->setFocus(Qt::OtherFocusReason);
    QTest::keyClick(terminalWidget, Qt::Key_F, terminalcore::shortcuts::applicationModifier);
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return terminalWidget->selectedText() == QStringLiteral("alpha"); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return findStatus->text() == QStringLiteral("terminal.find.not-found"); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return terminalWidget->selectedText().contains(QStringLiteral("hello world")); }));
    // clang-format on
    terminalMenu->actions().at(4)->trigger();
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return !terminalWidget->hasSelection(); }));
    ASSERT_TRUE(test::waitUntil([&]() { terminalWidget->update(); return !terminalWidget->selectedText().contains(QStringLiteral("hello")); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return session->programWantsMouse(); }));
    // clang-format on
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return session->programWantsFocus(); }));
    // clang-format on
    backends.first()->writes.clear();
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return bellIndicator->isVisible(); }));
    // clang-format on
    QTest::mousePress(terminalWidget, Qt::LeftButton, Qt::NoModifier, QPoint(4, 4));
    ...
    menu->close();
    ASSERT_EQ(host.requests.size(), 1);
    ...
    EXPECT_EQ(host.requests.at(0).payload.value(QStringLiteral("path")).toString(), session->cwd());
    ...
    EXPECT_EQ(host.requests.at(1).payload.value(QStringLiteral("path")).toString(), session->cwd());
    ...
    ui::ShelfSessionChip chip(sessionId, QStringLiteral("Terminal"), host);
    EXPECT_EQ(chip.draggedSessionId(), sessionId);
    chip.setDropDestination({ui::SessionDropTarget::Slot, 0});
    chip.show();
    QSignalSpy activated(&chip, &ui::ShelfSessionChip::activated);
    ...
    EXPECT_EQ(newTerminalAction->shortcut(), terminalcore::shortcuts::newTerminal());
    EXPECT_EQ(newTabAction->shortcut(), terminalcore::shortcuts::newTab());
    EXPECT_EQ(layoutAction->shortcut(), terminalcore::shortcuts::layout());
    ...
    const QKeyCombination hiddenShortcut = terminalcore::shortcuts::newTerminal()[0];
...
TEST(TerminalPluginTest, DeclaresCompleteMetadataAndRejectsUnavailableOperations) {
    ...
    EXPECT_FALSE(plugin.navigationItems(ui::themeManager().theme()).first().icon.isNull());
    ...
    // clang-format off
    plugin.handleRequest(QStringLiteral("sample"), QStringLiteral("unknown"), {}, [&response](utils::Result<QJsonObject> result) { response = std::move(result); });
    // clang-format on
...
TEST(TerminalWorkspaceRepositoryTest, RejectsStorageErrorsMissingStateAndInvalidWrites) {
    ...
    EXPECT_EQ(test::awaitFuture(repository.save(invalid)).error().code, QStringLiteral("terminal_workspace_invalid"));
    ...
    EXPECT_EQ(test::awaitFuture(repository.save(TerminalTestsHelper::validWorkspace())).error().code, QStringLiteral("write_failed"));
}
...
TEST(TerminalSessionTest, ControlsBackendLifecycleDataAndFailures) {
    ...
    // clang-format off
    ASSERT_TRUE(test::waitUntil([&]() { return session->snapshot().cells.size() > 0; }));
    // clang-format on
...
TEST(TerminalSessionTest, PropagatesStartWriteResizeAndRestartErrors) {
    auto backend = std::make_unique<FakePtyBackend>();
    auto* backendPointer = backend.get();
    backendPointer->startError = utils::Error{QStringLiteral("start_failed"), QStringLiteral("Start failed"), {}};
    auto session = TerminalTestsHelper::createSession(std::move(backend));
    ...
    ASSERT_TRUE(session->start().hasValue());
    backendPointer->writeError = utils::Error{QStringLiteral("write_failed"), QStringLiteral("Write failed"), {}};
...
QJsonObject TerminalTestsHelper::serializedWorkspace() {
    test::TestPluginHost host;
    plugins::terminalplugin::TerminalWorkspaceRepository repository(host);
    EXPECT_TRUE(test::awaitFuture(repository.save(TerminalTestsHelper::validWorkspace())).hasValue());
