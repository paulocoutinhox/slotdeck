...
TerminalPane::TerminalPane(terminalcore::TerminalSession& session, plugins::PluginHost& host, QWidget* parent) : QFrame(parent), m_session(&session), m_host(host), m_sessionId(session.id()) {
    ...
    m_header->setFixedHeight(m_host.theme().metric(ThemeMetric::TerminalHeaderHeight));
    ...
    m_focusIndicator = new StatusIndicator(m_header);
    m_focusIndicator->setObjectName(QStringLiteral("terminalFocusIndicator"));
    m_focusIndicator->setColor(m_host.theme().color(ThemeColor::TextMuted));
    ...
    namePalette.setColor(QPalette::WindowText, m_host.theme().color(ThemeColor::TextMuted));
    ...
    m_focusButton = toolButton(IconName::Focus, m_host.theme(), m_host.translate(QStringLiteral("terminal.actions.focus")), m_header);
    m_actionsButton = toolButton(IconName::More, m_host.theme(), m_host.translate(QStringLiteral("terminal.actions.menu")), m_header);
    auto* closeButton = toolButton(IconName::Close, m_host.theme(), m_host.translate(QStringLiteral("terminal.actions.close-terminal")), m_header);
    ...
    m_bellIndicator->setPixmap(icon(IconName::Bell, m_host.theme().color(ThemeColor::Warning)).pixmap(m_host.theme().metric(ThemeMetric::SmallIconSize)));
    ...
    m_terminal = new TerminalWidget(m_host, this);
    ...
    restartButton->setIcon(icon(IconName::Refresh, m_host.theme()));
    ...
    connect(m_terminal, &TerminalWidget::focused, this, &TerminalPane::selectSession);
    connect(m_terminal, &TerminalWidget::interactionError, this, &TerminalPane::interactionError);
    connect(m_terminal, &TerminalWidget::linkActivated, this, &TerminalPane::openAddress);
    // clang-format off
    connect(m_session, &terminalcore::TerminalSession::bellRang, this, [this]() { m_bellIndicator->setVisible(!m_terminal->hasFocus()); });
    connect(m_terminal, &TerminalWidget::focused, this, [this]() { m_bellIndicator->hide(); });
...
void TerminalPane::setSelected(bool isSelected) {
    ...
    QPalette namePalette = m_nameLabel->palette();
    namePalette.setColor(QPalette::WindowText, m_host.theme().color(m_selected ? ThemeColor::Text : ThemeColor::TextMuted));
    m_nameLabel->setPalette(namePalette);
...
void TerminalPane::setFocusMode(bool focusMode) {
    m_focusButton->setIcon(icon(focusMode ? IconName::Restore : IconName::Focus, m_host.theme()));
...
void TerminalPane::showActionsMenu() {
    ...
    auto* restartAction = menu->addAction(icon(IconName::Refresh, m_host.theme()), m_host.translate(QStringLiteral("terminal.actions.restart-shell")));
...
void TerminalPane::openAddress(const QString& address) {
    ...
    if (!m_host.pluginAvailable(QStringLiteral("browser"))) {
        QDesktopServices::openUrl(url);
        return;
    }

    // clang-format off
    const auto answered = [this](utils::Result<QJsonObject> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("terminal.plugin.title")), result.error().message, plugins::AlertSeverity::Error); } };
    // clang-format on
    m_host.request(QStringLiteral("browser"), QStringLiteral("browser.open"), {{QStringLiteral("url"), url.toString()}}, *this, answered);
}
...
void TerminalPane::offerDirectory(const QString& pluginId, const QString& topic) {
    ...
    const auto answered = [this](utils::Result<QJsonObject> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("terminal.plugin.title")), result.error().message, plugins::AlertSeverity::Error); } };
    // clang-format on
    m_host.request(pluginId, topic, {{QStringLiteral("path"), m_session->cwd()}}, *this, answered);
}
