...
class ShelfFrame final : public QFrame {
  public:
    explicit ShelfFrame(workspace::WorkspaceManager& manager, QWidget* parent = nullptr) : QFrame(parent), m_manager(manager) {
        setAcceptDrops(true);
    }
...
WorkspaceView::WorkspaceView(workspace::WorkspaceManager& manager, plugins::terminalplugin::TerminalSettingsStore& settings, plugins::PluginHost& host, QWidget* parent) : QWidget(parent), m_manager(manager), m_settings(settings), m_host(host) {
    ...
    shelfIcon->setPixmap(icon(IconName::Shelf, m_host.theme()).pixmap(16, 16));
    ...
    connect(&m_manager, &workspace::WorkspaceManager::layoutChanged, this, &WorkspaceView::synchronize);
    connect(&m_manager, &workspace::WorkspaceManager::currentTabChanged, this, &WorkspaceView::synchronize);
    connect(&m_manager, &workspace::WorkspaceManager::focusedSessionChanged, this, &WorkspaceView::updateSelection);
    // clang-format off
...
QWidget* WorkspaceView::createEmptySlot(int slotIndex) {
    ...
    layout->setSpacing(5);
    auto* button = new QPushButton(icon(IconName::Add, m_host.theme()), m_host.translate(QStringLiteral("terminal.actions.new-terminal")), empty);
