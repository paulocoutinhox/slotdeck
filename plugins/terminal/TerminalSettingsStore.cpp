...
void TerminalSettingsStore::setFontFamily(const QString& family) {
    if (family == m_settings.fontFamily || !ui::monospacedFontFamilies().contains(family)) {
        return;
    }
...
void TerminalSettingsStore::setFontSize(int pointSize) {
    if (pointSize == m_settings.fontSize || !ui::validContentFontSize(pointSize)) {
        return;
    }
...
void TerminalSettingsStore::stepFontSize(ContentFontStep step) {
...
void TerminalSettingsStore::setThemeId(const QString& themeId) {
    if (themeId == m_settings.themeId || !terminalcore::terminalThemeExists(themeId)) {
        return;
    }
...
void TerminalSettingsStore::apply(TerminalSettings next) {
    ...
    // clang-format off
    future.then(this, [this, next, revision](utils::Result<void> result) {
        if (result.hasValue()) {
