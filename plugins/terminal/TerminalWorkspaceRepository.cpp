...
class TerminalWorkspaceRepositoryHelper final {
  public:
    ...
    static utils::Result<domain::Workspace> failure(const QString& detail);
    ...
    static utils::Result<domain::Workspace> parseWorkspace(const QJsonObject& object);
...
utils::Result<domain::Workspace> TerminalWorkspaceRepositoryHelper::failure(const QString& detail) {
    return utils::Result<domain::Workspace>::failure({"terminal_workspace_invalid", "The terminal workspace is invalid", detail});
}
...
bool TerminalWorkspaceRepositoryHelper::readInteger64(const QJsonObject& object, const QString& key, qint64& output) {
    return readJsonInteger(object.value(key), output);
}
...
utils::Result<domain::Workspace> TerminalWorkspaceRepositoryHelper::parseWorkspace(const QJsonObject& object) {
    if (!hasExactKeys(object, {QStringLiteral("id"), QStringLiteral("name"), QStringLiteral("createdAt"), QStringLiteral("updatedAt"), QStringLiteral("lastOpenedAt"), QStringLiteral("selectedMainTabId"), QStringLiteral("tabs"), QStringLiteral("sessions")})) {
...
utils::Result<void> TerminalWorkspaceRepository::saveInitial(const domain::Workspace& workspace) {
...
QFuture<utils::Result<void>> TerminalWorkspaceRepository::save(const domain::Workspace& workspace) {
    if (!TerminalWorkspaceRepositoryHelper::validateWorkspace(workspace)) {
