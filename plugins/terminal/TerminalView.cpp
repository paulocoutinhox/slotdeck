...
TerminalView::TerminalView(workspace::WorkspaceManager& manager, TerminalSettingsStore& settings, PluginHost& host, QWidget* parent) : QWidget(parent), m_manager(manager), m_settings(settings), m_host(host) {
    createActions();
    createInterface();
    ...
    connect(&m_manager, &workspace::WorkspaceManager::currentTabChanged, this, &TerminalView::synchronizeCurrentTab);
    connect(&m_manager, &workspace::WorkspaceManager::currentTabChanged, this, &TerminalView::updateLayoutSelection);
    connect(&m_manager, &workspace::WorkspaceManager::currentTabChanged, this, &TerminalView::updateStatus);
    connect(&m_manager, &workspace::WorkspaceManager::sessionsChanged, this, &TerminalView::updateStatus);
...
void TerminalView::createActions() {
    m_newTerminalAction = new QAction(ui::icon(ui::IconName::Terminal, m_host.theme()), m_host.translate(QStringLiteral("terminal.actions.new-terminal")), this);
    m_newTerminalAction->setObjectName(QStringLiteral("terminalNewTerminalAction"));
    m_newTerminalAction->setShortcut(terminalcore::shortcuts::newTerminal());
    m_newTerminalAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    m_newTabAction = new QAction(ui::icon(ui::IconName::Add, m_host.theme()), m_host.translate(QStringLiteral("terminal.actions.new-tab")), this);
    m_newTabAction->setObjectName(QStringLiteral("terminalNewTabAction"));
    m_newTabAction->setShortcut(terminalcore::shortcuts::newTab());
    m_newTabAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    m_layoutAction = new QAction(ui::icon(ui::IconName::Layout, m_host.theme()), m_host.translate(QStringLiteral("terminal.actions.layout")), this);
    m_layoutAction->setObjectName(QStringLiteral("terminalLayoutAction"));
    m_layoutAction->setShortcut(terminalcore::shortcuts::layout());
    m_layoutAction->setShortcutContext(Qt::WidgetWithChildrenShortcut);
    m_closeTerminalAction = new QAction(ui::icon(ui::IconName::Close, m_host.theme()), m_host.translate(QStringLiteral("terminal.actions.close-terminal")), this);
    m_closeTerminalAction->setObjectName(QStringLiteral("terminalCloseTerminalAction"));
    m_closeTerminalAction->setShortcut(terminalcore::shortcuts::closeTerminal());
...
void TerminalView::createInterface() {
    ...
    addTabButton->setIcon(ui::icon(ui::IconName::Add, m_host.theme()));
    ...
    root->addWidget(ui::horizontalDivider(this));
    m_workspaceView = new ui::WorkspaceView(m_manager, m_settings, m_host, this);
    ...
    for (const auto& preset : m_manager.layoutPresets()) {
        ...
        button->setIcon(ui::layoutIcon(values.value("id").toString(), values.value("columns").toInt(), values.value("rows").toInt(), slotCount, m_host.theme()));
    ...
    // clang-format off
    connect(m_workspaceView, &ui::WorkspaceView::interactionError, this, [this](const utils::Error& error) { m_host.notify(m_host.translate(QStringLiteral("terminal.error.interaction")), terminalFailureMessage(error, m_host), plugins::AlertSeverity::Error); });
    // clang-format on
}
