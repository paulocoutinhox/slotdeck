...
class ServerSplitterHandle final : public QSplitterHandle {
  public:
    ServerSplitterHandle(Qt::Orientation orientation, const Theme& theme, QSplitter* parent) : QSplitterHandle(orientation, parent), m_theme(theme) {
  ...
  protected:
    void paintEvent(QPaintEvent*) override {
        ...
        painter.fillRect(rect(), m_theme.color(ThemeColor::Window));
        painter.setPen(m_theme.color(underMouse() ? ThemeColor::Accent : ThemeColor::Border));
...
class ServerSplitter final : public QSplitter {
  public:
    ServerSplitter(const Theme& theme, QWidget* parent) : QSplitter(Qt::Vertical, parent), m_theme(theme) {}
...
class WebServerViewHelper final {
  public:
    ...
    static QToolButton* actionButton(IconName iconName, const Theme& theme, ThemeColor role, const QString& tooltip, const QString& serverId, QWidget* parent);
};
...
QToolButton* WebServerViewHelper::actionButton(IconName iconName, const Theme& theme, ThemeColor role, const QString& tooltip, const QString& serverId, QWidget* parent) {
    QToolButton* button = rowActionButton(iconName, role, theme, tooltip, parent);
  ...
  public:
    ...
    void append(const QVariantList& values) {
        ...
        for (const auto& value : values) {
            ...
            incoming.append({localTimestamp(timestamp), entry.value("status").toInt(), entry.value("method").toString(), entry.value("path").toString(), entry.value("durationMs").toLongLong(), entry.value("responseBytes").toLongLong(), entry.value("remoteAddress").toString()});
        }
...
WebServerDialog::WebServerDialog(QString serverId, QString terminalId, QString initialRoot, plugins::webserver::WebServerPlugin& plugin, QWidget* parent) : QDialog(parent), m_serverId(std::move(serverId)), m_terminalId(std::move(terminalId)), m_initialRoot(std::move(initialRoot)), m_plugin(plugin) {
    ...
    m_rootButton->setIcon(icon(IconName::Search, m_plugin.host().theme()));
    ...
    auto* portRow = ui::stepperRow(m_portSpin, m_plugin.host().theme(), form);
    ...
    m_statusIndicator = new StatusIndicator(status);
    ...
    m_startButton = new QPushButton(primaryIcon(IconName::Start, m_plugin.host().theme()), m_plugin.host().translate(QStringLiteral("web-server.dialog.start")), this);
    m_startButton->setObjectName(QStringLiteral("webServerStartButton"));
    m_startButton->setProperty("primary", true);
    m_openButton = new QPushButton(icon(IconName::WebServer, m_plugin.host().theme()), m_plugin.host().translate(QStringLiteral("web-server.dialog.open")), this);
    m_editButton = new QPushButton(icon(IconName::Edit, m_plugin.host().theme()), m_plugin.host().translate(QStringLiteral("web-server.dialog.edit")), this);
    m_stopButton = new QPushButton(icon(IconName::Stop, m_plugin.host().theme().color(ThemeColor::Danger)), m_plugin.host().translate(QStringLiteral("web-server.dialog.stop")), this);
...
void WebServerDialog::refreshState(const QString& changedServerId) {
    ...
    const QColor statusColor = running ? m_plugin.host().theme().color(ThemeColor::Success) : pending ? m_plugin.host().theme().color(ThemeColor::Warning) : m_plugin.host().theme().color(ThemeColor::TextMuted);
...
void WebServerDialog::showError(const utils::Error& error) {
...
WebServerView::WebServerView(plugins::webserver::WebServerPlugin& plugin, QWidget* parent) : QWidget(parent), m_plugin(plugin) {
    ...
    auto* header = new PageHeader(m_plugin.host().theme(), m_plugin.host().translate(QStringLiteral("web-server.manager.title")), this);
    auto* terminalButton = new QPushButton(icon(IconName::Terminal, m_plugin.host().theme()), m_plugin.host().translate(QStringLiteral("web-server.manager.from-terminal")), header);
    terminalButton->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Fixed);
    terminalButton->setVisible(m_plugin.host().pluginAvailable(QStringLiteral("terminal")));
    auto* createButton = new QPushButton(primaryIcon(IconName::Add, m_plugin.host().theme()), m_plugin.host().translate(QStringLiteral("web-server.manager.new-server")), header);
    ...
    m_emptyLabel = emptyStateLabel(m_plugin.host().translate(QStringLiteral("web-server.manager.empty")), serverPane);
    m_emptyLabel->setContentsMargins(18, 12, 18, 12);
    serverPaneLayout->addWidget(m_emptyLabel, 1);

    m_serverTable = dataGrid({m_plugin.host().translate(QStringLiteral("web-server.manager.status")), m_plugin.host().translate(QStringLiteral("web-server.manager.name")), m_plugin.host().translate(QStringLiteral("web-server.manager.address")), m_plugin.host().translate(QStringLiteral("web-server.manager.document-root")), {}}, serverPane);
    ...
    stretchGridColumn(m_serverTable, 3);
    QPalette serverTablePalette = m_serverTable->palette();
    serverTablePalette.setColor(QPalette::Highlight, m_plugin.host().theme().color(ThemeColor::Hover));
    serverTablePalette.setColor(QPalette::HighlightedText, m_plugin.host().theme().color(ThemeColor::Text));
    ...
    m_requestTitle = sectionTitleLabel(m_plugin.host().translate(QStringLiteral("web-server.manager.requests")), requestHeader);
    m_clearRequestsButton = new QPushButton(icon(IconName::Clear, m_plugin.host().theme()), m_plugin.host().translate(QStringLiteral("web-server.manager.clear-log")), requestHeader);
...
void WebServerView::refreshInstances(const QString& changedServerId) {
    ...
    for (int row = 0; row < static_cast<int>(servers.size()); ++row) {
        ...
        const auto statusColor = running ? ThemeColor::Success : pending ? ThemeColor::Warning : ThemeColor::TextMuted;
        QTableWidgetItem* statusItem = gridStatusItem(statusText, IconName::WebServer, statusColor, m_plugin.host().theme());
        ...
        if (pending) {
            auto* stop = WebServerViewHelper::actionButton(IconName::Stop, m_plugin.host().theme(), ThemeColor::Danger, m_plugin.host().translate(QStringLiteral("web-server.manager.stop-server")), serverId, actions);
            actionsLayout->addWidget(stop);
            connect(stop, &QToolButton::clicked, this, &WebServerView::stopServer);
        } else if (running) {
            auto* edit = WebServerViewHelper::actionButton(IconName::Edit, m_plugin.host().theme(), ThemeColor::TextMuted, m_plugin.host().translate(QStringLiteral("web-server.dialog.edit")), serverId, actions);
            auto* openExternally = WebServerViewHelper::actionButton(IconName::ExternalLink, m_plugin.host().theme(), ThemeColor::Success, m_plugin.host().translate(QStringLiteral("web-server.manager.open-browser")), serverId, actions);
            auto* openInBrowser = WebServerViewHelper::actionButton(IconName::Browser, m_plugin.host().theme(), ThemeColor::Success, m_plugin.host().translate(QStringLiteral("web-server.manager.open-browser-plugin")), serverId, actions);
            auto* stop = WebServerViewHelper::actionButton(IconName::Stop, m_plugin.host().theme(), ThemeColor::Danger, m_plugin.host().translate(QStringLiteral("web-server.manager.stop-server")), serverId, actions);
            ...
            auto* edit = WebServerViewHelper::actionButton(IconName::Edit, m_plugin.host().theme(), ThemeColor::TextMuted, m_plugin.host().translate(QStringLiteral("web-server.dialog.edit")), serverId, actions);
            auto* start = WebServerViewHelper::actionButton(IconName::Start, m_plugin.host().theme(), ThemeColor::Success, m_plugin.host().translate(QStringLiteral("web-server.manager.start-server")), serverId, actions);
            auto* remove = WebServerViewHelper::actionButton(IconName::Close, m_plugin.host().theme(), ThemeColor::Danger, m_plugin.host().translate(QStringLiteral("web-server.manager.remove-server")), serverId, actions);
            actionsLayout->addWidget(edit);
...
void WebServerView::openDialog(WebServerDialog* dialog) {
    ...
    showDialogWindow(dialog, m_plugin.host().translate(QStringLiteral("web-server.dialog.title")));
}
