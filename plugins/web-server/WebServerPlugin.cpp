...
class WebServerRuntime final {
  public:
    WebServerRuntime() : m_server(new WebServerInstance()) {
        ...
        m_workerThread.setObjectName(QStringLiteral("slotdeckWebServer"));
...
QVector<NavigationItem> WebServerPlugin::navigationItems(const ui::Theme& theme) const {
    return {{QStringLiteral("manager"), QStringLiteral("web-server.navigation.server"), ui::icon(ui::IconName::WebServer, theme), NavigationPlacement::Primary, NavigationOrder::Server}};
}
...
utils::Result<void> WebServerPlugin::initialize(PluginHost& host) {
    if (m_host != nullptr) {
...
QWidget* WebServerPlugin::createSettingsSection(const QString& groupId, const QString& sectionId, QWidget* parent) {
    ...
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("web-server.settings.splitter-label")), ui::stepperRow(ratio, m_host->theme(), page));
...
void WebServerPlugin::handleRequest(const QString&, const QString& topic, const QJsonObject& payload, PluginReply reply) {
    if (topic == QStringLiteral("web-server.instance.create") && hasExactKeys(payload, {QStringLiteral("path")}) && payload.value(QStringLiteral("path")).isString()) {
...
void WebServerPlugin::openWebServerForFolder(const QString& path, const PluginReply& reply) {
    ...
    reply(utils::Result<QJsonObject>::success({}));
...
QFuture<utils::Result<void>> WebServerPlugin::configureAndStartWebServer(const QString& serverId, const QString& name, const QString& root, const QString& bindHost, int port, const QString& terminalId) {
    ...
    auto validation = QtConcurrent::run([root]() {
        ...
        return !canonicalRoot.isEmpty() && QFileInfo(canonicalRoot).isDir() && QFileInfo(canonicalRoot).isReadable() ? utils::Result<QString>::success(canonicalRoot) : utils::Result<QString>::failure(utils::Error{"web_server_root_invalid", "The Web Server document root is invalid", root});
    });
    m_rootValidationFutures.removeIf([](const auto& future) { return future.isFinished(); });
    m_rootValidationFutures.append(validation);
    auto operation = validation.then(m_asyncContext.get(), [this, serverId, name = name.trimmed(), terminalId, bindHost, port](utils::Result<QString> rootResult) {
        if (m_cancelledServerOperations.contains(serverId)) {
            return QtFuture::makeReadyValueFuture(utils::Result<void>::failure(utils::Error{"web_server_start_cancelled", "The Web Server start was cancelled", serverId}));
        }
        ...
        return persistFuture.then(m_asyncContext.get(), [this, serverId, previous](utils::Result<void> persistResult) {
    ...
    return operation.then(m_asyncContext.get(), [this, serverId](utils::Result<void> result) {
...
QFuture<utils::Result<void>> WebServerPlugin::startWebServer(const QString& serverId) {
...
QFuture<utils::Result<void>> WebServerPlugin::removeWebServer(const QString& serverId) {
    if (m_pendingServerOperations.contains(serverId)) {
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, serverId, removedServer](utils::Result<void> result) {
        if (!result.hasValue()) {
...
void WebServerPlugin::setSplitRatio(int ratio) {
    ...
    // clang-format off
    future.then(m_asyncContext.get(), [this, ratio, revision](utils::Result<void> result) {
        if (result.hasValue()) {
...
utils::Result<void> WebServerPlugin::restoreState() {
    ...
    for (const auto& row : configurationRows.value()) {
        ...
        qint64 port = 0;
        QHostAddress address;
        if (serverId.isEmpty() || name.trimmed().isEmpty() || name != name.trimmed() || root.isEmpty() || !QDir::isAbsolutePath(root) || !address.setAddress(bindHost) || !persistence::readStoredInteger(row.value(QStringLiteral("port")), port) || port < 1 || port > 65535 || m_webServers.contains(serverId) || (!terminalId.isEmpty() && !serverIdForTerminal(terminalId).isEmpty())) {
            return utils::Result<void>::failure({"web_server_state_invalid", "A Web Server configuration is invalid", serverId});
        }
...
void WebServerPlugin::synchronizeTerminals() {
    ...
    // clang-format off
    const auto applySnapshot = [this](utils::Result<QJsonObject> result) {
        if (!result.hasValue()) {
        ...
        if (!hasExactKeys(snapshot, {QStringLiteral("activeTerminalId"), QStringLiteral("terminals")}) || !snapshot.value(QStringLiteral("activeTerminalId")).isString() || !snapshot.value(QStringLiteral("terminals")).isArray()) {
            host().notify(host().translate(QStringLiteral("web-server.plugin.title")), host().translate(QStringLiteral("web-server.error.message-invalid")), plugins::AlertSeverity::Error);
            return;
        }
        ...
        for (const auto& value : values) {
            ...
            if (!hasExactKeys(terminal, {QStringLiteral("id"), QStringLiteral("name"), QStringLiteral("cwd")}) || !terminal.value(QStringLiteral("id")).isString() || !terminal.value(QStringLiteral("name")).isString() || !terminal.value(QStringLiteral("cwd")).isString()) {
                host().notify(host().translate(QStringLiteral("web-server.plugin.title")), host().translate(QStringLiteral("web-server.error.message-invalid")), plugins::AlertSeverity::Error);
                return;
            }
    ...
    // clang-format on
    host().request(QStringLiteral("terminal"), QStringLiteral("terminal.workspace.snapshot"), {}, *this, applySnapshot);
}

QFuture<utils::Result<void>> WebServerPlugin::startConfiguredWebServer(const QString& serverId) {
    if (m_pendingServerOperations.contains(serverId)) {
        return QtFuture::makeReadyValueFuture(startFailure(host().translate(QStringLiteral("web-server.error.operation-pending"))));
    }
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, serverId](utils::Result<void> result) {
        m_pendingServerOperations.remove(serverId);
        m_cancelledServerOperations.remove(serverId);
        emit webServerChanged(serverId);
        return result;
...
QFuture<utils::Result<void>> WebServerPlugin::startRuntime(const QString& serverId) {
    ...
    return future.then(m_asyncContext.get(), [this, serverId, runtime, root, bindHost, port](bool started) {
        ...
        if (m_cancelledServerOperations.contains(serverId) || current == m_webServers.end() || current->second.runtime.get() != runtime) {
            return utils::Result<void>::failure({"web_server_start_cancelled", "The Web Server start was cancelled", serverId});
        }
        if (!started) {
...
utils::Result<void> WebServerPlugin::startFailure(const QString& message, const QString& detail) {
    host().notify(host().translate(QStringLiteral("web-server.error.start-title")), message, plugins::AlertSeverity::Error);
    return utils::Result<void>::failure({"web_server_start_failed", message, detail});
}
