...
CodeWorkspaceView::CodeWorkspaceView(CodeWorkspaceState state, QVector<ResolvedLanguageServer> languageServers, bool wordWrap, CodeEditorFont font, TextCharset defaultCharset, PluginHost& host, QWidget* parent) : QWidget(parent), m_initialState(std::move(state)), m_defaultCharset(defaultCharset), m_host(host), m_fileModel(new QFileSystemModel(this)), m_tree(new QTreeView(this)), m_documents(new ui::TabWidget(host.theme(), this)), m_problems(new QTreeWidget(this)), m_references(new QTreeWidget(this)), m_symbols(new QTreeWidget(this)), m_symbolSearch(new QLineEdit(this)), m_bottomPanel(new ui::TabWidget(host.theme(), this)), m_availableLanguageServers(std::move(languageServers)), m_wordWrapEnabled(wordWrap), m_font(std::move(font)) {
    ...
    m_problems->setHeaderLabels({m_host.translate(QStringLiteral("code-editor.problems.file")), m_host.translate(QStringLiteral("code-editor.problems.line")), m_host.translate(QStringLiteral("code-editor.problems.message"))});
    ...
    treeLayout->addWidget(ui::horizontalDivider(treePanel));
...
void CodeWorkspaceView::saveAll() {
    for (int index = 0; index < m_documents->count(); ++index) {
        qobject_cast<CodeDocument*>(m_documents->widget(index))->save();
    }
}
...
void CodeWorkspaceView::updateStatusBar() {
    ...
    m_indentation->setText(m_host.translate(configuration.indentStyle.value_or(IndentStyle::Space) == IndentStyle::Tab ? QStringLiteral("code-editor.status.tab-size") : QStringLiteral("code-editor.status.space-size")).arg(resolvedIndentWidth(configuration)));
...
void CodeWorkspaceView::showContextMenu(const QPoint& position) {
    ...
    auto* newFile = menu->addAction(ui::icon(ui::IconName::Add, m_host.theme()), m_host.translate(QStringLiteral("code-editor.actions.new-file")));
    auto* newDirectory = menu->addAction(ui::icon(ui::IconName::Folder, m_host.theme()), m_host.translate(QStringLiteral("code-editor.actions.new-folder")));
    ...
    auto* remove = menu->addAction(ui::icon(ui::IconName::Clear, m_host.theme()), m_host.translate(QStringLiteral("code-editor.actions.delete")));
...
void CodeWorkspaceView::createFile(const QString& directory) {
    ...
    // clang-format off
    future.then(this, [this, path](utils::Result<void> result) { reportResult(result); if (result.hasValue()) { openFile(path); } });
    // clang-format on
}
...
void CodeWorkspaceView::createDirectory(const QString& directory) {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { reportResult(result); });
    // clang-format on
}
...
void CodeWorkspaceView::renameEntry(const QString& path, const QString& name) {
    ...
    // clang-format off
    future.then(this, [this, source, destination](utils::Result<void> result) { reportResult(result); if (result.hasValue()) { updateDocumentPaths(source, destination); } });
    // clang-format on
}
...
void CodeWorkspaceView::removePath(const QString& path) {
    ...
    // clang-format off
    future.then(this, [this, removed](utils::Result<void> result) { reportResult(result); if (result.hasValue()) { for (int index = m_documents->count() - 1; index >= 0; --index) { auto* openDocument = qobject_cast<CodeDocument*>(m_documents->widget(index)); if (openDocument->path() == removed || openDocument->path().startsWith(removed + QLatin1Char('/'))) { m_documents->removeTab(index); openDocument->deleteLater(); } } emit stateChanged(); } });
    // clang-format on
}
...
void CodeWorkspaceView::showFileFinder() {
    ...
    scan.then(this, [this, root, bound](const QStringList& paths) {
        ...
        ui::showDialogWindow(finder, m_host.translate(QStringLiteral("code-editor.finder.title")));
    });
    // clang-format on
}
...
void CodeWorkspaceView::showEncodingMenu() {
    ...
    for (const auto charset : textCharsets()) {
        ...
        reopenEntry->setData(QStringLiteral("reopen/") + textCharsetName(charset));
