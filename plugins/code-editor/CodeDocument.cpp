...
void CodeDocument::setLanguageServer(LanguageServerClient* server) {
    ...
    connect(server, &LanguageServerClient::completionsReady, this, [this](const QString& path, const QVector<CompletionProposal>& proposals) { if (path != m_path) { return; } m_completionItems.clear(); for (const auto& proposal : proposals) { m_completionItems.append(proposal.item); } m_editor->showCompletions(proposals); });
...
void CodeDocument::reload() {
    ...
    // clang-format off
    future.then(this, [this, generation](utils::Result<QByteArray> result) {
...
void CodeDocument::loadEditorConfig() {
    ...
    for (qsizetype index = 0; index < paths.size(); ++index) {
        ...
        // clang-format off
        future.then(this, [this, collection, index, generation](utils::Result<QByteArray> result) {
            ...
            if (collection->pending == 0 && generation == m_editorConfigGeneration) {
                applyEditorConfig(resolveEditorConfig(m_path, collection->files));
            }
        });
        // clang-format on
    }
