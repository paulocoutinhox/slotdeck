...
void LanguageServerClient::processStarted() {
    ...
    m_initializeRequestId = sendRequest(QStringLiteral("initialize"), {{QStringLiteral("processId"), QCoreApplication::applicationPid()}, {QStringLiteral("clientInfo"), QJsonObject{{QStringLiteral("name"), QStringLiteral("SlotDeck")}}}, {QStringLiteral("rootUri"), uri(m_rootPath)}, {QStringLiteral("workspaceFolders"), QJsonArray{QJsonObject{{QStringLiteral("uri"), uri(m_rootPath)}, {QStringLiteral("name"), QFileInfo(m_rootPath).fileName()}}}}, {QStringLiteral("capabilities"), capabilities}});
}
...
void LanguageServerClient::handleCompletionResponse(int requestId, const QJsonObject& message) {
    ...
    for (const auto& value : items) {
        ...
        CompletionProposal proposal{label, item.value(QStringLiteral("detail")).toString(), LanguageServerClientHelper::hoverText(item.value(QStringLiteral("documentation"))), label, item.value(QStringLiteral("sortText")).toString(label), item, false, 0, 0, 0, 0};
