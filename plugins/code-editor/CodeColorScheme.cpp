...
class CodeColorSchemeHelper final {
  public:
    static QByteArray readCatalog(utils::Result<void>& outcome);
...
QByteArray CodeColorSchemeHelper::readCatalog(utils::Result<void>& outcome) {
    QFile file(QStringLiteral(":/slotdeck/code-editor/assets/schemes.json"));

    if (!file.open(QIODevice::ReadOnly)) {
        outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "The colour scheme catalog is unavailable", {}});
        return {};
    }
...
QVector<CodeColorScheme> CodeColorSchemeCatalog::parse(const QByteArray& text, utils::Result<void>& outcome) {
    ...
    if (error.error != QJsonParseError::NoError || !document.isObject() || !document.object().value(QStringLiteral("schemes")).isArray()) {
        outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "The colour scheme catalog is not a catalog", error.errorString()});
        return {};
    }
    ...
    for (const auto& value : catalog.value(QStringLiteral("schemes")).toArray()) {
        ...
        if (!valid) {
            outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme is not a scheme", entry.value(QStringLiteral("id")).toString()});
            return {};
        }
        ...
        if (scheme.id.isEmpty() || scheme.name.isEmpty() || identifiers.contains(scheme.id)) {
            outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme repeats an identifier or declares none", scheme.id});
            return {};
        }
        ...
        if (!valid) {
            outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme declares an invalid surface", scheme.id});
            return {};
        }
        ...
        for (const auto& key : roles.keys()) {
            if (!roles.value(key).isObject()) {
                outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme role is not a role", key});
                return {};
            }
        }

        for (const HighlightRole role : highlightRoles()) {
            const QString identifier = highlightRoleIdentifier(role);

            if (!roles.contains(identifier)) {
                outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme leaves a declared role uncoloured", QStringLiteral("%1/%2").arg(scheme.id, identifier)});
                return {};
            }
            ...
            if (!valid) {
                outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme role is invalid", QStringLiteral("%1/%2").arg(scheme.id, identifier)});
                return {};
            }
        }

        if (roles.keys().size() != highlightRoles().size()) {
            outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "A colour scheme declares a role nobody declares", scheme.id});
            return {};
        }
    ...
    if (schemes.isEmpty()) {
        outcome = utils::Result<void>::failure({"code_editor_schemes_invalid", "The catalog declares no colour scheme", {}});
    }
...
utils::Result<void>& CodeColorSchemeCatalog::mutableCatalogError() {
