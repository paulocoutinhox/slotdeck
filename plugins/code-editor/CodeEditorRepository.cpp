...
class CodeEditorRepositoryHelper final {
  public:
    ...
    static utils::Result<void> validateState(const QVector<CodeWorkspaceState>& workspaces);
    static utils::Result<QVector<CodeWorkspaceState>> invalidState(const QString& detail);
...
utils::Result<void> CodeEditorRepositoryHelper::validateState(const QVector<CodeWorkspaceState>& workspaces) {
    QSet<QString> ids;
    QSet<QString> roots;
    ...
    for (int index = 0; index < workspaces.size(); ++index) {
        ...
        if (!validWorkspace(workspace, index) || ids.contains(workspace.id) || roots.contains(workspace.rootPath)) {
            return utils::Result<void>::failure({"code_editor_state_invalid", "The code editor state is invalid", workspace.rootPath});
        }
...
utils::Result<QVector<CodeWorkspaceState>> CodeEditorRepositoryHelper::invalidState(const QString& detail) {
    return utils::Result<QVector<CodeWorkspaceState>>::failure({"code_editor_state_invalid", "The saved code editor state is invalid", detail});
}

CodeEditorRepository::CodeEditorRepository(PluginHost& host) : m_host(host) {}

utils::Result<void> CodeEditorRepository::initialize() {
...
utils::Result<QVector<CodeWorkspaceState>> CodeEditorRepository::load() const {
    ...
    for (const auto& row : workspaceRows.value()) {
        ...
        if (!persistence::validStoredTimestamp(workspace.createdAtUtc) || !persistence::validStoredTimestamp(workspace.updatedAtUtc) || workspace.updatedAtUtc < workspace.createdAtUtc) {
    ...
    return validation.hasValue() ? utils::Result<QVector<CodeWorkspaceState>>::success(std::move(workspaces)) : CodeEditorRepositoryHelper::invalidState(validation.error().detail);
}
...
QFuture<utils::Result<void>> CodeEditorRepository::saveSettings(const CodeEditorSettings& settings) {
