...
void EditorConfigHelper::applyCharset(EditorConfigProperties& properties, const QString& value) {
    if (const auto charset = parseTextCharset(value); charset.has_value()) {
...
void EditorConfigHelper::mergeFile(EditorConfigProperties& properties, const EditorConfigFile& file, const QString& filePath, bool& indentSizeFollowsTab) {
    ...
    for (const auto& rawLine : lines) {
        ...
        if (line.startsWith(QLatin1Char('[')) && line.endsWith(QLatin1Char(']'))) {
            insideSection = true;
            sectionMatches = editorConfigSectionMatches(line.mid(1, line.size() - 2), file.directoryPath, filePath);
            continue;
        }
...
QStringList editorConfigSearchPaths(const QString& filePath, const QString& rootPath) {
...
QString textCharsetName(TextCharset charset) {
    switch (charset) {
...
std::optional<TextCharset> parseTextCharset(const QString& name) {
    for (const auto charset : textCharsets()) {
        if (textCharsetName(charset) == name) {
            return charset;
        }
    }
...
const QVector<TextCharset>& textCharsets() {
...
int resolvedIndentWidth(const EditorConfigProperties& properties) {
