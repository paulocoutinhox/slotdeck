...
class LogsView final : public QWidget {
  public:
    LogsView(LogsPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host) {
        ...
        m_table = ui::dataGrid({m_host.translate(QStringLiteral("logs.viewer.time")), m_host.translate(QStringLiteral("logs.viewer.level")), m_host.translate(QStringLiteral("logs.viewer.source")), m_host.translate(QStringLiteral("logs.viewer.category")), m_host.translate(QStringLiteral("logs.viewer.message"))}, this);
        m_table->setObjectName(QStringLiteral("logsTable"));
        ui::stretchGridColumn(m_table, 4);
        root->addWidget(m_table, 1);

        m_empty = ui::emptyStateLabel(m_host.translate(QStringLiteral("logs.viewer.empty")), this);
        m_empty->hide();
  ...
  private:
    ...
    void loadPage() {
        ...
        // clang-format off
        future.then(this, [this, generation, requestedBeforeSequence](utils::Result<QVector<LogEntry>> result) {
    ...
    void render() {
        ...
        for (const auto& entry : m_entries) {
            ...
            m_table->setItem(row, 0, new QTableWidgetItem(ui::localTimestamp(entry.timestampUtc)));
...
QVector<NavigationItem> LogsPlugin::navigationItems(const ui::Theme& theme) const {
    return {{QStringLiteral("viewer"), QStringLiteral("logs.navigation.viewer"), ui::icon(ui::IconName::Logs, theme), NavigationPlacement::Secondary, NavigationOrder::Logs}};
}
...
utils::Result<void> LogsPlugin::initialize(PluginHost& host) {
    if (m_host != nullptr) {
...
QWidget* LogsPlugin::createSettingsSection(const QString& groupId, const QString& sectionId, QWidget* parent) {
    ...
    auto* clear = new QPushButton(ui::destructiveIcon(ui::IconName::Clear, m_host->theme()), m_host->translate(QStringLiteral("logs.viewer.clear")), view);
    clear->setObjectName(QStringLiteral("destructiveButton"));
    clear->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Fixed);
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("logs.settings.storage")), clear);
    ...
    const auto clearLogs = [this, view]() {
        ...
        future.then(view, [this](utils::Result<void> result) {
            if (!result.hasValue()) {
                m_host->notify(m_host->translate(QStringLiteral("logs.plugin.title")), m_host->translate(QStringLiteral("logs.error.clear-message")), AlertSeverity::Error);
            }
        });
...
void LogsPlugin::handleRequest(const QString&, const QString& topic, const QJsonObject& payload, PluginReply reply) {
    ...
    if (topic == QStringLiteral("logs.entries.clear") && payload.isEmpty()) {
        auto future = clearEntries();
        // clang-format off
        future.then(m_asyncContext.get(), [reply = std::move(reply)](utils::Result<void> result) { reply(result.hasValue() ? utils::Result<QJsonObject>::success({}) : utils::Result<QJsonObject>::failure(result.error())); });
        // clang-format on
        return;
    }
...
QFuture<utils::Result<QVector<LogEntry>>> LogsPlugin::entries(qint64 beforeSequence, int limit) {
    ...
    // clang-format off
    return future.then([](utils::Result<persistence::DatabaseRows> rows) {
        if (!rows.hasValue()) {
            return utils::Result<QVector<LogEntry>>::failure(rows.error());
        }
        QVector<LogEntry> values;
        values.reserve(rows.value().size());
        for (const auto& row : rows.value()) {
            qint64 sequence = 0;
            const QDateTime timestampUtc = persistence::parseStoredTimestamp(row.value(QStringLiteral("timestamp_utc")));
            ...
            if (!persistence::readStoredInteger(row.value(QStringLiteral("sequence")), sequence) || sequence < 1 || !persistence::validStoredTimestamp(timestampUtc) || sourcePluginId.isEmpty() || !LogsPluginHelper::isLevel(level) || category.trimmed().isEmpty() || message.trimmed().isEmpty() || error.error != QJsonParseError::NoError || !details.isObject()) {
                return utils::Result<QVector<LogEntry>>::failure({"logs_entry_invalid", "A stored log entry is invalid", error.errorString()});
            }
            values.append({sequence, timestampUtc, sourcePluginId, level, category, message, details.object()});
        }
        return utils::Result<QVector<LogEntry>>::success(std::move(values));
    });
    // clang-format on
}

QFuture<utils::Result<void>> LogsPlugin::clearEntries() {
    if (m_host == nullptr) {
...
QFuture<utils::Result<void>> LogsPlugin::appendEntry(const QString& senderPluginId, const QJsonObject& payload) {
