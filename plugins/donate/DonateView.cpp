...
DonateView::DonateView(PluginHost& host, QWidget* parent) : QWidget(parent), m_host(host) {
    ...
    profile->setPixmap(profilePixmap(profileSize, profile->devicePixelRatioF()));
    ...
    auto* github = new QPushButton(ui::primaryIcon(ui::IconName::Donate, m_host.theme()), m_host.translate(QStringLiteral("donate.view.github-sponsors")), card);
    github->setObjectName(QStringLiteral("primaryButton"));
    auto* kofi = new QPushButton(ui::icon(ui::IconName::Donate, m_host.theme()), m_host.translate(QStringLiteral("donate.view.kofi")), card);
...
QPixmap profilePixmap(int logicalSize, qreal devicePixelRatio) {
