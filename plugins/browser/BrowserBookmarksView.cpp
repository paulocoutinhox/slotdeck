...
BrowserBookmarksView::BrowserBookmarksView(BrowserPlugin& plugin, QWidget* parent) : QWidget(parent), m_plugin(plugin) {
    ...
    auto* title = ui::sectionTitleLabel(m_plugin.host().translate(QStringLiteral("browser.bookmarks.title")), headerBand);
    auto* addGroupButton = ui::toolButton(ui::IconName::Folder, m_plugin.host().theme(), m_plugin.host().translate(QStringLiteral("browser.bookmarks.add-group")), headerBand);
    auto* addBookmarkButton = ui::toolButton(ui::IconName::Bookmark, m_plugin.host().theme(), m_plugin.host().translate(QStringLiteral("browser.bookmarks.add-bookmark")), headerBand);
    m_edit = ui::toolButton(ui::IconName::Edit, m_plugin.host().theme(), m_plugin.host().translate(QStringLiteral("browser.actions.edit")), headerBand);
    m_remove = ui::toolButton(ui::IconName::Clear, m_plugin.host().theme(), m_plugin.host().translate(QStringLiteral("browser.actions.remove")), headerBand);
    ...
    // clang-format off
    connect(m_tree, &QTreeWidget::itemSelectionChanged, this, [this]() { ui::repaintTreeGlyphs(m_tree, m_plugin.host().theme()); });
    // clang-format on
...
void BrowserBookmarksView::rebuild() {
    ...
    for (const auto& bookmark : m_plugin.bookmarks()) {
        ...
        ui::setItemGlyph(item, 0, ui::IconName::Bookmark, ui::ThemeColor::Text, m_plugin.host().theme());
    ...
    for (const auto& group : m_plugin.bookmarkGroups()) {
        ...
        ui::setItemGlyph(groupItem, 0, ui::IconName::Folder, ui::ThemeColor::Text, m_plugin.host().theme());
