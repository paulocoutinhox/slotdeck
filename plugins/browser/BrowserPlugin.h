...
class BrowserPlugin final : public QObject, public PluginInterface {
  ...
  public:
    ...
    [[nodiscard]] static utils::Result<QUrl> normalizeAddress(const QString& address);
    ...
    [[nodiscard]] utils::Result<QString> createTab(const QUrl& url, bool activate);
    [[nodiscard]] utils::Result<void> closeTab(const QString& tabId);
    [[nodiscard]] utils::Result<void> activateTab(const QString& tabId);
    [[nodiscard]] utils::Result<void> moveTab(int from, int to);
    [[nodiscard]] utils::Result<void> updateTabUrl(const QString& tabId, const QUrl& url);
    [[nodiscard]] utils::Result<void> updateTabTitle(const QString& tabId, const QString& title);
    [[nodiscard]] utils::Result<QString> createBookmarkGroup(const QString& name);
    [[nodiscard]] utils::Result<void> updateBookmarkGroup(const QString& groupId, const QString& name);
    [[nodiscard]] utils::Result<void> removeBookmarkGroup(const QString& groupId);
    [[nodiscard]] utils::Result<QString> createBookmark(const QString& name, const QString& address, const QString& groupId);
    [[nodiscard]] utils::Result<void> updateBookmark(const QString& bookmarkId, const QString& name, const QString& address, const QString& groupId);
    [[nodiscard]] utils::Result<void> removeBookmark(const QString& bookmarkId);
    [[nodiscard]] utils::Result<void> applyBookmarkLayout(const QVector<QString>& orderedGroupIds, const QVector<BrowserBookmarkPlacement>& orderedBookmarks);
    [[nodiscard]] utils::Result<void> setHomepage(const QString& address);
  ...
  private:
    [[nodiscard]] utils::Result<void> restoreState();
    [[nodiscard]] utils::Result<void> restoreBookmarks();
