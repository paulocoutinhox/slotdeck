...
QVector<NavigationItem> BrowserPlugin::navigationItems(const ui::Theme& theme) const {
    return {{QStringLiteral("web"), QStringLiteral("browser.navigation.web"), ui::icon(ui::IconName::Browser, theme), NavigationPlacement::Primary, NavigationOrder::Browser}};
}
...
utils::Result<void> BrowserPlugin::initialize(PluginHost& host) {
    if (m_host != nullptr) {
        return utils::Result<void>::failure({"browser_already_initialized", "The Browser plugin is already initialized", {}});
    }
    ...
    if (!dataRoot.isAbsolute() || !dataRoot.isDir()) {
        shutdown();
        return utils::Result<void>::failure({"browser_data_directory_invalid", "The Browser application data directory is invalid", host.applicationDataPath()});
    }
    ...
    if (!QDir().mkpath(QDir(m_browserDataPath).filePath(QStringLiteral("storage"))) || !QDir().mkpath(QDir(m_browserDataPath).filePath(QStringLiteral("cache")))) {
        const utils::Error error{"browser_data_directory_failed", "The Browser data directory is unavailable", m_browserDataPath};
        shutdown();
        return utils::Result<void>::failure(error);
    }
...
QWidget* BrowserPlugin::createSettingsSection(const QString& groupId, const QString& sectionId, QWidget* parent) {
    ...
    ui::addSettingsRow(form, host().translate(QStringLiteral("browser.settings.homepage")), homepage);
...
void BrowserPlugin::handleRequest(const QString&, const QString& topic, const QJsonObject& payload, PluginReply reply) {
    ...
    if (!url.hasValue()) {
        reply(utils::Result<QJsonObject>::failure(url.error()));
        return;
    }
    ...
    reply(tab.hasValue() ? utils::Result<QJsonObject>::success({{QStringLiteral("tabId"), tab.value()}}) : utils::Result<QJsonObject>::failure(tab.error()));
}

void BrowserPlugin::handleEvent(const QString&, const QString&, const QJsonObject&) {}
...
utils::Result<QUrl> BrowserPlugin::normalizeAddress(const QString& address) {
    QString normalized = address.trimmed();

    if (normalized.isEmpty()) {
        return utils::Result<QUrl>::failure({"browser_address_invalid", "The browser address is empty", {}});
    }
    ...
    if (!url.isValid() || !schemes.contains(url.scheme().toLower()) || ((url.scheme() == QStringLiteral("http") || url.scheme() == QStringLiteral("https")) && url.host().isEmpty())) {
        return utils::Result<QUrl>::failure({"browser_address_invalid", "The browser address is invalid", address});
    }

    return utils::Result<QUrl>::success(url);
}
...
QWebEngineProfile& BrowserPlugin::profile() {
    if (m_profile == nullptr) {
        m_profile = std::make_unique<QWebEngineProfile>(QStringLiteral("slotdeck-browser"));
...
utils::Result<QString> BrowserPlugin::createTab(const QUrl& url, bool activate) {
    const auto normalized = normalizeAddress(url.toString());

    if (!normalized.hasValue()) {
        return utils::Result<QString>::failure(normalized.error());
    }
    ...
    persistTabs();
    emit tabsChanged();
    return utils::Result<QString>::success(tabId);
}

utils::Result<void> BrowserPlugin::closeTab(const QString& tabId) {
    int index = -1;
    ...
    if (index < 0) {
        return utils::Result<void>::failure({"browser_tab_unknown", "The browser tab does not exist", tabId});
    }
...
utils::Result<void> BrowserPlugin::activateTab(const QString& tabId) {
...
utils::Result<void> BrowserPlugin::moveTab(int from, int to) {
    if (from < 0 || to < 0 || from >= m_tabs.size() || to >= m_tabs.size()) {
        return utils::Result<void>::failure({"browser_tab_position_invalid", "The browser tab position is invalid", QStringLiteral("%1 -> %2").arg(from).arg(to)});
    }
...
utils::Result<void> BrowserPlugin::updateTabUrl(const QString& tabId, const QUrl& url) {
    BrowserTab* tab = findTab(tabId);
    const auto normalized = normalizeAddress(url.toString());

    if (tab == nullptr || !normalized.hasValue()) {
        return tab == nullptr ? utils::Result<void>::failure(utils::Error{"browser_tab_unknown", "The browser tab does not exist", tabId}) : utils::Result<void>::failure(normalized.error());
    }
...
utils::Result<void> BrowserPlugin::updateTabTitle(const QString& tabId, const QString& title) {
    BrowserTab* tab = findTab(tabId);
    const QString normalized = title.trimmed();

    if (tab == nullptr || normalized.isEmpty()) {
        return utils::Result<void>::failure({"browser_tab_title_invalid", "The browser tab title is invalid", tabId});
    }
...
utils::Result<QString> BrowserPlugin::createBookmarkGroup(const QString& name) {
    const QString normalizedName = name.trimmed();

    if (normalizedName.isEmpty()) {
        return utils::Result<QString>::failure({"browser_bookmark_group_name_invalid", "The bookmark group name is invalid", name});
    }
    ...
    return utils::Result<QString>::success(groupId);
}

utils::Result<void> BrowserPlugin::updateBookmarkGroup(const QString& groupId, const QString& name) {
    BrowserBookmarkGroup* group = findBookmarkGroup(groupId);
    const QString normalizedName = name.trimmed();

    if (group == nullptr) {
        return utils::Result<void>::failure({"browser_bookmark_group_unknown", "The bookmark group does not exist", groupId});
    }
    if (normalizedName.isEmpty()) {
        return utils::Result<void>::failure({"browser_bookmark_group_name_invalid", "The bookmark group name is invalid", name});
    }
...
utils::Result<void> BrowserPlugin::removeBookmarkGroup(const QString& groupId) {
    int groupIndex = -1;
...
utils::Result<QString> BrowserPlugin::createBookmark(const QString& name, const QString& address, const QString& groupId) {
    const QString normalizedName = name.trimmed();
    const auto normalizedUrl = normalizeAddress(address);

    if (normalizedName.isEmpty()) {
        return utils::Result<QString>::failure({"browser_bookmark_name_invalid", "The bookmark name is invalid", name});
    }
    if (!normalizedUrl.hasValue()) {
        return utils::Result<QString>::failure(normalizedUrl.error());
    }
    if (!groupId.isEmpty() && !bookmarkGroupExists(groupId)) {
        return utils::Result<QString>::failure({"browser_bookmark_group_unknown", "The bookmark group does not exist", groupId});
    }
    ...
    persistBookmarks();
    emit bookmarksChanged();
    return utils::Result<QString>::success(bookmarkId);
}

utils::Result<void> BrowserPlugin::updateBookmark(const QString& bookmarkId, const QString& name, const QString& address, const QString& groupId) {
    BrowserBookmark* bookmark = findBookmark(bookmarkId);
    const QString normalizedName = name.trimmed();
    const auto normalizedUrl = normalizeAddress(address);

    if (bookmark == nullptr) {
        return utils::Result<void>::failure({"browser_bookmark_unknown", "The bookmark does not exist", bookmarkId});
    }
    if (normalizedName.isEmpty()) {
        return utils::Result<void>::failure({"browser_bookmark_name_invalid", "The bookmark name is invalid", name});
    }
    if (!normalizedUrl.hasValue()) {
        return utils::Result<void>::failure(normalizedUrl.error());
    }
    if (!groupId.isEmpty() && !bookmarkGroupExists(groupId)) {
        return utils::Result<void>::failure({"browser_bookmark_group_unknown", "The bookmark group does not exist", groupId});
    }
...
utils::Result<void> BrowserPlugin::removeBookmark(const QString& bookmarkId) {
...
utils::Result<void> BrowserPlugin::applyBookmarkLayout(const QVector<QString>& orderedGroupIds, const QVector<BrowserBookmarkPlacement>& orderedBookmarks) {
    if (orderedGroupIds.size() != m_bookmarkGroups.size() || orderedBookmarks.size() != m_bookmarks.size()) {
        return utils::Result<void>::failure({"browser_bookmark_layout_invalid", "The bookmark layout is incomplete", {}});
    }
    ...
    for (const auto& groupId : orderedGroupIds) {
        if (groupIds.contains(groupId)) {
            return utils::Result<void>::failure({"browser_bookmark_layout_invalid", "The bookmark layout contains a duplicate group", groupId});
        }
        const BrowserBookmarkGroup* group = findBookmarkGroup(groupId);
        if (group == nullptr) {
            return utils::Result<void>::failure({"browser_bookmark_layout_invalid", "The bookmark layout contains an unknown group", groupId});
        }
    ...
    for (const auto& placement : orderedBookmarks) {
        if (bookmarkIds.contains(placement.bookmarkId) || (!placement.groupId.isEmpty() && !groupIds.contains(placement.groupId))) {
            return utils::Result<void>::failure({"browser_bookmark_layout_invalid", "The bookmark layout contains an invalid placement", placement.bookmarkId});
        }
        const BrowserBookmark* bookmark = findBookmark(placement.bookmarkId);
        if (bookmark == nullptr) {
            return utils::Result<void>::failure({"browser_bookmark_layout_invalid", "The bookmark layout contains an unknown bookmark", placement.bookmarkId});
        }
...
utils::Result<void> BrowserPlugin::setHomepage(const QString& address) {
...
utils::Result<void> BrowserPlugin::restoreState() {
    ...
    if (!rows.hasValue()) {
        return utils::Result<void>::failure(rows.error());
    }
    ...
    for (int position = 0; position < rows.value().size(); ++position) {
        ...
        const QDateTime created = persistence::parseStoredTimestamp(row.value(QStringLiteral("created_at_utc")));
        const QDateTime updated = persistence::parseStoredTimestamp(row.value(QStringLiteral("updated_at_utc")));
        ...
        if (tabId.isEmpty() || tabIds.contains(tabId) || title.trimmed().isEmpty() || title != title.trimmed() || !url.hasValue() || url.value().toString() != row.value(QStringLiteral("url")).toString() || !persistence::readStoredInteger(row.value(QStringLiteral("position")), storedPosition) || storedPosition != position || !persistence::readStoredInteger(row.value(QStringLiteral("active")), activeValue) || (activeValue != 0 && activeValue != 1) || !persistence::validStoredTimestamp(created) || !persistence::validStoredTimestamp(updated) || updated < created) {
            return utils::Result<void>::failure({"browser_session_invalid", "The saved browser session is invalid", tabId});
        }
        tabIds.insert(tabId);
    ...
    if (!m_tabs.isEmpty() && activeCount != 1) {
        return utils::Result<void>::failure({"browser_session_invalid", "The saved browser session has an invalid active tab", {}});
    }
    ...
    if (m_tabs.isEmpty()) {
        ...
        const auto saved = host().executeBootstrapDatabaseTransaction({{QStringLiteral("INSERT INTO browser_tabs(id, position, title, url, created_at_utc, updated_at_utc, active) VALUES(?, 0, ?, ?, ?, ?, 1)"), {m_tabs.first().id, m_tabs.first().title, m_tabs.first().url.toString(), persistence::storedTimestamp(now), persistence::storedTimestamp(now)}}});
        if (!saved.hasValue()) {
            return saved;
        }
    }
...
utils::Result<void> BrowserPlugin::restoreBookmarks() {
    ...
    if (!groupRows.hasValue()) {
        return utils::Result<void>::failure(groupRows.error());
    }
    ...
    for (int position = 0; position < groupRows.value().size(); ++position) {
        ...
        const QDateTime updated = persistence::parseStoredTimestamp(row.value(QStringLiteral("updated_at_utc")));
        qint64 storedPosition = -1;
        if (groupId.isEmpty() || groupIds.contains(groupId) || name.isEmpty() || name != name.trimmed() || !persistence::readStoredInteger(row.value(QStringLiteral("position")), storedPosition) || storedPosition != position || !persistence::validStoredTimestamp(created) || !persistence::validStoredTimestamp(updated) || updated < created) {
            return utils::Result<void>::failure({"browser_bookmarks_invalid", "The saved bookmark groups are invalid", groupId});
        }
        groupIds.insert(groupId);
    ...
    if (!bookmarkRows.hasValue()) {
        return utils::Result<void>::failure(bookmarkRows.error());
    }
    ...
    for (const auto& row : bookmarkRows.value()) {
        ...
        qint64 storedPosition = -1;
        if (bookmarkId.isEmpty() || bookmarkIds.contains(bookmarkId) || (!groupId.isEmpty() && !groupIds.contains(groupId)) || name.isEmpty() || name != name.trimmed() || !url.hasValue() || url.value().toString() != row.value(QStringLiteral("url")).toString() || !persistence::readStoredInteger(row.value(QStringLiteral("position")), storedPosition) || storedPosition != expectedPosition || !persistence::validStoredTimestamp(created) || !persistence::validStoredTimestamp(updated) || updated < created) {
            return utils::Result<void>::failure({"browser_bookmarks_invalid", "The saved bookmarks are invalid", bookmarkId});
        }
...
void BrowserPlugin::persistTabs() {
    ...
    for (int position = 0; position < candidate.size(); ++position) {
        ...
        statements.append({QStringLiteral("INSERT INTO browser_tabs(id, position, title, url, created_at_utc, updated_at_utc, active) VALUES(?, ?, ?, ?, ?, ?, ?)"), {tab.id, position, tab.title, tab.url.toString(), persistence::storedTimestamp(tab.createdAtUtc), persistence::storedTimestamp(tab.updatedAtUtc), tab.active}});
    }

    auto future = host().executeDatabaseTransaction(statements);
    // clang-format off
    future.then(m_asyncContext.get(), [this, candidate, revision](utils::Result<void> result) {
        if (result.hasValue()) {
...
void BrowserPlugin::persistBookmarks() {
    ...
    for (int position = 0; position < candidateGroups.size(); ++position) {
        ...
        statements.append({QStringLiteral("INSERT INTO browser_bookmark_groups(id, position, name, created_at_utc, updated_at_utc) VALUES(?, ?, ?, ?, ?)"), {group.id, position, group.name, persistence::storedTimestamp(group.createdAtUtc), persistence::storedTimestamp(group.updatedAtUtc)}});
    }
    ...
    for (const auto& bookmark : candidateBookmarks) {
        ...
        statements.append({QStringLiteral("INSERT INTO browser_bookmarks(id, group_id, position, name, url, created_at_utc, updated_at_utc) VALUES(?, ?, ?, ?, ?, ?, ?)"), {bookmark.id, groupId, position, bookmark.name, bookmark.url.toString(), persistence::storedTimestamp(bookmark.createdAtUtc), persistence::storedTimestamp(bookmark.updatedAtUtc)}});
        positions[bookmark.groupId] = position + 1;
    }

    auto future = host().executeDatabaseTransaction(statements);
    // clang-format off
    future.then(m_asyncContext.get(), [this, candidateGroups, candidateBookmarks, revision](utils::Result<void> result) {
        if (result.hasValue()) {
