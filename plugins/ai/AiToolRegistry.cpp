...
const SkillDescriptor* AiToolRegistryHelper::findSkill(const QVector<SkillDescriptor>& catalog, const QString& name) {
    for (const auto& skill : catalog) {
        if (skill.name.compare(name, Qt::CaseInsensitive) == 0) {
            return &skill;
        }
    }
...
bool toolAccessesConflict(const ToolAccess& first, const ToolAccess& second) {
...
utils::Result<QString> AiToolRegistry::resolveSandboxPath(const QString& sandboxRoot, const QString& path) const {
    if (sandboxRoot.isEmpty()) {
        return utils::Result<QString>::failure({"ai_tool_sandbox_missing", m_host.translate(QStringLiteral("ai.error.tool-workdir-none")), {}});
    }
...
void AiToolRegistry::setMcpClients(const QList<AiMcpClient*>& clients) {
    m_mcpTools.clear();
    ...
    for (auto* client : clients) {
        ...
        for (const auto& tool : client->tools()) {
            ...
            if (!validateToolSchema(schema).hasValue()) {
                continue;
            }
...
void AiToolRegistry::listServerCatalog(const ToolCall& call, bool prompts, const ToolCompletion& completion) {
    ...
    for (const auto& client : m_mcpClients) {
        ...
        // clang-format off
        const auto collect = [call, completion, sections, remaining, serverId, prompts](utils::Result<QJsonObject> result) {
            if (result.hasValue()) {
...
AiMcpClient* AiToolRegistry::readyServer(const QString& serverId) const {
    for (const auto& client : m_mcpClients) {
...
void AiToolRegistry::readServerResource(const ToolCall& call, const ToolCompletion& completion) {
    ...
    // clang-format off
    const auto reply = [call, completion](utils::Result<QJsonObject> result) {
        if (!result.hasValue()) {
...
void AiToolRegistry::callMcpTool(const ToolCall& call, const ToolCompletion& completion) {
    ...
    // clang-format off
    const auto reply = [call, completion](utils::Result<QJsonObject> result) { if (!result.hasValue()) { completion({call.id, result.error().message, true}); return; } completion(AiToolRegistryHelper::mcpToolResult(call, result.value())); };
    // clang-format on
...
void AiToolRegistry::createDirectory(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [call, completion](utils::Result<void> result) { completion(result.hasValue() ? ToolResult{call.id, QStringLiteral("created"), false} : AiToolRegistryHelper::failure(call, result.error().message)); });
    // clang-format on
}
...
void AiToolRegistry::movePath(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [call, completion](utils::Result<void> result) { completion(result.hasValue() ? ToolResult{call.id, QStringLiteral("moved"), false} : AiToolRegistryHelper::failure(call, result.error().message)); });
    // clang-format on
}
...
void AiToolRegistry::copyFile(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [call, completion](utils::Result<void> result) { completion(result.hasValue() ? ToolResult{call.id, QStringLiteral("copied"), false} : AiToolRegistryHelper::failure(call, result.error().message)); });
    // clang-format on
}
...
void AiToolRegistry::removePath(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [call, completion](utils::Result<void> result) { completion(result.hasValue() ? ToolResult{call.id, QStringLiteral("removed"), false} : AiToolRegistryHelper::failure(call, result.error().message)); });
    // clang-format on
}
...
void AiToolRegistry::writeFile(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [call, completion, path](utils::Result<void> result) { completion(result.hasValue() ? ToolResult{call.id, path, false} : AiToolRegistryHelper::failure(call, result.error().message)); });
    // clang-format on
}
...
void AiToolRegistry::editFile(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    const auto applyEdit = [this, call, completion, path, oldText, newText, replaceAll](utils::Result<QByteArray> result) {
        if (!result.hasValue()) {
            completion(AiToolRegistryHelper::failure(call, result.error().message));
            return;
        }
        ...
        written.then(this, [call, completion, path, occurrences, replaceAll](utils::Result<void> saved) { completion(saved.hasValue() ? ToolResult{call.id, QStringLiteral("%1 %2").arg(path, QString::number(replaceAll ? occurrences : 1)), false} : AiToolRegistryHelper::failure(call, saved.error().message)); });
    };
...
void AiToolRegistry::readImage(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [this, call, completion, path, mediaType](utils::Result<QByteArray> result) {
        if (!result.hasValue()) {
            completion(AiToolRegistryHelper::failure(call, result.error().message));
            return;
        }
...
void AiToolRegistry::listDirectory(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    future.then(this, [call, completion](utils::Result<QVector<filesystem::DirectoryEntry>> listed) {
        if (!listed.hasValue()) {
...
void AiToolRegistry::generateImage(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    QUrl endpoint(m_mediaAddress);
    endpoint.setPath(endpoint.path() + QStringLiteral("/images/generations"));
...
void AiToolRegistry::generateSpeech(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    const bool openAi = m_speech.provider == SpeechProvider::OpenAi;
    QUrl endpoint(m_speechAddress);
    endpoint.setPath(endpoint.path() + (openAi ? QStringLiteral("/audio/speech") : QStringLiteral("/v1/text-to-speech/") + voice));
    QNetworkRequest request(endpoint);
    request.setHeader(QNetworkRequest::ContentTypeHeader, QStringLiteral("application/json"));
    request.setTransferTimeout(imageTimeoutMs);
...
void AiToolRegistry::listVoices(const ToolCall& call, const ToolCompletion& completion) {
    ...
    QUrl endpoint(m_speechAddress);
    endpoint.setPath(endpoint.path() + QStringLiteral("/v1/voices"));
    QNetworkRequest request(endpoint);
    request.setTransferTimeout(searchTimeoutMs);
    request.setRawHeader(QByteArrayLiteral("xi-api-key"), apiKey.value().toUtf8());
...
void AiToolRegistry::listSkills(const QString& sandboxRoot, const ToolCompletion& completion, const ToolCall& call) {
    // clang-format off
    m_skills->discover(sandboxRoot, [this, completion, call](const QVector<SkillDescriptor>& catalog) {
        QStringList lines;
        for (const auto& skill : catalog) {
...
void AiToolRegistry::searchSkills(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    m_skills->discover(sandboxRoot, [this, completion, call, query](const QVector<SkillDescriptor>& catalog) {
        QStringList lines;
        for (const auto& skill : catalog) {
...
void AiToolRegistry::readSkill(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    m_skills->discover(sandboxRoot, [this, completion, call, name](const QVector<SkillDescriptor>& catalog) {
        const SkillDescriptor* skill = AiToolRegistryHelper::findSkill(catalog, name);
        if (skill == nullptr) {
        ...
        future.then(this, [this, completion, call, name](utils::Result<QByteArray> content) {
            if (!content.hasValue()) {
...
void AiToolRegistry::readSkillFile(const ToolCall& call, const QString& sandboxRoot, const ToolCompletion& completion) {
    ...
    // clang-format off
    m_skills->discover(sandboxRoot, [this, completion, call, name, relative](const QVector<SkillDescriptor>& catalog) {
        const SkillDescriptor* skill = AiToolRegistryHelper::findSkill(catalog, name);
        if (skill == nullptr) {
            completion(AiToolRegistryHelper::failure(call, m_host.translate(QStringLiteral("ai.skill.unknown")).arg(name)));
            return;
        ...
        future.then(this, [completion, call](utils::Result<QByteArray> content) {
            if (!content.hasValue()) {
