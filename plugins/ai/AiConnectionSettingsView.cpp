...
AiConnectionSettingsView::AiConnectionSettingsView(AiPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host), m_connections(plugin.connections()) {
    ...
    ui::addSettingsRow(form, m_host.translate(QStringLiteral("ai.settings.default-connection")), m_defaultConnection);
    layout->addLayout(form);

    auto* actions = ui::settingsActionRow(page);
    auto* actionsLayout = qobject_cast<QHBoxLayout*>(actions->layout());
    auto* add = ui::toolButton(ui::IconName::Add, m_host.theme(), m_host.translate(QStringLiteral("ai.connection.add")), actions);
    add->setObjectName(QStringLiteral("aiConnectionAdd"));
    auto* edit = ui::toolButton(ui::IconName::Edit, m_host.theme(), m_host.translate(QStringLiteral("ai.connection.edit")), actions);
    edit->setObjectName(QStringLiteral("aiConnectionEdit"));
    auto* remove = ui::toolButton(ui::IconName::Close, m_host.theme(), m_host.translate(QStringLiteral("ai.connection.remove")), actions);
    ...
    m_empty = ui::emptyStateLabel(m_host.translate(QStringLiteral("ai.connection.empty")), page);
...
void AiConnectionSettingsView::rebuild() {
    ...
    for (int row = 0; row < static_cast<int>(m_connections.size()); ++row) {
        ...
        m_grid->setItem(row, 0, new QTableWidgetItem(connectionLabel(connection)));
...
void AiConnectionSettingsView::addConnection() {
    ...
    persist(updated, m_connections.isEmpty() ? connectionKey(dialog.connection()) : selectedDefaultKey());
}
...
void AiConnectionSettingsView::editConnection() {
    ...
    // clang-format off
    future.then(this, [this, updated](utils::Result<void> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("ai.plugin.title")), m_host.translate(QStringLiteral("ai.error.connection-save")), AlertSeverity::Error); return; } m_connections = updated; rebuild(); });
    // clang-format on
}
...
void AiConnectionSettingsView::removeConnection() {
    ...
    if (!m_host.confirm(this, m_host.translate(QStringLiteral("ai.connection.remove-title")), m_host.translate(QStringLiteral("ai.connection.remove-message")), connectionLabel(m_connections.at(row)), m_host.translate(QStringLiteral("ai.connection.remove")), true)) {
        return;
    }

    QVector<ModelConnection> updated = m_connections;
    const QString removedKey = connectionKey(updated.at(row));
    updated.removeAt(row);
    const QString defaultKey = m_plugin.defaultConnectionKey() == removedKey ? (updated.isEmpty() ? QString{} : connectionKey(updated.first())) : selectedDefaultKey();
