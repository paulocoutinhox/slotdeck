...
AiConversationView::AiConversationView(AiPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host), m_fontSize(plugin.executionSettings().chatFontSize) {
    ...
    m_empty = ui::emptyStateLabel(m_host.translate(QStringLiteral("ai.conversation.empty")), this);
    ...
    m_send = new QPushButton(ui::primaryIcon(ui::IconName::Forward, m_host.theme()), {}, composerRow);
...
void AiConversationView::setTask(const QString& taskId) {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("ai.plugin.title")), m_host.translate(QStringLiteral("ai.error.conversation-save")), AlertSeverity::Error); } rebuild(); });
    // clang-format on
    rebuild();
}
...
void AiConversationView::appendToolRow(QWidget* bubble, const QJsonObject& call, const QColor& ink) {
    ...
    mark->setPixmap(ui::icon(ui::IconName::Tool, m_host.theme().color(ui::ThemeColor::Accent)).pixmap(glyph, glyph));
...
void AiConversationView::loadOlder() {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<bool> result) { m_loadingOlder = false; if (result.hasValue() && result.value()) { m_scroll->verticalScrollBar()->setValue(1); } });
    // clang-format on
}
...
void AiConversationView::submit() {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("ai.plugin.title")), m_host.translate(QStringLiteral("ai.error.conversation-save")), AlertSeverity::Error); } });
    // clang-format on
}
