...
class AiConnectionDialogHelper final {
  public:
    static utils::Result<QJsonObject> parseExtraParameters(const QString& text);
};

// An empty editor declares nothing extra, and anything else has to be one JSON object.
utils::Result<QJsonObject> AiConnectionDialogHelper::parseExtraParameters(const QString& text) {
    if (text.trimmed().isEmpty()) {
        return utils::Result<QJsonObject>::success({});
    }
...
AiConnectionDialog::AiConnectionDialog(PluginHost& host, const ModelConnection& connection, QStringList takenKeys, QWidget* parent) : QDialog(parent), m_host(host), m_original(connection), m_takenKeys(std::move(takenKeys)) {
    ...
    m_refreshModels = ui::toolButton(ui::IconName::Refresh, m_host.theme(), m_host.translate(QStringLiteral("ai.settings.refresh-models")), this);
    ...
    connect(&m_discovery, &AiModelDiscovery::failed, this, [this](const utils::Error& error) { m_refreshModels->setEnabled(true); showValidation(validationMessage(error)); });
    // clang-format on
}

const ProviderDescriptor* AiConnectionDialog::selectedProvider() const {
    return findProvider(m_provider->currentData().toString());
}
...
void AiConnectionDialog::selectProvider() {
    ...
    rebuildModels();
    const ModelConnection declared = declaredConnection(*descriptor, m_model->currentText().trimmed());
...
void AiConnectionDialog::rebuildParameters() {
    ...
    for (const auto& parameter : applicableParameters(*descriptor, modelId)) {
        ...
        m_parameterForm->addRow(m_host.translate(parameter.titleKey), numericEditor == nullptr ? editor : ui::stepperRow(numericEditor, m_host.theme(), this));
    }
}
...
QString AiConnectionDialog::validationMessage(const utils::Error& error) const {
