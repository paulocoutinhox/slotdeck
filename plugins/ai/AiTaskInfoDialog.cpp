...
AiTaskInfoDialog::AiTaskInfoDialog(AiPlugin& plugin, PluginHost& host, const AiTask& task, QWidget* parent) : QDialog(parent), m_plugin(plugin), m_host(host), m_task(task) {
    ...
    m_stop = new QPushButton(ui::destructiveIcon(ui::IconName::Stop, m_host.theme()), m_host.translate(QStringLiteral("ai.task.stop")), header);
    ...
    ui::stretchGridColumn(m_executionGrid, 4);
    ...
    if (m_task.executionKind == TaskExecutionKind::Agent) {
        ...
        m_logGrid = ui::dataGrid({m_host.translate(QStringLiteral("ai.log.timestamp")), m_host.translate(QStringLiteral("ai.log.level")), m_host.translate(QStringLiteral("ai.log.event")), m_host.translate(QStringLiteral("ai.log.detail"))}, logsPage);
        ...
        ui::stretchGridColumn(m_logGrid, 3);
    ...
    // clang-format off
    connect(m_stop, &QPushButton::clicked, this, [this]() { auto future = m_plugin.stopTask(m_task.id); future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("ai.plugin.title")), m_host.translate(QStringLiteral("ai.error.stop")), AlertSeverity::Error); } }); });
...
void AiTaskInfoDialog::loadExecutions() {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<QVector<TaskExecution>> loaded) { showExecutions(loaded); });
    // clang-format on
}

void AiTaskInfoDialog::showExecutions(const utils::Result<QVector<TaskExecution>>& loaded) {
    if (!loaded.hasValue()) {
...
void AiTaskInfoDialog::showExecution(int row) {
    ...
    // clang-format off
    future.then(this, [this, revision](utils::Result<QVector<ExecutionLogEntry>> logs) { if (revision == m_logRevision) { showLogs(logs); } });
    // clang-format on
}

void AiTaskInfoDialog::showLogs(const utils::Result<QVector<ExecutionLogEntry>>& logs) {
    if (!logs.hasValue()) {
        m_logGrid->setRowCount(0);
        m_logEntries.clear();
        return;
    ...
    for (int entry = 0; entry < static_cast<int>(m_logEntries.size()); ++entry) {
        ...
        auto* open = ui::chipButton(m_host.translate(QStringLiteral("ai.log.open-payload")), m_host.theme(), actions);
...
void AiTaskInfoDialog::showPayload(const QString& title, const QString& payload) {
    ...
    ui::showDialogWindow(viewer, title);
}
