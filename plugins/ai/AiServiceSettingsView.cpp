...
AiSearchSettingsView::AiSearchSettingsView(AiPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host) {
    ...
    m_service->addItem(m_host.translate(QStringLiteral("ai.search.brave")), searchProviderIdentifier(SearchProvider::Brave));
    m_service->addItem(m_host.translate(QStringLiteral("ai.search.tavily")), searchProviderIdentifier(SearchProvider::Tavily));
    m_service->addItem(m_host.translate(QStringLiteral("ai.search.searxng")), searchProviderIdentifier(SearchProvider::SearxNg));
    ui::sortComboBoxItems(m_service);
    m_service->setCurrentIndex(m_service->findData(searchProviderIdentifier(settings.provider)));
    ...
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.search-provider")), m_service);
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.search-instance")), m_instance);
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.search-key")), m_apiKey);
...
void AiSearchSettingsView::persist() {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("ai.plugin.title")), m_host.translate(QStringLiteral("ai.error.service-save")), AlertSeverity::Error); } });
    // clang-format on
}
...
AiSpeechSettingsView::AiSpeechSettingsView(AiPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host) {
    ...
    m_service->addItem(m_host.translate(QStringLiteral("ai.speech.elevenlabs")), speechProviderIdentifier(SpeechProvider::ElevenLabs));
    m_service->addItem(m_host.translate(QStringLiteral("ai.speech.openai")), speechProviderIdentifier(SpeechProvider::OpenAi));
    ui::sortComboBoxItems(m_service);
    m_service->setCurrentIndex(m_service->findData(speechProviderIdentifier(settings.provider)));
    ...
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.speech-provider")), m_service);
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.speech-voice")), m_voice);
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.speech-voice")), m_declaredVoice);
    ui::addSettingsRow(m_form, m_host.translate(QStringLiteral("ai.settings.speech-key")), m_apiKey);
    layout->addLayout(m_form);
    layout->addStretch(1);
    root->addWidget(page, 1);
...
SpeechProvider AiSpeechSettingsView::selectedService() const {
    return speechProviderFromIdentifier(m_service->currentData().toString()).value_or(SpeechProvider::ElevenLabs);
}
...
void AiSpeechSettingsView::applyService() {
    const SpeechProvider selected = selectedService();
...
QString AiSpeechSettingsView::selectedVoice() const {
    return speechProviderDeclaredVoices(selectedService()).isEmpty() ? m_voice->text().trimmed() : m_declaredVoice->currentData().toString();
}
