...
utils::Result<void> validateToolSchema(const ToolSchema& schema) {
    if (!AiToolContractHelper::toolNamePattern().match(schema.name).hasMatch()) {
        return utils::Result<void>::failure({"ai_tool_invalid", "The tool name is invalid", schema.name});
    }
    if (schema.descriptionKey.isEmpty()) {
        return utils::Result<void>::failure({"ai_tool_invalid", "The tool description is missing", schema.name});
    }
...
QJsonObject serializeAssistantTurn(WireProtocol protocol, const QString& content, const QVector<ToolCall>& calls) {
    if (protocol == WireProtocol::Anthropic) {
        QJsonArray blocks;
        if (!content.isEmpty()) {
...
QVector<QJsonObject> serializeToolResults(WireProtocol protocol, const QVector<ToolResult>& results) {
    if (protocol == WireProtocol::Anthropic) {
        QJsonArray blocks;
...
bool protocolRequiresAlternatingRoles(WireProtocol protocol) {
...
QJsonArray enforceProtocolShape(WireProtocol protocol, const QJsonArray& messages) {
    if (!protocolRequiresAlternatingRoles(protocol)) {
        return messages;
    }
...
utils::Result<QVector<ToolCall>> ToolCallAccumulator::calls() const {
    ...
    for (auto entry = m_pending.constBegin(); entry != m_pending.constEnd(); ++entry) {
        ...
        if (pending.id.isEmpty() || pending.name.isEmpty()) {
            return utils::Result<QVector<ToolCall>>::failure({"ai_tool_call_invalid", "The model returned an incomplete tool call", pending.name});
        }
