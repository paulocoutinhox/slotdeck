...
QString AiTasksViewHelper::scheduleSummary(const AiTask& task, PluginHost& host) {
    ...
    if (task.schedule->enabled) {
        return host.translate(QStringLiteral("ai.task.scheduled")).arg(ui::localTimestamp(task.schedule->nextRunAtUtc));
    }
...
class TaskCard final : public QFrame {
  public:
    TaskCard(AiPlugin& plugin, PluginHost& host, AiTask task, QWidget* parent) : QFrame(parent), m_plugin(plugin), m_host(host), m_task(std::move(task)) {
        ...
        m_start = ui::toolButton(ui::IconName::Start, m_host.theme(), m_host.translate(QStringLiteral("ai.task.start")), this);
        m_start->setObjectName(QStringLiteral("aiTaskStart"));
        m_stop = ui::toolButton(ui::IconName::Stop, m_host.theme(), m_host.translate(QStringLiteral("ai.task.stop")), this);
        m_stop->setObjectName(QStringLiteral("aiTaskStop"));
        m_edit = ui::toolButton(ui::IconName::Edit, m_host.theme(), m_host.translate(QStringLiteral("ai.task.edit")), this);
        m_edit->setObjectName(QStringLiteral("aiTaskEdit"));
        m_remove = ui::toolButton(ui::IconName::Clear, m_host.theme(), m_host.translate(QStringLiteral("ai.task.remove")), this);
        m_remove->setObjectName(QStringLiteral("aiTaskRemove"));
        m_info = ui::toolButton(ui::IconName::Information, m_host.theme(), m_host.translate(QStringLiteral("ai.task.info")), this);
        m_info->setObjectName(QStringLiteral("aiTaskInfo"));
        // An agent task is a conversation, so the card opens it directly.
        m_chat = ui::toolButton(ui::IconName::Chat, m_host.theme(), m_host.translate(QStringLiteral("ai.task.chat")), this);
        ...
        m_schedule = ui::toolButton(ui::IconName::Schedule, m_host.theme(), m_host.translate(QStringLiteral("ai.task.schedule-remove")), this);
        ...
        m_openFolder = ui::toolButton(ui::IconName::Folder, m_host.theme(), m_host.translate(QStringLiteral("ai.task.open-workdir")), this);
        m_openFolder->setObjectName(QStringLiteral("aiTaskOpenWorkdir"));
        m_serveFolder = ui::toolButton(ui::IconName::WebServer, m_host.theme(), m_host.translate(QStringLiteral("ai.task.serve-workdir")), this);
...
AiTasksView::AiTasksView(AiPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host) {
    ...
    auto* addTask = ui::toolButton(ui::IconName::Add, m_host.theme(), m_host.translate(QStringLiteral("ai.task.add")), header);
    addTask->setObjectName(QStringLiteral("aiAddTask"));
    auto* addWorkspace = ui::toolButton(ui::IconName::Workspace, m_host.theme(), m_host.translate(QStringLiteral("ai.workspace.add")), header);
    addWorkspace->setObjectName(QStringLiteral("aiAddWorkspace"));
    auto* renameWorkspaceButton = ui::toolButton(ui::IconName::Edit, m_host.theme(), m_host.translate(QStringLiteral("ai.workspace.rename")), header);
    ...
    m_workspaceSeparator = ui::horizontalDivider(board);
    ...
    emptyLayout->addWidget(ui::emptyStateLabel(m_host.translate(QStringLiteral("ai.workspace.empty")), m_empty));
    auto* createFirstWorkspace = new QPushButton(ui::primaryIcon(ui::IconName::Add, m_host.theme()), m_host.translate(QStringLiteral("ai.workspace.add")), m_empty);
    ...
    connect(m_workspaces, &QTabBar::currentChanged, this, [this](int index) { if (!m_synchronizing && index >= 0) { auto future = m_plugin.activateWorkspace(m_workspaces->tabData(index).toString()); future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.workspace-save"))); } }); } });
...
void AiTasksView::createWorkspace() {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<QString> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.workspace-save"))); } });
    // clang-format on
}
...
void AiTasksView::renameWorkspace() {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.workspace-save"))); } });
    // clang-format on
}
...
void AiTasksView::removeSchedule(const AiTask& task) {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.task-save"))); } });
    // clang-format on
}
...
void AiTasksView::editTask(std::optional<AiTask> task) {
    ...
    // clang-format off
    future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.schedule-save"))); } });
    // clang-format on
}
...
TaskCard* AiTasksView::createCard(const AiTask& task) {
    ...
    // clang-format off
    connect(card->startButton(), &QToolButton::clicked, this, [this, taskId]() { auto future = m_plugin.startTask(taskId); future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.task-run"))); } }); });
    connect(card->stopButton(), &QToolButton::clicked, this, [this, taskId]() { auto future = m_plugin.stopTask(taskId); future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.task-run"))); } }); });
    ...
    connect(card->openFolderButton(), &QToolButton::clicked, this, [this, card]() { requestFolderDestination(QStringLiteral("code-editor"), QStringLiteral("code-editor.workspace.open"), card->task().workdir); });
    connect(card->serveFolderButton(), &QToolButton::clicked, this, [this, card]() { requestFolderDestination(QStringLiteral("web-server"), QStringLiteral("web-server.instance.create"), card->task().workdir); });
    connect(card->removeButton(), &QToolButton::clicked, this, [this, card, taskId]() { if (m_host.confirm(this, m_host.translate(QStringLiteral("ai.task.remove")), m_host.translate(QStringLiteral("ai.task.remove-message")), card->task().title, m_host.translate(QStringLiteral("ai.task.remove")), true)) { auto future = m_plugin.removeTask(taskId); future.then(this, [this](utils::Result<void> result) { if (!result.hasValue()) { showError(result.error(), m_host.translate(QStringLiteral("ai.error.task-save"))); } }); } });
    // clang-format on
    return card;
}

// The plugin that owns the folder decides what to do with it and reveals itself, so the board never reaches into another plugin.
void AiTasksView::requestFolderDestination(const QString& pluginId, const QString& topic, const QString& path) {
    if (path.isEmpty()) {
        return;
    }
    ...
    // clang-format off
    const auto answered = [this](utils::Result<QJsonObject> result) { if (!result.hasValue()) { showError(result.error(), result.error().message); } };
    // clang-format on
    m_host.request(pluginId, topic, {{QStringLiteral("path"), path}}, *this, answered);
}
