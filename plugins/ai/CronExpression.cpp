...
class CronExpressionHelper final {
  public:
    static utils::Result<int> parseNumber(const QString& value, int minimum, int maximum);
    static utils::Result<ParsedField> parseField(const QString& field, int minimum, int maximum, bool normalizeSunday);
...
utils::Result<ParsedField> CronExpressionHelper::parseField(const QString& field, int minimum, int maximum, bool normalizeSunday) {
    ...
    for (const auto& element : field.split(QLatin1Char(','))) {
        ...
        if (range.size() < 1 || range.size() > 2 || range.first().isEmpty() || range.last().isEmpty()) {
            return utils::Result<ParsedField>::failure({"ai_tasks_cron_field_invalid", "The cron expression contains an invalid field", field});
        }
        ...
        if (!first.hasValue() || !last.hasValue() || first.value() > last.value()) {
            return utils::Result<ParsedField>::failure(!first.hasValue() ? first.error() : !last.hasValue() ? last.error() : utils::Error{"ai_tasks_cron_range_invalid", "The cron expression contains an invalid range", element});
        }
...
utils::Result<QDateTime> CronExpression::nextAfter(const QDateTime& afterUtc, const QTimeZone& timeZone) const {
    if (!afterUtc.isValid() || afterUtc.timeSpec() != Qt::UTC || !timeZone.isValid()) {
        return utils::Result<QDateTime>::failure({"ai_tasks_schedule_context_invalid", "The cron scheduling context is invalid", {}});
    }
        ...
        for (int hour = 0; hour < hoursInDay; ++hour) {
            ...
            for (int minute = 0; minute < minutesInHour; ++minute) {
                ...
                if (instant.has_value() && instant.value() > afterUtc) {
                    return utils::Result<QDateTime>::success(instant.value());
                }
            }
        }
    }

    return utils::Result<QDateTime>::failure({"ai_tasks_cron_occurrence_unavailable", "The cron expression has no occurrence within the supported scheduling horizon", {}});
}
...
bool CronExpression::matches(const QDateTime& localDateTime) const {
