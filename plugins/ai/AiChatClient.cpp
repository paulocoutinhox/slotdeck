...
QString completionPath(const ProviderDescriptor& provider) {
    return provider.protocol == WireProtocol::Anthropic ? QStringLiteral("/v1/messages") : QStringLiteral("/chat/completions");
}
...
bool truncatedByOutputBudget(const QString& finishReason) {
...
QJsonObject buildRequestBody(const ProviderDescriptor& provider, const ChatRequest& request, const std::function<QString(const QString&)>& translate) {
    ...
    for (const auto& parameter : applicableParameters(provider, connection.modelId)) {
        ...
        if (parameter.modelMaximumWhenZero && declared.toInteger(0) == 0) {
            const qint64 maximum = outputBudget(connection);
            if (maximum > 0) {
...
void AiHttpChatClient::dispatch() {
    ...
    const QJsonObject body = buildRequestBody(*provider, m_request, m_translate);
...
qint64 AiHttpChatClient::retryDelay(const QByteArray& retryAfter) const {
    ...
    return std::min(ceiling, static_cast<qint64>(aiLimits().retryBackoffMs) << m_attempt);
}
