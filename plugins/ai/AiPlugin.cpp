...
constexpr qint64 maximumContextFileBytes = 1 << 17;
...
class AiPluginHelper final {
  public:
    ...
    static utils::Result<AiTask> prepareTaskSchedule(AiTask task, const QDateTime& nowUtc);
    static utils::Result<TaskSchedule> advanceSchedule(TaskSchedule schedule, const QDateTime& triggeredAtUtc);
...
utils::Result<AiTask> AiPluginHelper::prepareTaskSchedule(AiTask task, const QDateTime& nowUtc) {
    if (!task.schedule.has_value()) {
        return utils::Result<AiTask>::success(std::move(task));
    }
    ...
    if (schedule.kind == ScheduleKind::Cron) {
        ...
        if (!expression.hasValue()) {
            return utils::Result<AiTask>::failure(expression.error());
        }
        const auto next = expression.value().nextAfter(nowUtc, QTimeZone(schedule.timeZoneId));
        if (!next.hasValue()) {
            return utils::Result<AiTask>::failure(next.error());
        }
        schedule.nextRunAtUtc = next.value();
    }

    const auto validation = AiTaskRepository::validateSchedule(schedule);
    return validation.hasValue() ? utils::Result<AiTask>::success(std::move(task)) : utils::Result<AiTask>::failure(validation.error());
}
...
utils::Result<void> AiPlugin::initialize(PluginHost& host) {
    if (m_host != nullptr) {
        return utils::Result<void>::failure({"ai_tasks_already_initialized", "The AI plugin is already initialized", {}});
    }
    ...
    if (const auto& catalogError = aiCatalogError(); !catalogError.hasValue()) {
        shutdown();
        return catalogError;
    }
...
QWidget* AiPlugin::createProviderSelectionSection(QWidget* parent) {
    ...
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("ai.settings.provider-scope")), provider);
...
QWidget* AiPlugin::createExecutionSection(QWidget* parent) {
    ...
    iterations->setRange(0, aiLimits().maximumAgentIterations);
    ...
    timeout->setRange(0, aiLimits().maximumCommandTimeoutSeconds);
    ...
    parallel->setRange(0, aiLimits().maximumParallelExecutions);
    parallel->setValue(m_settings.execution.parallelExecutions);
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("ai.settings.maximum-iterations")), ui::stepperRow(iterations, m_host->theme(), page));
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("ai.settings.command-timeout")), ui::stepperRow(timeout, m_host->theme(), page));
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("ai.settings.parallel-limit")), ui::stepperRow(parallel, m_host->theme(), page));
    ui::addSettingsRow(form, m_host->translate(QStringLiteral("ai.settings.chat-font-size")), ui::stepperRow(chatFont, m_host->theme(), page));
    layout->addLayout(form);
    layout->addStretch(1);

    // clang-format off
    const auto persist = [this, iterations, timeout, parallel, chatFont]() { auto future = saveExecutionSettings({iterations->value(), timeout->value(), parallel->value(), chatFont->value()}); future.then(iterations, [this](utils::Result<void> result) { if (!result.hasValue()) { reportFailure(result.error(), m_host->translate(QStringLiteral("ai.error.settings-save"))); } }); };
...
void AiPlugin::handleRequest(const QString&, const QString& topic, const QJsonObject& payload, PluginReply reply) {
    if (topic == QStringLiteral("ai.task.start") && hasExactKeys(payload, {QStringLiteral("taskId")}) && payload.value(QStringLiteral("taskId")).isString()) {
        auto future = startTask(payload.value(QStringLiteral("taskId")).toString());
        // clang-format off
        future.then(m_asyncContext.get(), [reply](utils::Result<void> result) { reply(result.hasValue() ? utils::Result<QJsonObject>::success({}) : utils::Result<QJsonObject>::failure(result.error())); });
        // clang-format on
        return;
    }
...
void AiPlugin::handleEvent(const QString&, const QString& topic, const QJsonObject& payload) {
    ...
    stepped.chatFontSize = step == ContentFontStep::Reset ? defaultChatFontSize : ui::steppedContentFontSize(stepped.chatFontSize, step == ContentFontStep::Increase ? 1 : -1);
    auto future = saveExecutionSettings(stepped);
    // clang-format off
    future.then(m_asyncContext.get(), [](utils::Result<void>) {});
    // clang-format on
}
...
std::optional<ModelConnection> AiPlugin::defaultConnection() const {
    const ModelConnection* connection = findConnection(m_settings.connections, m_settings.defaultConnectionKey);
...
utils::Result<AiAgent> AiPlugin::agentForTask(const AiTask& task) const {
    ...
    return utils::Result<AiAgent>::success(*agent);
}
...
QFuture<utils::Result<void>> AiPlugin::persistSettings(AiSettings next) {
    if (m_repository == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_not_initialized", "The AI plugin is not initialized", {}}));
    }
...
QFuture<utils::Result<void>> AiPlugin::saveSpeechSettings(const SpeechSettings& settings) {
    AiSettings next = m_settings;
    next.speech = settings;
    return persistSettings(std::move(next));
}

QFuture<utils::Result<void>> AiPlugin::saveExecutionSettings(const ExecutionSettings& settings) {
    if (settings.parallelExecutions < 0) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_parallel_limit_invalid", "The parallel execution limit is invalid", QString::number(settings.parallelExecutions)}));
    }
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this](utils::Result<void> result) { if (result.hasValue()) { emit executionSettingsChanged(); dispatchQueue(); } return result; });
    // clang-format on
}

QFuture<utils::Result<QVector<TaskExecution>>> AiPlugin::executions(const QString& taskId) {
    if (m_repository == nullptr) {
...
QFuture<utils::Result<QVector<ExecutionLogEntry>>> AiPlugin::executionLogs(const QString& executionId) {
    if (m_repository == nullptr) {
...
QFuture<utils::Result<void>> AiPlugin::replaceConnection(const QString& previousKey, const ModelConnection& connection) {
    if (findConnection(m_settings.connections, previousKey) == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_connection_unknown", "The AI connection is not configured", previousKey}));
    }
    ...
    auto future = persistSettings(std::move(next));
    // clang-format off
    return future.then(m_asyncContext.get(), [this](utils::Result<void> result) { if (result.hasValue()) { dispatchQueue(); } return result; });
    // clang-format on
}
...
QFuture<utils::Result<void>> AiPlugin::saveConnections(const QVector<ModelConnection>& connections, const QString& defaultConnectionKey) {
    ...
    if (!defaultConnectionKey.isEmpty() && findConnection(connections, defaultConnectionKey) == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_connection_unknown", "The default AI connection is not configured", defaultConnectionKey}));
    }
    ...
    for (const auto& agent : m_settings.agents) {
        if (findConnection(connections, agent.connectionKey) == nullptr) {
            return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_connection_in_use", "The connection is used by an agent", agent.name}));
        }
    }
...
QFuture<utils::Result<void>> AiPlugin::saveRateLimits(const QVector<ProviderRateLimit>& limits) {
    ...
    for (const auto& limit : limits) {
        ...
        if (providers.contains(limit.providerId)) {
            return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_rate_limit_invalid", "The provider rate limit is declared twice", limit.providerId}));
        }
        if (limit.minimumIntervalMs < 0 || limit.minimumIntervalMs > aiLimits().maximumRequestDelayMs || limit.maximumRequestsPerMinute < 0 || limit.maximumRequestsPerMinute > aiLimits().maximumRequestsPerMinute || limit.maximumConcurrentRequests < 0 || limit.maximumConcurrentRequests > aiLimits().maximumConcurrentRequests) {
            return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_rate_limit_invalid", "The provider rate limit is out of range", limit.providerId}));
        }
...
QFuture<utils::Result<void>> AiPlugin::loadConversation(const QString& taskId) {
    if (m_repository == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_not_initialized", "The AI plugin is not initialized", {}}));
    }
    ...
    // clang-format off
    const auto loaded = [this, taskId](utils::Result<QVector<ConversationMessage>> result) {
        if (!result.hasValue()) {
            return utils::Result<void>::failure(result.error());
        }
...
QFuture<utils::Result<bool>> AiPlugin::loadOlderConversation(const QString& taskId) {
    ...
    // clang-format off
    const auto prepended = [this, taskId](utils::Result<QVector<ConversationMessage>> result) {
        if (!result.hasValue()) {
...
QFuture<utils::Result<void>> AiPlugin::recordConversation(const QString& taskId, const QVector<ConversationMessage>& messages) {
    if (messages.isEmpty()) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::success());
    }
    ...
    // clang-format off
    const auto written = [this, taskId, messages](utils::Result<void> result) {
        if (result.hasValue()) {
            return result;
        }
...
QFuture<utils::Result<void>> AiPlugin::sendMessage(const QString& taskId, const QString& text) {
    ...
    if (target->executionKind != TaskExecutionKind::Agent) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_conversation_invalid", "A command task holds no conversation", taskId}));
    }

    auto future = loadConversation(taskId);
    // clang-format off
    const auto queued = [this, taskId, text](utils::Result<void> result) {
        if (!result.hasValue()) {
            return result;
        }
        if (runState(taskId) == TaskRunState::Idle) {
            ...
            // clang-format off
            auto started = written.then(m_asyncContext.get(), [this, taskId](utils::Result<void> stored) { return stored.hasValue() ? enqueueRun(taskId) : QtFuture::makeReadyValueFuture(stored); }).unwrap();
            started.then(m_asyncContext.get(), [](utils::Result<void>) {});
            // clang-format on
            return utils::Result<void>::success();
        }
...
QFuture<utils::Result<void>> AiPlugin::resetConversation(const QString& taskId) {
    ...
    // clang-format off
    const auto cleared = [this, taskId](utils::Result<void> result) {
        if (!result.hasValue()) {
            return result;
        }
...
QFuture<utils::Result<void>> AiPlugin::saveAgents(const QVector<AiAgent>& agents) {
    ...
    auto future = persistSettings(std::move(next));
    // clang-format off
    return future.then(m_asyncContext.get(), [this, orphaned](utils::Result<void> result) { if (result.hasValue()) { stopOrphanedTasks(orphaned); } return result; });
    // clang-format on
}
...
void AiPlugin::stopOrphanedTasks(const QStringList& taskIds) {
    for (const auto& taskId : taskIds) {
        ...
        if (orphan == nullptr || findAgent(m_settings.agents, orphan->agentId) != nullptr) {
            continue;
        }
...
QFuture<utils::Result<QString>> AiPlugin::createWorkspace(const QString& name) {
    if (m_repository == nullptr || name.trimmed().isEmpty()) {
        return QtFuture::makeReadyValueFuture(utils::Result<QString>::failure({"ai_tasks_workspace_invalid", "The AI workspace name is invalid", name}));
    }
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, workspace](utils::Result<void> result) {
        if (!result.hasValue()) {
            return utils::Result<QString>::failure(result.error());
        }
        ...
        return utils::Result<QString>::success(workspace.id);
    });
    // clang-format on
}

QFuture<utils::Result<void>> AiPlugin::renameWorkspace(const QString& workspaceId, const QString& name) {
    if (m_repository == nullptr || name.trimmed().isEmpty()) {
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, workspaceId, name = name.trimmed(), now](utils::Result<void> result) {
        if (result.hasValue()) {
...
QFuture<utils::Result<void>> AiPlugin::removeWorkspace(const QString& workspaceId) {
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, removedTaskIds](utils::Result<void> result) {
        if (!result.hasValue()) {
            return result;
        }
...
QFuture<utils::Result<void>> AiPlugin::activateWorkspace(const QString& workspaceId) {
    if (m_repository == nullptr) {
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, workspaceId](utils::Result<void> result) {
        if (result.hasValue()) {
            for (auto& workspace : m_workspaces) {
...
QFuture<utils::Result<void>> AiPlugin::saveTask(AiTask task) {
    if (m_repository == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_not_initialized", "The AI plugin is not initialized", task.id}));
    }
    if (runState(task.id) != TaskRunState::Idle) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_task_busy", "A running AI task cannot be edited", task.id}));
    }
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, stored](utils::Result<void> result) {
        if (result.hasValue()) {
...
QFuture<utils::Result<void>> AiPlugin::removeTask(const QString& taskId) {
    if (m_repository == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_not_initialized", "The AI plugin is not initialized", taskId}));
    }
    if (runState(taskId) != TaskRunState::Idle) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_task_busy", "A running AI task cannot be removed", taskId}));
    }

    auto future = m_repository->removeTask(taskId);
    // clang-format off
    return future.then(m_asyncContext.get(), [this, taskId](utils::Result<void> result) {
        if (result.hasValue()) {
            forgetTask(taskId);
...
QFuture<utils::Result<void>> AiPlugin::moveTask(const QString& taskId, TaskColumn column) {
    ...
    // clang-format off
    return future.then(m_asyncContext.get(), [this, taskId, column, position, now](utils::Result<void> result) {
        if (result.hasValue()) {
...
QFuture<utils::Result<void>> AiPlugin::startTask(const QString& taskId) {
    ...
    if (started->executionKind == TaskExecutionKind::Agent) {
        ...
        if (!connection.hasValue()) {
            return QtFuture::makeReadyValueFuture(utils::Result<void>::failure(connection.error()));
        }
    }
    ...
    // clang-format off
    const auto prompted = [this, taskId](utils::Result<void> result) {
        if (!result.hasValue()) {
        ...
        // clang-format off
        return written.then(m_asyncContext.get(), [this, taskId](utils::Result<void> stored) { return stored.hasValue() ? enqueueRun(taskId) : QtFuture::makeReadyValueFuture(stored); }).unwrap();
        // clang-format on
    };
    // clang-format on
...
QFuture<utils::Result<void>> AiPlugin::stopTask(const QString& taskId) {
    if (m_repository == nullptr || task(taskId) == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_task_unknown", "The AI task is unknown", taskId}));
    }
    if (runState(taskId) == TaskRunState::Idle) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"ai_tasks_task_idle", "The AI task is not queued or running", taskId}));
    }
...
void AiPlugin::startExecution(const AiTask& task) {
    ...
    // clang-format off
    started.then(m_asyncContext.get(), [this](utils::Result<void> result) { if (!result.hasValue()) { reportFailure(result.error(), m_host->translate(QStringLiteral("ai.error.execution-save"))); } });
    // clang-format on
...
void AiPlugin::startAgentExecution(const AiTask& task, const QString& executionId) {
    ...
    appendLog(executionId, ExecutionLogLevel::Info, ExecutionLogKind::Started, connectionKey(execution->connection));
    ...
    connect(client, &AiChatClient::finished, this, [this, taskId, executionId](const QString& content, const QVector<ToolCall>& calls, ChatUsage usage, const QString& finishReason) {
        ...
        if (truncatedByOutputBudget(finishReason)) {
    ...
    connect(client, &AiChatClient::failed, this, [this, taskId, executionId](const utils::Error& error) {
        ...
        const QString message = error.code == QStringLiteral("ai_output_truncated") ? m_host->translate(QStringLiteral("ai.error.output-truncated")).arg(QString::number(outputBudget(m_active.value(taskId) == nullptr ? ModelConnection{} : m_active.value(taskId)->connection))) : error.message;
    ...
    // The skills of the working directory are read before the first turn, because the prompt lists what the agent may load.
    m_tools->discoverSkills(task.workdir, [this, taskId, executionId, task, resolvedAgent = agent.value()](const QVector<SkillDescriptor>& skills) {
...
void AiPlugin::summarizeDroppedTurns(const QString& taskId, const FittedConversation& fitted) {
    ...
    setOutputBudget(connection, aiLimits().summaryMaximumTokens);
...
void AiPlugin::handleToolCalls(const QString& taskId, const QString& content, const QVector<ToolCall>& calls) {
    ...
    for (const auto& call : calls) {
        ...
        if (++position->toolCallSignatures[signature] > aiLimits().repeatedToolCallLimit) {
    ...
    for (;;) {
        ...
        for (int index = 0; index < position->toolCalls.size() && candidate < 0; ++index) {
            ...
            for (const auto& other : position->toolCalls) {
                if (other.started && !other.finished && toolAccessesConflict(position->toolCalls.at(index).access, other.access)) {
                    blocked = true;
                    break;
                }
            }
...
const QVector<McpServerDescriptor>& AiPlugin::mcpServers() const {
...
QFuture<utils::Result<void>> AiPlugin::saveSearchSettings(const SearchSettings& settings) {
    AiSettings next = m_settings;
...
QFuture<utils::Result<void>> AiPlugin::saveMcpServers(const QVector<McpServerDescriptor>& servers) {
    AiSettings next = m_settings;
    next.mcpServers = servers;
    auto future = persistSettings(std::move(next));
    // clang-format off
    return future.then(m_asyncContext.get(), [this](utils::Result<void> result) { if (result.hasValue()) { restartMcpClients(); } return result; });
    // clang-format on
}
...
void AiPlugin::restartMcpClients() {
    ...
    for (const auto& server : m_settings.mcpServers) {
        auto* client = new AiMcpClient(server, this);
        // clang-format off
        client->setSamplingHandler([this](const QJsonObject& parameters, int maximumTokens, McpReply reply) { runSampling(parameters, maximumTokens, reply); });
        connect(client, &AiMcpClient::toolsChanged, this, [this]() { if (m_tools != nullptr) { m_tools->setMcpClients(m_mcpClients.values()); } });
        connect(client, &AiMcpClient::failed, this, [this](const utils::Error& error) { reportFailure(error, m_host->translate(QStringLiteral("ai.error.server-failed"))); });
        connect(client, &AiMcpClient::progressReported, this, [this, server](const QString& token, double progress, double total, const QString& message) { m_host->log(LogLevel::Info, QStringLiteral("mcp"), message.isEmpty() ? token : message, {{QStringLiteral("server"), server.id}, {QStringLiteral("progress"), progress}, {QStringLiteral("total"), total}}); });
        // clang-format on
...
QString AiPlugin::systemPromptData(const AiTask& task, const QVector<SkillDescriptor>& skills) const {
...
QString AiPlugin::skillCatalog(const QVector<SkillDescriptor>& skills) const {
    if (skills.isEmpty()) {
        return {};
    }
...
QString AiPlugin::renderedSystemPrompt(const AiTask& task, const AiAgent& agent, const QVector<SkillDescriptor>& skills) const {
    const QDateTime now = QDateTime::currentDateTime();
    const QLocale locale = QLocale::system();
    ...
    values.insert(QStringLiteral("CONTEXT_FILES"), readContextFiles(task.workdir).join(QStringLiteral("\n\n")));
    ...
    return renderPrompt(agent.systemPrompt, values);
}

// A model that declares no system role receives the instructions as the first user message, which is what the market does.
QJsonObject AiPlugin::instructionMessage(const AiTask& task, const AiAgent& agent, const ModelConnection& connection, const QVector<SkillDescriptor>& skills) const {
    const ProviderDescriptor* provider = findProvider(connection.providerId);
    const QSet<ModelTrait> traits = provider == nullptr ? QSet<ModelTrait>{} : modelTraits(*provider, connection.modelId);
...
QJsonArray AiPlugin::projectConversation(const QJsonObject& instructions, const ModelConnection& connection, const QVector<ConversationMessage>& conversation, const QHash<QString, ToolResult>& images, QVector<qint64>* sequences) const {
    const WireProtocol protocol = connectionProtocol(connection);
    const ProviderDescriptor* provider = findProvider(connection.providerId);
    const bool sees = provider != nullptr && modelTraits(*provider, connection.modelId).contains(ModelTrait::Vision);
    ...
    const auto flushResults = [&messages, &pendingResults, protocol]() {
        ...
        for (const auto& message : serializeToolResults(protocol, pendingResults)) {
    ...
    for (const auto& message : conversation) {
        ...
        case ConversationRole::Assistant:
            messages.append(serializeAssistantTurn(protocol, message.content, AiPluginHelper::toolCallsFromDocument(message.toolCalls)));
            break;
...
QJsonArray AiPlugin::shapeForProtocol(const ModelConnection& connection, const QJsonArray& messages) {
    return enforceProtocolShape(connectionProtocol(connection), messages);
}

// The nearest published context file wins, so a subproject overrides the root instructions the way the format defines.
QStringList AiPlugin::readContextFiles(const QString& workdir) const {
    if (workdir.isEmpty()) {
        return {};
    }
    ...
    for (const auto& name : {QStringLiteral("AGENTS.md"), QStringLiteral("AGENT.md"), QStringLiteral("CLAUDE.md")}) {
        ...
        const QString content = QString::fromUtf8(file.read(maximumContextFileBytes)).trimmed();
        if (!content.isEmpty()) {
...
const ModelDescriptor* AiPlugin::findModelDescriptor(const ModelConnection& connection) const {
    ...
    return descriptor == nullptr ? nullptr : findModel(*descriptor, connection.modelId);
}
...
void AiPlugin::completeExecution(const QString& taskId, ExecutionStatus status, const QString& errorMessage, AgentStopReason stopReason) {
    ...
    // clang-format off
    persisted.then(m_asyncContext.get(), [this](utils::Result<void> result) { if (!result.hasValue()) { reportFailure(result.error(), m_host->translate(QStringLiteral("ai.error.execution-save"))); } });
    // clang-format on
    ...
    // clang-format off
    future.then(m_asyncContext.get(), [this, taskId, target, now](utils::Result<void> result) {
        if (!result.hasValue()) {
...
void AiPlugin::appendLog(const QString& executionId, ExecutionLogLevel level, ExecutionLogKind kind, const QString& detail) {
    ...
    // clang-format off
    future.then(m_asyncContext.get(), [this](utils::Result<void> result) { if (!result.hasValue()) { reportFailure(result.error(), m_host->translate(QStringLiteral("ai.error.log-save"))); } });
    // clang-format on
...
void AiPlugin::reportFailure(const utils::Error& error, const QString& message) {
    if (m_host == nullptr) {
        return;
    }
