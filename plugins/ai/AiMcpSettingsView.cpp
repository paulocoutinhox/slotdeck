...
AiMcpServerDialog::AiMcpServerDialog(PluginHost& host, McpServerDescriptor server, QStringList takenIdentifiers, QWidget* parent) : QDialog(parent), m_host(host), m_takenIdentifiers(std::move(takenIdentifiers)) {
    ...
    ui::sortComboBoxItems(m_transport);
    m_transport->setCurrentIndex(m_transport->findData(server.transport == McpTransport::Http ? QStringLiteral("http") : QStringLiteral("stdio")));
    ...
    m_samplingTokens->setRange(0, aiLimits().maximumSamplingTokens);
    ...
    form->addRow(m_host.translate(QStringLiteral("ai.mcp.sampling-tokens")), ui::stepperRow(m_samplingTokens, m_host.theme(), this));
...
McpServerDescriptor AiMcpServerDialog::server() const {
    McpServerDescriptor server;
    server.id = m_identifier->text().trimmed();
    server.transport = m_transport->currentData().toString() == QStringLiteral("http") ? McpTransport::Http : McpTransport::Stdio;
...
void AiMcpServerDialog::accept() {
    const McpServerDescriptor candidate = server();
    QString message;
    ...
    } else if (candidate.transport == McpTransport::Stdio && candidate.command.isEmpty()) {
        message = m_host.translate(QStringLiteral("ai.validation.mcp-command"));
    } else if (candidate.transport == McpTransport::Http && !AiMcpSettingsViewHelper::isWebAddress(candidate.url)) {
...
AiMcpSettingsView::AiMcpSettingsView(AiPlugin& plugin, PluginHost& host, QWidget* parent) : QWidget(parent), m_plugin(plugin), m_host(host), m_servers(plugin.mcpServers()) {
    ...
    auto* add = ui::toolButton(ui::IconName::Add, m_host.theme(), m_host.translate(QStringLiteral("ai.mcp.add")), actions);
    add->setObjectName(QStringLiteral("aiMcpAdd"));
    auto* edit = ui::toolButton(ui::IconName::Edit, m_host.theme(), m_host.translate(QStringLiteral("ai.mcp.edit")), actions);
    edit->setObjectName(QStringLiteral("aiMcpEdit"));
    auto* remove = ui::toolButton(ui::IconName::Close, m_host.theme(), m_host.translate(QStringLiteral("ai.mcp.remove")), actions);
    ...
    m_empty = ui::emptyStateLabel(m_host.translate(QStringLiteral("ai.mcp.empty")), page);
...
void AiMcpSettingsView::addServer() {
    ...
    QVector<McpServerDescriptor> updated = m_servers;
...
void AiMcpSettingsView::persist(const QVector<McpServerDescriptor>& servers) {
    auto future = m_plugin.saveMcpServers(servers);
    // clang-format off
    future.then(this, [this, servers](utils::Result<void> result) { if (!result.hasValue()) { m_host.notify(m_host.translate(QStringLiteral("ai.plugin.title")), m_host.translate(QStringLiteral("ai.error.mcp-save")), AlertSeverity::Error); return; } m_servers = servers; rebuild(); });
    // clang-format on
}
