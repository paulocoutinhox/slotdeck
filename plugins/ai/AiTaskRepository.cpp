...
QFuture<utils::Result<void>> AiTaskRepository::startExecution(const TaskExecution& execution) {
    ...
    return m_host.executeDatabase(QStringLiteral("INSERT INTO ai_tasks_executions(id, task_id, status, started_at_utc, finished_at_utc, input_tokens, output_tokens, finish_reason, error_message, content, stop_reason, provider_id, model_id) VALUES(?, ?, ?, ?, NULL, 0, 0, '', '', '', 'answered', ?, ?)"), {execution.id, execution.taskId, executionStatusName(ExecutionStatus::Running), persistence::storedTimestamp(execution.startedAtUtc), persistence::storedText(execution.providerId), persistence::storedText(execution.modelId)});
}
