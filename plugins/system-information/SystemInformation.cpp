...
class SystemInformationHelper final {
  public:
    ...
    static utils::Result<SystemSnapshot> cancelledCollection();
...
utils::Result<SystemSnapshot> SystemInformationHelper::cancelledCollection() {
    return utils::Result<SystemSnapshot>::failure({"system_information_collection_cancelled", "System information collection was cancelled", {}});
}
...
utils::Result<SystemSnapshot> HwinfoSystemInformationProvider::collect(const std::atomic_bool& cancelled) {
...
SystemInformationCollection collectSystemInformation(std::shared_ptr<SystemInformationProvider> provider) {
    ...
    // clang-format off
    auto future = QtConcurrent::run([provider = std::move(provider), cancelled]() { try { return provider->collect(*cancelled); } catch (const std::exception& exception) { return utils::Result<SystemSnapshot>::failure({"system_information_collection_failed", "System information could not be collected", QString::fromUtf8(exception.what())}); } });
    // clang-format on
