...
Section SystemInformationViewHelper::createSection(QWidget* parent, PluginHost& host, const QString& titleKey, ui::IconName iconName) {
    ...
    icon->setPixmap(ui::icon(iconName, host.theme()).pixmap(18, 18));
...
void SystemInformationView::render(const SystemSnapshot& snapshot) {
    clearContent();
    m_updated->setText(m_host.translate(QStringLiteral("system-information.view.updated")).arg(ui::localTimestamp(snapshot.capturedAtUtc)));
