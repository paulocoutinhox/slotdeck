...
QVector<NavigationItem> SystemInformationPlugin::navigationItems(const ui::Theme& theme) const {
    return {{QStringLiteral("overview"), QStringLiteral("system-information.navigation.overview"), ui::icon(ui::IconName::System, theme), NavigationPlacement::Secondary, NavigationOrder::System}};
}
...
utils::Result<void> SystemInformationPlugin::initialize(PluginHost& host) {
    if (m_host != nullptr) {
        return utils::Result<void>::failure({"system_information_already_initialized", "The System Information plugin is already initialized", {}});
    }
    if (m_provider == nullptr) {
        return utils::Result<void>::failure({"system_information_provider_unavailable", "The System Information provider is unavailable", {}});
    }
...
utils::Result<void> SystemInformationPlugin::refresh() {
    if (m_host == nullptr || m_asyncContext == nullptr) {
        return utils::Result<void>::failure({"system_information_not_initialized", "The System Information plugin is not initialized", {}});
    }
    if (m_refreshing) {
        return utils::Result<void>::failure({"system_information_refresh_in_progress", "System information is already being collected", {}});
    }

    m_refreshing = true;
    emit refreshStateChanged(true);
    const quint64 generation = ++m_collectionGeneration;
    m_collection.emplace(collectSystemInformation(m_provider));
    auto future = m_collection->future;
    // clang-format off
    future.then(m_asyncContext.get(), [this, generation](utils::Result<SystemSnapshot> result) { completeRefresh(generation, std::move(result)); });
    // clang-format on
    return utils::Result<void>::success();
}

void SystemInformationPlugin::completeRefresh(quint64 generation, utils::Result<SystemSnapshot> result) {
