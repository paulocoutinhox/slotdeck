...
utils::Result<void> ConfigurationTransferHelper::copyAtomically(const QString& sourcePath, const QString& destinationPath) {
    ...
    while (!source.atEnd()) {
        ...
        if (chunk.isEmpty() && source.error() != QFileDevice::NoError) {
            ...
            return utils::Result<void>::failure({"configuration_source_read_failed", "The configuration source could not be read", source.errorString()});
        }
        if (destination.write(chunk) != chunk.size()) {
            destination.cancelWriting();
            return utils::Result<void>::failure({"configuration_destination_write_failed", "The configuration destination could not be written", destination.errorString()});
        }
    }
...
utils::Result<void> ConfigurationTransferHelper::validateDatabase(const QString& filePath, const QHash<QString, int>& pluginSchemaVersions) {
    ...
    const QString connectionName = QStringLiteral("slotdeck-configuration-validation-%1").arg(QUuid::createUuid().toString(QUuid::WithoutBraces));
    ...
    if (!database.open()) {
        ...
        return utils::Result<void>::failure({"configuration_database_open_failed", "The configuration database could not be opened", detail});
    }

    bool integrityValid = false;
    utils::Result<QHash<QString, int>> coreValidation = utils::Result<QHash<QString, int>>::failure({"configuration_database_invalid", "The configuration database is invalid or incompatible", filePath});
    {
        ...
        coreValidation = validateCoreDatabase(database);
    }
    database.close();
    database = {};
    QSqlDatabase::removeDatabase(connectionName);
...
utils::Result<void> ConfigurationTransferHelper::createSnapshot(const QString& databasePath, const QString& destinationPath) {
    ...
    const QString connectionName = QStringLiteral("slotdeck-configuration-export-%1").arg(QUuid::createUuid().toString(QUuid::WithoutBraces));
    QSqlDatabase database = QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), connectionName);
    database.setConnectOptions(QStringLiteral("QSQLITE_OPEN_READONLY"));
    ...
    if (!database.open()) {
        ...
        database = {};
        QSqlDatabase::removeDatabase(connectionName);
        return utils::Result<void>::failure({"configuration_database_open_failed", "The application configuration could not be opened", detail});
    }
    ...
    const QString snapshotConnectionName = QStringLiteral("slotdeck-configuration-snapshot-%1").arg(QUuid::createUuid().toString(QUuid::WithoutBraces));
...
QFuture<utils::Result<void>> ConfigurationTransfer::exportDatabase(const QString& databasePath, const QString& destinationPath) {
    // clang-format off
    return QtConcurrent::run([databasePath, destinationPath]() { return exportDatabaseNow(databasePath, destinationPath); });
    // clang-format on
}

utils::Result<void> ConfigurationTransfer::exportDatabaseNow(const QString& databasePath, const QString& destinationPath) {
    return ConfigurationTransferHelper::createSnapshot(databasePath, destinationPath);
}

QFuture<utils::Result<void>> ConfigurationTransfer::stageImport(const QString& sourcePath, const QString& pendingPath, const QHash<QString, int>& pluginSchemaVersions) {
    // clang-format off
    return QtConcurrent::run([sourcePath, pendingPath, pluginSchemaVersions]() {
        if (ConfigurationTransferHelper::fileIdentity(sourcePath) == ConfigurationTransferHelper::fileIdentity(pendingPath)) {
            return utils::Result<void>::failure({"configuration_import_conflict", "The configuration import source conflicts with the staging file", sourcePath});
        }
...
utils::Result<bool> ConfigurationTransfer::beginPendingImport(const QString& databasePath, const QString& pendingPath, const QString& backupPath, const QHash<QString, int>& pluginSchemaVersions) {
    if (QFileInfo::exists(backupPath)) {
        ...
        if (QFileInfo::exists(pendingPath) && !QFile::remove(pendingPath)) {
            return utils::Result<bool>::failure({"configuration_pending_remove_failed", "The interrupted configuration staging file could not be removed", pendingPath});
        }
        if (!QFile::remove(backupPath)) {
            return utils::Result<bool>::failure({"configuration_backup_remove_failed", "The interrupted configuration backup could not be removed", backupPath});
        }
    ...
    if (ConfigurationTransferHelper::fileIdentity(databasePath) == ConfigurationTransferHelper::fileIdentity(pendingPath) || ConfigurationTransferHelper::fileIdentity(databasePath) == ConfigurationTransferHelper::fileIdentity(backupPath) || ConfigurationTransferHelper::fileIdentity(pendingPath) == ConfigurationTransferHelper::fileIdentity(backupPath)) {
        return utils::Result<bool>::failure({"configuration_import_conflict", "The configuration import paths conflict", pendingPath});
    }
...
utils::Result<void> ConfigurationTransfer::finalizePendingImport(const QString& pendingPath, const QString& backupPath) {
    if (QFileInfo::exists(pendingPath) && !QFile::remove(pendingPath)) {
        return utils::Result<void>::failure({"configuration_pending_remove_failed", "The imported configuration staging file could not be removed", pendingPath});
    }
...
utils::Result<void> ConfigurationTransfer::rollbackPendingImport(const QString& databasePath, const QString& pendingPath, const QString& backupPath) {
    if (!QFileInfo::exists(backupPath)) {
    ...
    if (QFileInfo::exists(pendingPath) && !QFile::remove(pendingPath)) {
        return utils::Result<void>::failure({"configuration_pending_remove_failed", "The rejected configuration staging file could not be removed", pendingPath});
    }
    if (!QFile::remove(backupPath)) {
        return utils::Result<void>::failure({"configuration_backup_remove_failed", "The configuration backup could not be removed", backupPath});
    }
