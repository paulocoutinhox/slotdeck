...
class CoreDatabaseSchemaHelper final {
  public:
    ...
    static utils::Error queryError(const QSqlQuery& query);
};
...
utils::Error CoreDatabaseSchemaHelper::queryError(const QSqlQuery& query) {
    return {"database_query_failed", "The core database could not be validated", query.lastError().text()};
}

const QHash<QString, QString>& coreDatabaseTableSchemas() {
...
utils::Result<QHash<QString, int>> validateCoreDatabase(const QSqlDatabase& database) {
    ...
    while (plugins.next()) {
        ...
        if (!CoreDatabaseSchemaHelper::pluginIdPattern().match(pluginId).hasMatch() || pluginVersions.contains(pluginId) || plugins.value(1).toString() != QStringLiteral("integer") || !readStoredInteger(plugins.value(2), pluginVersion) || pluginVersion < 0 || pluginVersion > std::numeric_limits<int>::max()) {
            return utils::Result<QHash<QString, int>>::failure({"database_plugin_schema_invalid", "A plugin database schema version is invalid", pluginId});
        }
