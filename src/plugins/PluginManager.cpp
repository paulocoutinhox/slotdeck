...
class ScopedPluginHost final : public PluginHost {
  public:
    ...
    [[nodiscard]] bool pluginAvailable(const QString& pluginId) const override {
        return m_manager.pluginAvailable(pluginId);
    }
    ...
    void request(const QString& targetPluginId, const QString& topic, const QJsonObject& payload, QObject& callbackContext, PluginReply reply) override {
        m_manager.request(m_pluginId, targetPluginId, topic, payload, callbackContext, std::move(reply));
    }
...
constexpr auto coreSourceId = "slotdeck";
...
utils::Result<void> PluginManager::loadPlugins() {
    ...
    for (const auto& path : paths) {
        ...
        if (instance == nullptr) {
            const auto error = utils::Error{"plugin_load_failed", "A bundled plugin could not be loaded", loader->errorString()};
            unloadPlugins();
        ...
        if (interface == nullptr) {
            const auto error = utils::Error{"plugin_interface_invalid", "A bundled plugin has an invalid interface", loader->fileName()};
            loader->unload();
            unloadPlugins();
            return utils::Result<void>::failure(error);
        }
...
utils::Result<void> PluginManager::initialize(QString applicationDataPath, persistence::StateStore& stateStore, persistence::DatabaseExecutor& databaseExecutor) {
    ...
    if (applicationDataPath.isEmpty()) {
        return utils::Result<void>::failure({"plugin_data_path_invalid", "The plugin data path is invalid", {}});
    }
    ...
    while (!pending.isEmpty()) {
        ...
        for (qsizetype index = pending.size() - 1; index >= 0; --index) {
            ...
            for (const auto& dependencyId : loadedPlugin->dependencies()) {
                if (plugin(dependencyId) == nullptr) {
                    shutdown();
                    return utils::Result<void>::failure({"plugin_dependency_missing", "A plugin dependency is missing", QStringLiteral("%1 -> %2").arg(loadedPlugin->id(), dependencyId)});
                }
            ...
            if (pluginHost == nullptr) {
                shutdown();
                return utils::Result<void>::failure({"plugin_host_unavailable", "The plugin host is unavailable", loadedPlugin->id()});
            }
            ...
            if (loadedPlugin->databaseSchemaVersion() > 0) {
                ...
                if (!schemaVersion.hasValue() || schemaVersion.value() != loadedPlugin->databaseSchemaVersion()) {
                    loadedPlugin->shutdown();
                    shutdown();
                    return utils::Result<void>::failure(schemaVersion.hasValue() ? utils::Error{"plugin_database_version_mismatch", "The initialized plugin database schema does not match its declaration", loadedPlugin->id()} : schemaVersion.error());
                }
            }
        ...
        if (!progressed) {
            shutdown();
            return utils::Result<void>::failure({"plugin_dependency_cycle", "The plugin dependency graph contains a cycle", {}});
        }
    }
...
utils::Result<void> PluginManager::setLocale(const QString& localeName) {
...
QFuture<utils::Result<void>> PluginManager::saveSettings(const QString& ownerId, const QJsonObject& document) {
...
utils::Result<void> PluginManager::executeBootstrapDatabaseTransaction(const QString& pluginId, const QVector<persistence::DatabaseStatement>& statements) {
...
utils::Result<persistence::DatabaseRows> PluginManager::queryBootstrapDatabase(const QString& pluginId, const QString& statement, const QVariantList& bindings) const {
    if (m_stateStore == nullptr || plugin(pluginId) == nullptr) {
...
QFuture<utils::Result<void>> PluginManager::executeDatabase(const QString& pluginId, const QString& statement, const QVariantList& bindings) {
...
QFuture<utils::Result<void>> PluginManager::executeDatabaseTransaction(const QString& pluginId, const QVector<persistence::DatabaseStatement>& statements) {
    if (m_databaseExecutor == nullptr || plugin(pluginId) == nullptr) {
        return QtFuture::makeReadyValueFuture(utils::Result<void>::failure({"plugin_database_unavailable", "The plugin database is unavailable", pluginId}));
    }
...
QFuture<utils::Result<persistence::DatabaseRows>> PluginManager::queryDatabase(const QString& pluginId, const QString& statement, const QVariantList& bindings) {
    if (m_databaseExecutor == nullptr || plugin(pluginId) == nullptr) {
...
QFuture<utils::Result<QByteArray>> PluginManager::readFile(const QString& path, qint64 maximumBytes) {
    return m_fileSystem->readFile(path, maximumBytes);
}

QFuture<utils::Result<QVector<filesystem::DirectoryEntry>>> PluginManager::listDirectory(const QString& path, int maximumEntries) {
    return m_fileSystem->listDirectory(path, maximumEntries);
}

QFuture<utils::Result<void>> PluginManager::writeFile(const QString& path, const QByteArray& content) {
    return m_fileSystem->writeFile(path, content);
}

QFuture<utils::Result<void>> PluginManager::createFile(const QString& path) {
    return m_fileSystem->createFile(path);
}

QFuture<utils::Result<void>> PluginManager::createDirectory(const QString& path) {
    return m_fileSystem->createDirectory(path);
}

QFuture<utils::Result<void>> PluginManager::movePath(const QString& sourcePath, const QString& destinationPath) {
    return m_fileSystem->movePath(sourcePath, destinationPath);
}

QFuture<utils::Result<void>> PluginManager::copyFile(const QString& sourcePath, const QString& destinationPath) {
    return m_fileSystem->copyFile(sourcePath, destinationPath);
}

QFuture<utils::Result<void>> PluginManager::removeFile(const QString& path) {
    return m_fileSystem->removeFile(path);
}

QFuture<utils::Result<void>> PluginManager::removeDirectory(const QString& path) {
...
bool PluginManager::confirm(QWidget* parent, const QString& title, const QString& message, const QString& detail, const QString& action, bool destructive) const {
    return ui::ConfirmationDialog::confirm(parent, title, message, detail, translate(QStringLiteral("slotdeck.actions.cancel")), action, destructive);
}
...
void PluginManager::request(const QString& senderPluginId, const QString& targetPluginId, const QString& topic, const QJsonObject& payload, QObject& callbackContext, PluginReply reply) {
    ...
    // clang-format off
    const auto respond = [manager, requestId](utils::Result<QJsonObject> result) {
        if (manager == nullptr) {
            return;
        }
    ...
    QTimer::singleShot(0, this, [this, guardedTarget, targetPluginId, senderPluginId, topic, payload, respond]() mutable {
        ...
        if (!m_initialized || guardedTarget == nullptr || currentTarget == nullptr || dynamic_cast<QObject*>(currentTarget) != guardedTarget || !m_initializedPlugins.contains(currentTarget)) {
            respond(utils::Result<QJsonObject>::failure({"plugin_message_target_unavailable", "The target plugin is unavailable", targetPluginId}));
            return;
        }
...
void PluginManager::publishCoreEvent(const QString& topic, const QJsonObject& payload) {
    publish(QString::fromLatin1(coreSourceId), topic, payload);
}
...
utils::Result<void> PluginManager::registerPlugin(PluginInterface& loadedPlugin) {
    if (!PluginManagerHelper::contributionIdPattern().match(loadedPlugin.id()).hasMatch() || loadedPlugin.titleKey().isEmpty() || plugin(loadedPlugin.id()) != nullptr || loadedPlugin.databaseSchemaVersion() < 0) {
        return utils::Result<void>::failure({"plugin_metadata_invalid", "A bundled plugin has invalid metadata", loadedPlugin.id()});
    }
    ...
    for (const auto& item : navigationItems) {
        ...
        if (takenOrders.contains(static_cast<int>(item.order))) {
            return utils::Result<void>::failure({"plugin_navigation_order_taken", "The navigation position is already declared by another destination", item.id});
        }
    ...
    for (const auto& group : loadedPlugin.settingsGroups()) {
        ...
        if (group.sections.size() == 1 && group.sections.first().id != QStringLiteral("general")) {
            return utils::Result<void>::failure({"plugin_settings_general_required", "A single settings section must be General", group.id});
        }
        ...
        for (const auto& section : group.sections) {
            if (!PluginManagerHelper::contributionIdPattern().match(section.id).hasMatch() || section.titleKey.isEmpty() || !PluginManagerHelper::ownsTranslationKey(loadedPlugin.id(), section.titleKey) || sectionIds.contains(section.id) || section.searchKeys.isEmpty() || !englishTranslations.contains(section.titleKey)) {
                return utils::Result<void>::failure({"plugin_settings_section_invalid", "A plugin settings section is invalid", QStringLiteral("%1/%2").arg(group.id, section.id)});
            }
            ...
            for (const auto& searchKey : section.searchKeys) {
                if (!PluginManagerHelper::ownsTranslationKey(loadedPlugin.id(), searchKey) || searchKeys.contains(searchKey) || !englishTranslations.contains(searchKey)) {
                    return utils::Result<void>::failure({"plugin_settings_search_translation_missing", "A plugin settings search translation is missing", searchKey});
                }
...
bool PluginManager::pluginAvailable(const QString& pluginId) const {
