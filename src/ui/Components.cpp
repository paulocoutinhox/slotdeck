...
CalendarPopup::CalendarPopup(const Theme& theme, QWidget* parent) : QWidget(parent, Qt::Popup), m_theme(theme) {
    ...
    auto* previous = toolButton(IconName::Back, theme, {}, header);
    previous->setObjectName(QStringLiteral("calendarPrevious"));
    auto* next = toolButton(IconName::Forward, theme, {}, header);
...
SecretField::SecretField(const Theme& theme, const QString& placeholder, RevealConfirmation confirmReveal, QWidget* parent) : QWidget(parent), m_confirmReveal(std::move(confirmReveal)) {
    ...
    m_theme = &theme;
    m_reveal = toolButton(IconName::Visible, theme, placeholder, this);
...
void SecretField::applyReveal(bool revealed) {
    ...
    m_reveal->setIcon(icon(revealed ? IconName::Hidden : IconName::Visible, *m_theme));
...
AvatarBadge::AvatarBadge(IconName name, const QColor& fill, const QColor& ink, int diameter, QWidget* parent) : QWidget(parent), m_icon(icon(name, ink)), m_fill(fill), m_diameter(diameter) {
...
RoundedSurface::RoundedSurface(QColor fill, int radius, QWidget* parent) : QWidget(parent), m_fill(std::move(fill)), m_radius(radius) {
...
QTableWidgetItem* gridStatusItem(const QString& text, IconName name, ThemeColor role, const Theme& theme) {
    auto* item = new QTableWidgetItem(text);
    item->setData(iconNameRole, static_cast<int>(name));
    item->setData(iconRoleRole, static_cast<int>(role));
    item->setIcon(icon(name, theme.color(role)));
...
void setItemGlyph(QTreeWidgetItem* item, int column, IconName name, ThemeColor role, const Theme& theme) {
    ...
    item->setIcon(column, icon(name, theme.color(role)));
}
...
void repaintTreeGlyphs(QTreeWidget* tree, const Theme& theme) {
    ...
    while (*entry != nullptr) {
        ...
        for (int column = 0; column < tree->columnCount(); ++column) {
            ...
            item->setIcon(column, icon(name, ink));
        }
        ++entry;
    }
}

QToolButton* rowActionButton(IconName name, ThemeColor role, const Theme& theme, const QString& toolTip, QWidget* parent) {
    QToolButton* button = toolButton(icon(name, theme.color(role)), theme, toolTip, parent);
    ...
    for (int row = 0; row < grid->rowCount(); ++row) {
        ...
        for (int column = 0; column < grid->columnCount(); ++column) {
            ...
            for (auto* action : cell->findChildren<QToolButton*>()) {
                ...
                action->setIcon(icon(name, selected ? theme.color(ThemeColor::OnAccent) : resting));
            }
        }
    }
}

QTableWidget* dataGrid(const QStringList& headerLabels, QWidget* parent) {
    ...
    // clang-format off
    QObject::connect(settled, &QTimer::timeout, grid, [grid, &theme]() { repaintRowActions(grid, theme); });
...
QToolButton* toolButton(const QIcon& buttonIcon, const Theme& theme, const QString& toolTip, QWidget* parent) {
...
QToolButton* toolButton(IconName name, const Theme& theme, const QString& toolTip, QWidget* parent) {
    QToolButton* button = toolButton(icon(name, theme), theme, toolTip, parent);
    button->setProperty("iconName", static_cast<int>(name));
    return button;
}
...
QToolButton* chipButton(const QString& text, const Theme& theme, QWidget* parent) {
    auto* button = new QToolButton(parent);
...
QWidget* settingsSectionPage(QWidget* parent) {
...
QWidget* horizontalDivider(QWidget* parent) {
...
QWidget* verticalDivider(QWidget* parent) {
    auto* divider = new QWidget(parent);
    divider->setObjectName(QStringLiteral("sharedDivider"));
    divider->setAttribute(Qt::WA_StyledBackground, true);
...
QFormLayout* settingsForm() {
    const int inset = themeManager().theme().metric(ThemeMetric::SettingsHorizontalPadding);
...
QWidget* stepperRow(QAbstractSpinBox* box, const Theme& theme, QWidget* parent) {
    ...
    auto* decrease = toolButton(IconName::Minus, theme, QString{}, row);
    decrease->setObjectName(QStringLiteral("stepperButton"));
    auto* increase = toolButton(IconName::Add, theme, QString{}, row);
...
QWidget* settingsActionRow(QWidget* parent) {
    const int inset = themeManager().theme().metric(ThemeMetric::SettingsHorizontalPadding);
    auto* row = new QWidget(parent);
...
QLabel* sectionTitleLabel(const QString& text, QWidget* parent) {
    auto* label = new QLabel(text.toUpper(), parent);
    label->setObjectName(QStringLiteral("settingsSectionTitle"));
    return label;
}

QLabel* emptyStateLabel(const QString& text, QWidget* parent) {
...
const QStringList& monospacedFontFamilies() {
    // clang-format off
...
Qt::TextFlag labelWrapping(const QString& text, const QFontMetrics& metrics, int availableWidth) {
    return longestWordWidth(text, metrics) <= availableWidth ? Qt::TextWordWrap : Qt::TextWrapAnywhere;
}
