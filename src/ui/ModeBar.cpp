...
class ModeButton final : public QWidget {
  ...
  public:
    ModeButton(QString modeId, QIcon icon, QString title, QWidget* parent = nullptr) : QWidget(parent), m_modeId(std::move(modeId)), m_icon(std::move(icon)), m_title(std::move(title)) {
        ...
        setMinimumWidth(themeManager().theme().metric(ThemeMetric::ModeBarMinimumWidth));
    ...
    [[nodiscard]] int minimumReadableWidth() const {
        const int required = longestWordWidth(m_title, QFontMetrics(readableLabelFont())) + (themeManager().theme().metric(ThemeMetric::ModeButtonHorizontalPadding) * 2);
        return std::clamp(required, themeManager().theme().metric(ThemeMetric::ModeBarMinimumWidth), themeManager().theme().metric(ThemeMetric::ModeBarMaximumWidth));
    }

    void setContentWidth(int width) {
        const int horizontalPadding = themeManager().theme().metric(ThemeMetric::ModeButtonHorizontalPadding);
        ...
        const int contentHeight = themeManager().theme().metric(ThemeMetric::ModeButtonLabelTop) + textBounds.height() + themeManager().theme().metric(ThemeMetric::ModeButtonBottomPadding);

        setFixedHeight(std::max(themeManager().theme().metric(ThemeMetric::ModeButtonMinimumHeight), contentHeight));
    }
  ...
  private:
    [[nodiscard]] QFont readableLabelFont() const {
        return themeManager().theme().font(ThemeFont::Navigation);
    }

    [[nodiscard]] Qt::TextFlag wrapping(int textWidth) const {
        return labelWrapping(m_title, QFontMetrics(readableLabelFont()), textWidth);
    }
...
ModeBar::ModeBar(QWidget* parent) : QWidget(parent) {
    ...
    m_preferredWidth = themeManager().theme().metric(ThemeMetric::ModeBarMinimumWidth);
    setMinimumWidth(themeManager().theme().metric(ThemeMetric::ModeBarMinimumWidth));
