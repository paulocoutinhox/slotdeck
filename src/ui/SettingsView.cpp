...
SettingsView::SettingsView(plugins::PluginManager& pluginManager, QVector<CoreSettingsContribution> coreSettings, QWidget* parent) : QWidget(parent), m_pluginManager(pluginManager) {
    ...
    auto* header = new PageHeader(m_pluginManager.theme(), m_pluginManager.translate(QStringLiteral("slotdeck.settings.title")), this);
    header->addStretch();
    ...
    search->setPlaceholderText(m_pluginManager.translate(QStringLiteral("slotdeck.settings.search")));
    ...
    m_noResults = emptyStateLabel(m_pluginManager.translate(QStringLiteral("slotdeck.settings.no-results")), body);
    m_noResults->hide();
...
bool SettingsView::appendGroup(const plugins::SettingsGroup& group, const SettingsSectionFactory& createSection) {
    ...
    for (const auto& section : group.sections) {
        if (&section != &group.sections.first()) {
            pageLayout->addWidget(horizontalDivider(page));
        }
        ...
        auto* sectionTitle = sectionTitleLabel(m_pluginManager.translate(section.titleKey), sectionHost);
