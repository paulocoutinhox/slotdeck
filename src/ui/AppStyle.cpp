...
QString applicationStyleSheet(const Theme& theme) {
    const QString rules = QStringLiteral(R"(
        ...
        QPushButton { min-width: 0; background: @raised; border: none; border-radius: @controlRadiuspx; padding: @controlVerticalPaddingpx @controlHorizontalPaddingpx; }
        QPushButton:hover { background: @hover; }
        QPushButton:pressed { background: @borderStrong; }
        QLineEdit, QComboBox, QAbstractSpinBox { min-height: 20px; background: @raised; border: 1px solid transparent; border-radius: @controlRadiuspx; padding: 4px 7px; selection-background-color: @accent; }
        ...
        slotdeck--ui--CalendarPopup { background: @panel; border: 1px solid @border; border-radius: @controlRadiuspx; }
        ...
        slotdeck--ui--TextField { background: @raised; border: 1px solid @borderStrong; border-radius: @controlRadiuspx; padding: 4px 7px; selection-background-color: @accent; }
        slotdeck--ui--TextField:focus { border-color: @accent; }
    ...
    return applyThemeTokens(QStringLiteral("QMainWindow, QWidget { color: @text; }") + rules, theme);
}

AppStyle::AppStyle(QStyle* baseStyle) : QProxyStyle(baseStyle) {}

QColor AppStyle::color(Color role) {
    return themeManager().theme().color(role);
}

QPalette AppStyle::applicationPalette() {
    return themeManager().theme().palette();
}
...
int AppStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const {
    ...
    case PM_SmallIconSize:
        return themeManager().theme().metric(ThemeMetric::SmallIconSize);
    case PM_ToolBarIconSize:
        return themeManager().theme().metric(ThemeMetric::SmallIconSize);
    case PM_ScrollBarExtent:
        return themeManager().theme().metric(ThemeMetric::ScrollBarExtent);
    case PM_SplitterWidth:
        return themeManager().theme().metric(ThemeMetric::SplitterWidth);
    case PM_TabBarTabHSpace:
        return themeManager().theme().metric(ThemeMetric::TabHorizontalPadding);
    case PM_TabBarTabVSpace:
        return themeManager().theme().metric(ThemeMetric::TabVerticalPadding);
    default:
...
void AppStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
    ...
    if (element == PE_IndicatorArrowDown || element == PE_IndicatorSpinMinus || element == PE_IndicatorSpinPlus) {
...
// A stepper reads as a plus and a minus, because a pair of tiny arrows says nothing at this size.
void AppStyle::drawSpinSign(const QStyleOption* option, QPainter* painter, bool plus) {
    const QPointF center = QRectF(option->rect).center();
    const qreal arm = std::min<qreal>(4.5, QRectF(option->rect).width() / 2.0);
