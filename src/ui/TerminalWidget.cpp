...
bool TerminalWidget::event(QEvent* event) {
    if (event->type() == QEvent::ShortcutOverride) {
        ...
        if (terminalcore::shortcuts::isTerminalOwned(*keyEvent)) {
            keyEvent->accept();
            return true;
        }
...
void TerminalWidget::dragEnterEvent(QDragEnterEvent* event) {
    if (m_session == nullptr || localPathsFromDrop(*event->mimeData()).isEmpty()) {
        event->ignore();
        return;
    }
...
void TerminalWidget::keyPressEvent(QKeyEvent* event) {
    ...
    if (terminalcore::shortcuts::isCopy(*event) && hasSelection()) {
        copySelection();
        event->accept();
        return;
    }

    if (terminalcore::shortcuts::isPaste(*event)) {
        pasteClipboard();
        event->accept();
        return;
    }

    if (terminalcore::shortcuts::isFind(*event)) {
        showFindBar();
        event->accept();
        return;
    }

    if (terminalcore::shortcuts::isFindNext(*event) || terminalcore::shortcuts::isFindPrevious(*event)) {
        stepMatch(terminalcore::shortcuts::isFindNext(*event));
        event->accept();
        return;
    }

    if (terminalcore::shortcuts::isSelectAll(*event)) {
        selectAll();
        event->accept();
        return;
    }

    if (terminalcore::shortcuts::isClearBuffer(*event)) {
        clearScrollback();
        event->accept();
        return;
    }
    ...
    if (!terminalcore::shortcuts::isTerminalOwned(*event) && terminalcore::shortcuts::isReservedForApplication(*event)) {
        event->ignore();
        return;
    }
...
void TerminalWidget::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton && event->modifiers().testFlag(terminalcore::shortcuts::applicationModifier)) {
...
void TerminalWidget::mouseMoveEvent(QMouseEvent* event) {
    ...
    const bool overLink = m_session != nullptr && event->modifiers().testFlag(terminalcore::shortcuts::applicationModifier) && !m_session->addressAt(gridPosition(event->position())).isEmpty();
