...
class GhosttyTerminalAdapterHelper final {
  public:
    ...
    static utils::Result<void> ghosttyFailure(const QString& operation, GhosttyResult result);
...
utils::Result<void> GhosttyTerminalAdapterHelper::ghosttyFailure(const QString& operation, GhosttyResult result) {
    return utils::Result<void>::failure({"ghostty_operation_failed", operation, QString::number(static_cast<int>(result))});
}
...
utils::Result<void> GhosttyTerminalAdapter::initialize(int columns, int rows, int cellWidth, int cellHeight, const domain::TerminalTheme& theme) {
...
utils::Result<void> GhosttyTerminalAdapter::setTheme(const domain::TerminalTheme& theme) {
    if (m_terminal == nullptr) {
        return utils::Result<void>::failure({"ghostty_not_initialized", "The terminal emulator is not initialized", {}});
    }
...
utils::Result<void> GhosttyTerminalAdapter::resize(int columns, int rows, int cellWidth, int cellHeight) {
    if (m_terminal == nullptr) {
        return utils::Result<void>::failure({"ghostty_not_initialized", "The terminal emulator is not initialized", {}});
    }
    if (!validTerminalGrid(columns, rows) || !validTerminalCellSize(cellWidth, cellHeight)) {
...
utils::Result<QByteArray> GhosttyTerminalAdapter::encodeKey(const QKeyEvent& event) {
    if (m_terminal == nullptr || m_keyEncoder == nullptr || m_keyEvent == nullptr) {
        return utils::Result<QByteArray>::failure({"ghostty_not_initialized", "The terminal emulator is not initialized", {}});
    }
...
GhosttyString GhosttyTerminalAdapter::xtversionCallback(GhosttyTerminal, void*) {
    static constexpr QLatin1StringView version(SLOTDECK_PRODUCT_NAME " " SLOTDECK_APP_VERSION);
...
utils::Result<QByteArray> GhosttyTerminalAdapter::encodePaste(const QByteArray& text) const {
    if (m_terminal == nullptr) {
        return utils::Result<QByteArray>::failure({"ghostty_not_initialized", "The terminal emulator is not initialized", {}});
    }
    ...
    return utils::Result<QByteArray>::success(encoded);
}
...
utils::Result<void> GhosttyTerminalAdapter::applyGesture(GhosttySelectionGestureEvent event) {
...
utils::Result<void> GhosttyTerminalAdapter::beginSelection(const QPointF& position, quint64 timeNanoseconds, quint64 repeatIntervalNanoseconds, double repeatDistance, bool rectangle) {
...
utils::Result<void> GhosttyTerminalAdapter::extendSelection(const QPointF& position, bool rectangle) {
    if (m_terminal == nullptr || m_gesture == nullptr) {
        return utils::Result<void>::failure({"ghostty_not_initialized", "The terminal emulator is not initialized", {}});
    }
...
#if defined(emit)
#undef emit
#define SLOTDECK_TERMINAL_RESTORE_EMIT
#endif
...
#if defined(SLOTDECK_TERMINAL_RESTORE_EMIT)
#undef SLOTDECK_TERMINAL_RESTORE_EMIT
#define emit
#endif
...
utils::Result<QByteArray> GhosttyTerminalAdapter::encodeMouse(const MouseReport& report) {
...
utils::Result<QByteArray> GhosttyTerminalAdapter::encodeFocus(bool gained) const {
...
utils::Result<void> GhosttyTerminalAdapter::applyTheme(const domain::TerminalTheme& theme) {
