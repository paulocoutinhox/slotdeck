...
utils::Result<void> TerminalSession::start() {
...
utils::Result<void> TerminalSession::write(const QByteArray& bytes) {
    return m_backend->write(bytes);
}

utils::Result<void> TerminalSession::writeLocalPaths(const QStringList& paths) {
    return write(formatLocalPathsForShell(m_profile, paths).toUtf8());
}
...
utils::Result<void> TerminalSession::beginSelection(const QPointF& position, quint64 timeNanoseconds, quint64 repeatIntervalNanoseconds, double repeatDistance, bool rectangle) {
    const auto result = m_emulator.beginSelection(position, timeNanoseconds, repeatIntervalNanoseconds, repeatDistance, rectangle);
    emit renderChanged();
    return result;
}

utils::Result<void> TerminalSession::extendSelection(const QPointF& position, bool rectangle) {
...
utils::Result<void> TerminalSession::advanceSelectionAutoscroll(const QPointF& position, bool rectangle) {
...
utils::Result<void> TerminalSession::resize(int columns, int rows, int cellWidth, int cellHeight) {
...
std::unique_ptr<IPtyBackend> createSystemPtyBackend() {
#ifdef Q_OS_WIN
