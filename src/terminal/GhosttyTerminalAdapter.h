...
class GhosttyTerminalAdapter final : public QObject {
  ...
  public:
    ...
    [[nodiscard]] utils::Result<void> initialize(int columns, int rows, int cellWidth, int cellHeight, const domain::TerminalTheme& theme);
    [[nodiscard]] utils::Result<void> setTheme(const domain::TerminalTheme& theme);
    void write(const QByteArray& bytes);
    [[nodiscard]] utils::Result<void> resize(int columns, int rows, int cellWidth, int cellHeight);
    [[nodiscard]] utils::Result<QByteArray> encodeKey(const QKeyEvent& event);
    ...
    [[nodiscard]] utils::Result<QByteArray> encodePaste(const QByteArray& text) const;
    ...
    [[nodiscard]] utils::Result<QByteArray> encodeMouse(const MouseReport& report);
    [[nodiscard]] utils::Result<QByteArray> encodeFocus(bool gained) const;
    ...
    [[nodiscard]] utils::Result<void> beginSelection(const QPointF& position, quint64 timeNanoseconds, quint64 repeatIntervalNanoseconds, double repeatDistance, bool rectangle);
    [[nodiscard]] utils::Result<void> extendSelection(const QPointF& position, bool rectangle);
    void endSelection(const QPointF& position);
    [[nodiscard]] SelectionAutoscroll selectionAutoscroll() const;
    [[nodiscard]] utils::Result<void> advanceSelectionAutoscroll(const QPointF& position, bool rectangle);
    void selectAll();
    void clearSelection();
  ...
  private:
    ...
    [[nodiscard]] utils::Result<void> applyGesture(GhosttySelectionGestureEvent event);
    ...
    [[nodiscard]] utils::Result<void> applyTheme(const domain::TerminalTheme& theme);
    void release();
