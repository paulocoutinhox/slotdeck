...
constexpr auto applicationSettingsOwner = "slotdeck";
...
utils::Result<void> ApplicationSettingsStore::initialize() {
    ...
    const domain::ApplicationSettings declared{{}, domain::resolveApplicationLanguage(QLocale::system().name(QLocale::TagSeparator::Dash)), ui::themeManager().catalog().defaultTheme().id()};
...
utils::Result<void> ApplicationSettingsStore::setLanguage(const QString& language) {
    if (!domain::isSupportedApplicationLanguage(language)) {
        return utils::Result<void>::failure({"application_language_invalid", "The application language is unsupported", language});
    }
...
utils::Result<void> ApplicationSettingsStore::setTheme(const QString& themeId) {
    if (!ui::themeManager().catalog().contains(themeId)) {
...
void ApplicationSettingsStore::save(domain::ApplicationSettings settings) {
    ...
    // clang-format off
    future.then(this, [this, candidate, revision](utils::Result<void> result) {
        if (result.hasValue()) {
